! Test program for SSYRK vector reverse (BLAS3 outlined)
program test_ssyrk_vector_reverse
  implicit none
  external :: ssyrk
  external :: ssyrk_bv
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing SSYRK (Vector Reverse, multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
    nbdirs = n_test
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    call ssyrk_bv(uplo, transa, nsize, ksize, alpha, alphab, a, ab, lda_val, beta, betab, c, cb, ldc_val, nbdirs)
    call set_ISIZE2OFA(-1)
    write(*,*) 'Testing SSYRK (Vector Reverse, n =', n, ')'
    ...
    do k = 1, nbdirs
      a_t = a + h * ab(k,:,:)
      c_plus = c
      call ssyrk(uplo, transa, nsize, ksize, alpha + h*alphab(k), a_t, lda_val, beta + h*betab(k), c_plus, ldc_val)
      a_t = a - h * ab(k,:,:)
      c_minus = c
      call ssyrk(uplo, transa, nsize, ksize, alpha - h*alphab(k), a_t, lda_val, beta - h*betab(k), c_minus, ldc_val)
      vjp_fd = 0.0d0
...
end program test_ssyrk_vector_reverse