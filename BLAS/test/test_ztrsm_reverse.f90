! Test program for ZTRSM reverse (adjoint) mode differentiation
...
program test_ztrsm_reverse
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing ZTRSM (reverse mode)'
  all_passed = .true.
  do id = 1, 2
    if (id == 1) then
      diag = 'N'
...
contains
  ...
  subroutine run_test_for_size(n, diag, passed)
    ...
    complex(8), dimension(n,n) :: a, b, ab, bb
    complex(8) :: alpha_orig
    complex(8), dimension(n,n) :: a_orig, b_orig, bb_orig
    integer :: i
    ...
    call ztrsm_b(side, uplo, transa, diag, msize, nsize, alpha, alphab, &
  ...
  subroutine check_vjp_numerically(n, side, uplo, transa, diag, msize, nsize, &
    ...
    complex(8), intent(in) :: a_orig(n,n), b_orig(n,n), bb_orig(n,n)
    complex(8), intent(in) :: alphab, ab(n,n), bb(n,n)
    logical, intent(out) :: passed
    ...
    integer :: i, j
    complex(8) :: alpha_dir, alpha
    complex(8), dimension(n,n) :: a_dir, b_dir, a, b, x_plus, x_minus, x_cdiff
