! Test program for STBMV vector forward - BLAS2 band
! Generated automatically by run_tapenade_blas.py
! Multi-size outlined run_test_for_size(n, passed, nbdirs) - band

program test_stbmv_vector_forward
  implicit none
  external :: stbmv
  external :: stbmv_dv
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    write(*,*) 'Testing STBMV (Vector Forward band, n =', n, ')'
    a_orig = a
    x_orig = x
    a_dv_seed = a_dv
    x_dv_seed = x_dv
  ...
  subroutine check_derivatives_numerically_band_tri(n, nbdirs, lda_val, ksize, uplo, trans, diag, nsize, incx_val, a_orig, a_dv_seed, x_orig, x_dv_seed, x_dv_out, passed)
    ...
    real(4), intent(in) :: a_orig(lda_val, n), a_dv_seed(nbdirs, lda_val, n), x_orig(n), x_dv_seed(nbdirs, n), x_dv_out(nbdirs, n)
    logical, intent(out) :: passed
    real(4), parameter :: h = 1.0e-3
    real(4) :: abs_error, abs_ref, err_bound
    real(4) :: central_diff, ad_result
    logical :: has_err
    real(4), dimension(n) :: x_fwd, x_bwd, x_t
...
end program test_stbmv_vector_forward