! Test program for CSYMM differentiation (BLAS3 outlined)
...
contains
  subroutine run_test_for_size(n, passed)
    ...
    c_d = 0.0d0
    c_orig = c
    call csymm_d(side, uplo, msize, nsize, alpha, alpha_d, a, a_d, lda_val, b, b_d, ldb_val, beta, beta_d, c, c_d, ldc_val)
    ! Finite-difference check: (output(alpha+h) - output(alpha-h))/(2h) vs derivative
    c_plus = c_orig
    call csymm(side, uplo, msize, nsize, alpha + h, a, lda_val, b, ldb_val, beta, c_plus, ldc_val)
    c_minus = c_orig
    call csymm(side, uplo, msize, nsize, alpha - h, a, lda_val, b, ldb_val, beta, c_minus, ldc_val)
    max_err = 0.0d0
    do jj = 1, n
      do ii = 1, n
