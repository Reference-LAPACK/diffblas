! Test program for SSYRK vector forward (BLAS3 outlined)
program test_ssyrk_vector_forward
  implicit none
  external :: ssyrk
  external :: ssyrk_dv
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing SSYRK (Vector Forward, multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
    nbdirs = n_test
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    real(4), dimension(nbdirs,n,n) :: c_dv_seed
    ...
    uplo = 'L'
    transa = 'N'
    write(*,*) 'Testing SSYRK (Vector Forward, n =', n, ')'
    ...
    c_orig = c
    c_dv_seed = c_dv
    call ssyrk_dv(uplo, transa, nsize, ksize, alpha, alpha_dv, a, a_dv, lda_val, beta, beta_dv, c, c_dv, ldc_val, nbdirs)
    ...
    do k = 1, nbdirs
      ...
      call ssyrk(uplo, transa, nsize, ksize, alpha + h*alpha_dv(k), a_t, lda_val, beta + h*beta_dv(k), c_plus, ldc_val)
      a_t = a - h * a_dv(k,:,:)
      c_minus = c_orig - h * c_dv_seed(k,:,:)
      call ssyrk(uplo, transa, nsize, ksize, alpha - h*alpha_dv(k), a_t, lda_val, beta - h*beta_dv(k), c_minus, ldc_val)
      max_err = 0.0d0
      do jj = 1, n
        do ii = 1, n
...
end program test_ssyrk_vector_forward