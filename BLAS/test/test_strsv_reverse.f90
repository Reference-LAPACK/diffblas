! Test program for STRSV reverse (adjoint) mode differentiation
...
program test_strsv_reverse
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing STRSV (reverse mode)'
  all_passed = .true.
  do id = 1, 2
    if (id == 1) then
      diag = 'N'
...
contains
  ...
  subroutine run_test_for_size(n, diag, passed)
    ...
    real(4), dimension(n,n) :: a, ab
    real(4), dimension(n) :: x, xb
    real(4), dimension(n,n) :: a_orig
    real(4), dimension(n) :: x_orig, xb_orig
    integer :: i
    ...
    write(*,*) 'Testing STRSV (n =', n, ', diag = ', diag, ')'

    call strsv_b(uplo, trans, diag, nsize, a, ab, lda_val, x, xb, incx)
  ...
  subroutine check_vjp_numerically(n, uplo, trans, diag, nsize, lda_val, &
    ...
    real(4), intent(in) :: a_orig(n,n), x_orig(n), xb_orig(n)
    real(4), intent(in) :: ab(n,n), xb(n)
    logical, intent(out) :: passed
    ...
    real(4), dimension(n) :: x_dir, x, x_plus, x_minus, x_cdiff
    ...
    x_dir = x_dir * 2.0e0 - 1.0e0

    a = a_orig + h * a_dir
    x = x_orig + h * x_dir
    call strsv(uplo, trans, diag, nsize, a, lda_val, x, incx)
    x_plus = x
...
end program test_strsv_reverse
