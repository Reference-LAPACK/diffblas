! Test program for ZHEMM vector forward (BLAS3 outlined)
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    do k = 1, nbdirs
      ...
      call zhemm(side, uplo, msize, nsize, alpha + h*alpha_dv(k), a_t, lda_val, b_t, ldb_val, beta + h*beta_dv(k), c_plus, ldc_val)
      a_t = a - h * a_dv(k,:,:)
      b_t = b - h * b_dv(k,:,:)
      c_minus = c_orig - h * c_dv_seed(k,:,:)
      call zhemm(side, uplo, msize, nsize, alpha - h*alpha_dv(k), a_t, lda_val, b_t, ldb_val, beta - h*beta_dv(k), c_minus, ldc_val)
      max_err = 0.0d0
      do jj = 1, n
        do ii = 1, n
