! Test program for SSYR2K vector forward (BLAS3 outlined)
program test_ssyr2k_vector_forward
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing SSYR2K (Vector Forward, multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
    nbdirs = n_test
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    uplo = 'L'
    transa = 'N'
    write(*,*) 'Testing SSYR2K (Vector Forward, n =', n, ')'
    ...
    c_orig = c
    c_dv_seed = c_dv
    call ssyr2k_dv(uplo, transa, nsize, ksize, alpha, alpha_dv, a, a_dv, lda_val, b, b_dv, ldb_val, beta, beta_dv, c, c_dv, ldc_val, nbdirs)
    ...
    do k = 1, nbdirs
      ...
      call ssyr2k(uplo, transa, nsize, ksize, alpha + h*alpha_dv(k), a_t, lda_val, b_t, ldb_val, beta + h*beta_dv(k), c_plus, ldc_val)
      a_t = a - h * a_dv(k,:,:)
      b_t = b - h * b_dv(k,:,:)
      c_minus = c_orig - h * c_dv_seed(k,:,:)
      call ssyr2k(uplo, transa, nsize, ksize, alpha - h*alpha_dv(k), a_t, lda_val, b_t, ldb_val, beta - h*beta_dv(k), c_minus, ldc_val)
      max_err = 0.0d0
      do jj = 1, n
        do ii = 1, n
