! Test program for STRSM reverse (BLAS3 outlined)
program test_strsm_reverse
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing STRSM (multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 1
...
contains
  subroutine run_test_for_size(n, passed)
    ...
    real(4), dimension(n,n) :: a, ab, b, bb
    real(4), dimension(n,n) :: bb_seed, b_orig, b_plus, b_minus
    real(4) :: alpha_dir
    ...
    call strsm_b(side, uplo, transa, diag, msize, nsize, alpha, alphab, a, ab, lda_val, b, bb, ldb_val)
    ...
    call strsm(side, uplo, transa, diag, msize, nsize, alpha + h*alpha_dir, a_fd, lda_val, b_plus, ldb_val)
    a_fd = a - h * a_dir
    b_minus = b_orig - h * b_dir
    call strsm(side, uplo, transa, diag, msize, nsize, alpha - h*alpha_dir, a_fd, lda_val, b_minus, ldb_val)
    vjp_fd = 0.0d0
