...
program test_dspmv
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing DSPMV (multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
...
contains
  subroutine run_test_for_size(n, passed)
    ...
    real(8), dimension(n) :: x, x_d, y, y_d, y_d_seed, y_orig, y_plus, y_minus
    real(8), dimension(:), allocatable :: ap, ap_d, ap_t, ap_orig
    real(8) :: alpha_t, beta_t
    real(8), dimension(n) :: x_t
    real(8) :: h
    parameter (h = 1.0e-7)
    real(8) :: abs_error, abs_ref, err_bound, max_err
    integer :: ii
    uplo = 'U'
    nsize = n
    incx_val = 1
    ...
    ap_t = ap_orig + h * ap_d
    call dspmv(uplo, nsize, alpha_t, ap_t, x_t, incx_val, beta_t, y_plus, incy_val)
    alpha_t = alpha - h * alpha_d
    beta_t = beta - h * beta_d
    x_t = x - h * x_d
    y_minus = y_orig - h * y_d_seed
    ap_t = ap_orig - h * ap_d
    call dspmv(uplo, nsize, alpha_t, ap_t, x_t, incx_val, beta_t, y_minus, incy_val)
    max_err = 0.0d0
    do ii = 1, n
