! Test program for DSBMV vector reverse mode differentiation
...
program test_dsbmv_vector_reverse
  ...
  integer :: ksize
  real(8) :: alpha
  real(8), dimension(max_size,max_size) :: a  ! Band storage
  integer :: lda_val
  ...
  real(8), dimension(nbdirs,max_size,max_size) :: ab  ! Band storage
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing DSBMV (Vector Reverse, multi-size: n = 4)'
  all_passed = .true.
  do itest = 1, 1
    n = test_sizes(itest)
    write(*,*) 'Testing DSBMV (Vector Reverse, n =', n, ')'
...
contains
  ...
  subroutine check_vjp_numerically(passed)
    ...
    real(8), dimension(max_size) :: y_plus, y_minus, y_central_diff
