! Test program for DSBMV vector reverse - BLAS2 band
! Generated automatically by run_tapenade_blas.py
! Multi-size outlined run_test_for_size(n, passed, nbdirs)

program test_dsbmv_vector_reverse
  implicit none
  external :: dsbmv
  external :: dsbmv_bv
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing DSBMV (Vector Reverse band, multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
    nbdirs = test_sizes(i)
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    real(8) :: alpha, beta
    real(8), dimension(:), allocatable :: alphab, betab
    real(8), dimension(:,:), allocatable :: a
    real(8), dimension(:,:,:), allocatable :: ab
    real(8), dimension(:), allocatable :: x, y
    real(8), dimension(:,:), allocatable :: xb, yb, xb_seed, yb_seed
    integer :: band_row, j
    real(4) :: temp_real
    ksize = max(0, n - 1)
    nsize = n
    ...
    yb = yb * 2.0d0 - 1.0d0
    yb_seed = yb
    write(*,*) 'Testing DSBMV (Vector Reverse band, n =', n, ')'
    call set_ISIZE1OFX(n)
    call set_ISIZE2OFA(n)
    call dsbmv_bv(uplo, nsize, ksize, alpha, alphab, a, ab, lda_val, x, xb, incx_val, beta, betab, y, yb, incy_val, nbdirs)
  ...
  subroutine check_vjp_numerically_band_vec(n, nbdirs, lda_val, ksize, uplo, nsize, incx_val, incy_val, alpha, alphab, beta, betab, a, ab, x, xb, y, yb_seed, yb, passed)
    ...
    real(8), intent(in) :: a(lda_val, n), ab(nbdirs, lda_val, n), x(n), xb(nbdirs, n), y(n), yb_seed(nbdirs, n), yb(nbdirs, n)
    logical, intent(out) :: passed
...
end program test_dsbmv_vector_reverse