! Test program for DTRMM vector forward (BLAS3 outlined)
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    do k = 1, nbdirs
      ...
      call dtrmm(side, uplo, transa, diag, msize, nsize, alpha + h*alpha_dv(k), a_t, lda_val, b_plus, ldb_val)
      a_t = a - h * a_dv(k,:,:)
      b_minus = b_orig - h * b_dv_seed(k,:,:)
      call dtrmm(side, uplo, transa, diag, msize, nsize, alpha - h*alpha_dv(k), a_t, lda_val, b_minus, ldb_val)
      max_err = 0.0d0
      do jj = 1, n
        do ii = 1, n
