! Test program for STRMM vector forward (BLAS3 outlined)
program test_strmm_vector_forward
  implicit none
  external :: strmm
  external :: strmm_dv
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing STRMM (Vector Forward, multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
    nbdirs = n_test
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    transa = 'N'
    diag = 'N'
    write(*,*) 'Testing STRMM (Vector Forward, n =', n, ')'
    ...
    b_orig = b
    b_dv_seed = b_dv
    call strmm_dv(side, uplo, transa, diag, msize, nsize, alpha, alpha_dv, a, a_dv, lda_val, b, b_dv, ldb_val, nbdirs)
    ...
    do k = 1, nbdirs
      ...
      call strmm(side, uplo, transa, diag, msize, nsize, alpha + h*alpha_dv(k), a_t, lda_val, b_plus, ldb_val)
      a_t = a - h * a_dv(k,:,:)
      b_minus = b_orig - h * b_dv_seed(k,:,:)
      call strmm(side, uplo, transa, diag, msize, nsize, alpha - h*alpha_dv(k), a_t, lda_val, b_minus, ldb_val)
      max_err = 0.0d0
      do jj = 1, n
        do ii = 1, n
...
end program test_strmm_vector_forward