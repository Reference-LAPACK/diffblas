! Test program for STRSV forward (tangent) mode differentiation
...
program test_strsv
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing STRSV (forward mode)'
  all_passed = .true.
  do id = 1, 2
    if (id == 1) then
      diag = 'N'
...
contains
  ...
  subroutine run_test_for_size(n, diag, passed)
    ...
    call strsv_d(uplo, trans, diag, nsize, a, a_d, lda_val, x, x_d, incx)
  ...
  subroutine check_derivatives_numerically(n, uplo, trans, diag, nsize, &
    ...
    real(4), intent(in) :: x_orig(n), x_d_orig(n), x_d(n)
    logical, intent(out) :: passed
    ...
    real(4), dimension(n) :: x, x_forward, x_backward
    integer :: i
    ...
    do i = 1, n
      ...
      relative_error = abs_error / max(abs_reference, 1.0e-8)
