! Test program for STRSM forward (tangent) mode differentiation
...
program test_strsm
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing STRSM (forward mode)'
  all_passed = .true.
  do id = 1, 2
    if (id == 1) then
      diag = 'N'
...
contains
  ...
  subroutine run_test_for_size(n, diag, passed)
    ...
    real(4) :: alpha
    real(4), dimension(n,n) :: a, b
    ! Derivative variables
    real(4) :: alpha_d
    real(4), dimension(n,n) :: a_d, b_d
    ! Restoration / stored directions
    real(4) :: alpha_orig, alpha_d_orig
    real(4), dimension(n,n) :: a_orig, a_d_orig, b_orig, b_d_orig
    integer :: i, j
    ...
    call strsm_d(side, uplo, transa, diag, msize, nsize, alpha, alpha_d, &
  ...
  subroutine check_derivatives_numerically(n, side, uplo, transa, diag, &
    ...
    real(4), dimension(n,n) :: b_forward, b_backward, a, b
    real(4) :: alpha
    integer :: i, j
