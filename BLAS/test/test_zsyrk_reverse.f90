...
program test_zsyrk_reverse
  ...
  complex(8) :: betab
  complex(8), dimension(max_size,max_size) :: cb
  ...
  complex(8), dimension(max_size,max_size) :: c_plus, c_minus

  ! Saved cotangents (output adjoints) for VJP verification
  complex(8), dimension(max_size,max_size) :: cb_orig
  ...
  call zsyrk_b(uplo, trans, nsize, ksize, alpha, alphab, a, ab, lda_val, beta, betab, c, cb, ldc_val)
...
contains
  ...
  subroutine check_vjp_numerically()
    ...
    complex(8), dimension(max_size,max_size) :: c_central_diff
    ...
    call zsyrk(uplo, trans, nsize, ksize, alpha, a, lda_val, beta, c, ldc_val)
    c_plus = c
...
end program test_zsyrk_reverse