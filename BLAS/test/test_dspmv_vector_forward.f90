! Test program for DSPMV vector forward mode differentiation
! Generated automatically by run_tapenade_blas.py
! Multi-size outlined - SPMV vector forward

program test_dspmv_vector_forward
  implicit none
  external :: dspmv
  external :: dspmv_dv
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing DSPMV (Vector Forward, multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
    nbdirs = n_test
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    real(8) :: alpha, beta
    real(8), dimension(n) :: x, y, y_orig, y_plus, y_minus
    real(8), dimension(nbdirs) :: alpha_dv, beta_dv
    real(8), dimension(nbdirs,n) :: x_dv, y_dv, y_dv_seed
    real(8), dimension(:), allocatable :: ap
    real(8), dimension(:,:), allocatable :: ap_dv
    real(8), dimension(:), allocatable :: ap_orig, ap_t
    real(8), parameter :: h = 1.0e-7
    real(8) :: max_err, abs_ref
    integer :: ii
    ...
    do k = 1, nbdirs
      ...
      call dspmv(uplo, nsize, alpha + h*alpha_dv(k), ap_t, x + h*x_dv(k,:), incx_val, beta + h*beta_dv(k), y_plus, incy_val)
      ap_t = ap_orig - h * ap_dv(k,:)
      call dspmv(uplo, nsize, alpha - h*alpha_dv(k), ap_t, x - h*x_dv(k,:), incx_val, beta - h*beta_dv(k), y_minus, incy_val)
      do ii = 1, n
...
end program test_dspmv_vector_forward