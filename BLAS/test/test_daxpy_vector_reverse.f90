...
program test_daxpy_vector_reverse
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing DAXPY (Vector Reverse, multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
    nbdirs = test_sizes(i)
...
contains
  ...
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    write(*,*) 'Testing DAXPY (Vector Reverse, n =', n, ')'

    ! Set ISIZE globals required by AXPY bv routine (dimension 1 of vectors).
    call set_ISIZE1OFDx(n)

    call daxpy_bv(nsize, alpha, alphab, x, xb, incx_val, y, yb, incy_val, nbdirs)
  ...
  subroutine check_vjp_numerically(n, nbdirs, nsize, incx_val, incy_val, alpha_orig, x_orig, y_orig, yb_orig, alphab, xb, yb, passed)
    ...
    do k = 1, nbdirs
      ...
      x = x_orig + h * x_dir
      y = y_orig + h * y_dir
      call daxpy(nsize, alpha, x, incx_val, y, incy_val)
      y_plus = y
      alpha = alpha_orig - h * alpha_dir
      x = x_orig - h * x_dir
      y = y_orig - h * y_dir
...
end program test_daxpy_vector_reverse