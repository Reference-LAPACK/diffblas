! Test program for CGBMV vector reverse - BLAS2 band
...
program test_cgbmv_vector_reverse
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing CGBMV (Vector Reverse band, multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
    nbdirs = test_sizes(i)
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    complex(4), dimension(:), allocatable :: alphab, betab
    ...
    yb_seed = yb
    write(*,*) 'Testing CGBMV (Vector Reverse band, n =', n, ')'
  ...
  subroutine check_vjp_numerically_band_gbmv_vec(n, nbdirs, lda_val, msize, nsize, kl, ku, trans, incx_val, incy_val, alpha, alphab, beta, betab, a, ab, x, xb, y, yb_seed, yb, passed)
    ...
    complex(4), intent(in) :: a(lda_val, n), ab(nbdirs, lda_val, n), x(n), xb(nbdirs, n), y(n), yb_seed(nbdirs, n), yb(nbdirs, n)
    logical, intent(out) :: passed
