...
program test_cdotc_vector_reverse
  ...
  complex(4), dimension(nbdirs) :: cdotcb

  ! Storage for original cotangents (for INOUT parameters in VJP verification)
  complex(4), dimension(nbdirs) :: cdotcb_orig
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing CDOTC (Vector Reverse, multi-size: n = 4)'
  all_passed = .true.
  do itest = 1, 1
    n = test_sizes(itest)
  ...
  call cdotc_bv(nsize, cx, cxb, incx_val, cy, cyb, incy_val, cdotcb, nbdirs)
