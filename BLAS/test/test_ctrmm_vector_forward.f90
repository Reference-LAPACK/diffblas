! Test program for CTRMM vector forward (BLAS3 outlined)
program test_ctrmm_vector_forward
  implicit none
  external :: ctrmm
  external :: ctrmm_dv
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing CTRMM (Vector Forward, multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
    nbdirs = n_test
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    do k = 1, nbdirs
      ...
      call ctrmm(side, uplo, transa, diag, msize, nsize, alpha + h*alpha_dv(k), a_t, lda_val, b_plus, ldb_val)
      a_t = a - h * a_dv(k,:,:)
      b_minus = b_orig - h * b_dv_seed(k,:,:)
      call ctrmm(side, uplo, transa, diag, msize, nsize, alpha - h*alpha_dv(k), a_t, lda_val, b_minus, ldb_val)
      max_err = 0.0d0
      do jj = 1, n
        do ii = 1, n
...
end program test_ctrmm_vector_forward