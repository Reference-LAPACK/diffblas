! Test program for ZCOPY vector reverse mode differentiation
! Generated automatically by run_tapenade_blas.py
! Using REAL*8 precision with nbdirs=n
! Multi-size test with outlined run_test_for_size(n) - arrays declared to size n

program test_zcopy_vector_reverse
  implicit none
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing ZCOPY (Vector Reverse, multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
    nbdirs = test_sizes(i)
...
contains
  ...
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    write(*,*) 'Testing ZCOPY (Vector Reverse, n =', n, ')'

    ! Set ISIZE globals required by COPY bv routine
    call set_ISIZE1OFZx(n)

    call zcopy_bv(nsize, x, xb, incx_val, y, yb, incy_val, nbdirs)
...
end program test_zcopy_vector_reverse