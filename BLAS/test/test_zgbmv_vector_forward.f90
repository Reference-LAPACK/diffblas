! Test program for ZGBMV vector forward - BLAS2 band
! Generated automatically by run_tapenade_blas.py
! Multi-size outlined run_test_for_size(n, passed, nbdirs) - band

program test_zgbmv_vector_forward
  implicit none
  external :: zgbmv
  external :: zgbmv_dv
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing ZGBMV (Vector Forward band, multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
    nbdirs = test_sizes(i)
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    complex(8), dimension(nbdirs) :: alpha_dv, beta_dv, alpha_dv_seed, beta_dv_seed
    ...
    write(*,*) 'Testing ZGBMV (Vector Forward band, n =', n, ')'
    a_orig = a
    x_orig = x
    a_dv_seed = a_dv
    x_dv_seed = x_dv
    ...
    call zgbmv_dv(trans, msize, nsize, kl, ku, alpha, alpha_dv, a, a_dv, lda_val, x, x_dv, incx_val, beta, beta_dv, y, y_dv, incy_val, nbdirs)
  ...
  subroutine check_derivatives_numerically_band_gbmv(n, nbdirs, lda_val, msize, nsize, kl, ku, trans, incx_val, incy_val, alpha, alpha_dv_seed, beta, beta_dv_seed, a_orig, a_dv_seed_mat, x_orig, x_dv_seed, y_orig, y_dv_seed, y_dv_out, passed)
    ...
    complex(8), intent(in) :: alpha_dv_seed(nbdirs), beta_dv_seed(nbdirs)
    complex(8), intent(in) :: a_orig(lda_val, n), a_dv_seed_mat(nbdirs, lda_val, n), x_orig(n), x_dv_seed(nbdirs, n), y_orig(n), y_dv_seed(nbdirs, n), y_dv_out(nbdirs, n)
    logical, intent(out) :: passed
...
end program test_zgbmv_vector_forward