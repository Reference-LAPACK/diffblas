! Test program for DCOPY reverse mode (adjoint) differentiation
! Generated automatically by run_tapenade_blas.py
! Using REAL*8 precision
! Multi-size test with outlined run_test_for_size(n) - arrays declared to size n

program test_dcopy_reverse
  implicit none
...
contains
  ...
  subroutine run_test_for_size(n, passed)
    ...
    call dcopy_b(nsize, dx, dxb, incx_val, dy, dyb, incy_val)

    call set_ISIZE1OFDx(-1)

    call check_vjp_numerically(n, nsize, incx_val, incy_val, dx_orig, dy_orig, dyb_orig, dxb, dyb, passed)

  end subroutine run_test_for_size

  subroutine check_vjp_numerically(n, nsize, incx_val, incy_val, dx_orig, dy_orig, dyb_orig, dxb, dyb, passed)
    implicit none
    integer, intent(in) :: n
    ...
    dx = dx_orig + h * dx_dir
    dy = dy_orig + h * dy_dir
    call dcopy(nsize, dx, incx_val, dy, incy_val)
    dy_plus = dy
...
end program test_dcopy_reverse