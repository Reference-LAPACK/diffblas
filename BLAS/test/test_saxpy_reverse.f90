! Test program for SAXPY reverse mode (adjoint) differentiation
! Generated automatically by run_tapenade_blas.py
! Using REAL*4 precision
! Multi-size test with outlined run_test_for_size(n) - arrays declared to size n

program test_saxpy_reverse
  implicit none
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing SAXPY (multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
...
contains
  ...
  subroutine run_test_for_size(n, passed)
    ...
    call saxpy_b(nsize, sa, sab, sx, sxb, incx_val, sy, syb, incy_val)

    call set_ISIZE1OFSx(-1)

    call check_vjp_numerically(n, nsize, incx_val, incy_val, sa_orig, sx_orig, sy_orig, syb_orig, sab, sxb, syb, passed)

  end subroutine run_test_for_size

  subroutine check_vjp_numerically(n, nsize, incx_val, incy_val, sa_orig, sx_orig, sy_orig, syb_orig, sab, sxb, syb, passed)
    implicit none
    integer, intent(in) :: n
...
end program test_saxpy_reverse