...
program test_zgemm_vector_reverse
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing ZGEMM (Vector Reverse, multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
    nbdirs = test_sizes(i)
...
contains
  ...
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    complex(8), dimension(nbdirs,n,n) :: cb_orig
    integer :: k, ii, jj
  ...
  subroutine check_vjp_numerically(n, nbdirs, transa, transb, msize, nsize, ksize, lda_val, ldb_val, ldc_val, alpha_orig, a_orig, b_orig, beta_orig, c_orig, cb_orig, alphab, ab, bb, betab, cb, passed)
    ...
    complex(8), intent(in) :: a_orig(n,n), b_orig(n,n), c_orig(n,n)
    complex(8), intent(in) :: cb_orig(nbdirs,n,n)
    complex(8), intent(in) :: alphab(nbdirs), betab(nbdirs)
    complex(8), intent(in) :: ab(nbdirs,n,n), bb(nbdirs,n,n), cb(nbdirs,n,n)
    logical, intent(out) :: passed
    ...
    complex(8) :: alpha, beta
    complex(8), dimension(n,n) :: a, b, c
    complex(8), dimension(n*n) :: temp_products
  ...
  subroutine sort_array(arr, n)
    implicit none
    integer, intent(in) :: n
    complex(8), dimension(n), intent(inout) :: arr
    integer :: i, j, min_idx
    complex(8) :: temp
    do i = 1, n-1
      min_idx = i
