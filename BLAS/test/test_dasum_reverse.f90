! Test program for DASUM reverse mode (adjoint) differentiation
! Generated automatically by run_tapenade_blas.py
! Using REAL*8 precision
! Multi-size test with outlined run_test_for_size(n) - arrays declared to size n

program test_dasum_reverse
  implicit none
...
contains
  ...
  subroutine run_test_for_size(n, passed)
    ...
    call random_number(dasumb)
    dasumb = dasumb * 2.0 - 1.0
    dasumb_orig = dasumb
    ...
    call dasum_b(nsize, dx, dxb, incx_val, dasumb)

    call set_ISIZE1OFDx(-1)

    call check_vjp_numerically(n, nsize, incx_val, dx_orig, dxb, dasumb_orig, passed)

  end subroutine run_test_for_size

  subroutine check_vjp_numerically(n, nsize, incx_val, dx_orig, dxb, dasumb_orig, passed)
    implicit none
    integer, intent(in) :: n
    ...
    dx = dx_orig + h * dx_dir
    dasum_plus = dasum(nsize, dx, incx_val)

    dx = dx_orig - h * dx_dir
    dasum_minus = dasum(nsize, dx, incx_val)


    vjp_fd = dasumb_orig * (dasum_plus - dasum_minus) / (2.0 * h)
...
end program test_dasum_reverse