! Test program for ZTRMV reverse mode (adjoint) differentiation
! Generated automatically by run_tapenade_blas.py
! Using REAL*8 precision
! Multi-size test with outlined run_test_for_size(n) - arrays declared to size n

program test_ztrmv_reverse
  implicit none
...
contains
  ...
  subroutine run_test_for_size(n, passed)
    ...
    write(*,*) 'Testing ZTRMV (n =', n, ')'

    call set_ISIZE2OFA(n)

    call ztrmv_b(uplo, trans, diag, nsize, a, ab, lda_val, x, xb, incx_val)
  ...
  subroutine check_vjp_numerically(n, uplo, trans, diag, nsize, lda_val, incx_val, a_orig, x_orig, xb_orig, ab, xb, passed)
    ...
    complex(8), dimension(n) :: x_plus, x_minus, x_central_diff
...
end program test_ztrmv_reverse