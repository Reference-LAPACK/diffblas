! Test program for DSYMM reverse (BLAS3 outlined)
program test_dsymm_reverse
  implicit none
  external :: dsymm
  external :: dsymm_b
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing DSYMM (multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 1
...
contains
  subroutine run_test_for_size(n, passed)
    ...
    real(8), dimension(n,n) :: c_orig
    real(8) :: alpha_dir, beta_dir
    real(8), dimension(n,n) :: a_dir, b_dir, c_dir, a_fd, b_fd
    ...
    call dsymm_b(side, uplo, msize, nsize, alpha, alphab, a, ab, lda_val, b, bb, ldb_val, beta, betab, c, cb, ldc_val)
    ...
    call dsymm(side, uplo, msize, nsize, alpha + h*alpha_dir, a_fd, lda_val, b_fd, ldb_val, beta + h*beta_dir, c_plus, ldc_val)
    a_fd = a - h * a_dir
    b_fd = b - h * b_dir
    c_minus = c_orig - h * c_dir
    call dsymm(side, uplo, msize, nsize, alpha - h*alpha_dir, a_fd, lda_val, b_fd, ldb_val, beta - h*beta_dir, c_minus, ldc_val)
    vjp_fd = 0.0d0
    do jj = 1, n
      do ii = 1, n
...
end program test_dsymm_reverse