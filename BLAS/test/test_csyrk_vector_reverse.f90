! Test program for CSYRK vector reverse (BLAS3 outlined)
program test_csyrk_vector_reverse
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing CSYRK (Vector Reverse, multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
    nbdirs = n_test
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    call csyrk_bv(uplo, transa, nsize, ksize, alpha, alphab, a, ab, lda_val, beta, betab, c, cb, ldc_val, nbdirs)
    call set_ISIZE2OFA(-1)
    write(*,*) 'Testing CSYRK (Vector Reverse, n =', n, ')'
    ...
    do k = 1, nbdirs
      a_t = a + h * ab(k,:,:)
      c_plus = c
      call csyrk(uplo, transa, nsize, ksize, alpha + h*alphab(k), a_t, lda_val, beta + h*betab(k), c_plus, ldc_val)
      a_t = a - h * ab(k,:,:)
      c_minus = c
      call csyrk(uplo, transa, nsize, ksize, alpha - h*alphab(k), a_t, lda_val, beta - h*betab(k), c_minus, ldc_val)
      vjp_fd = 0.0d0
      do jj = 1, n
        do ii = 1, n
...
end program test_csyrk_vector_reverse