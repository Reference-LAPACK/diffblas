! Test program for CCOPY vector reverse mode differentiation
! Generated automatically by run_tapenade_blas.py
! Using REAL*4 precision with nbdirs=n
! Multi-size test with outlined run_test_for_size(n) - arrays declared to size n

program test_ccopy_vector_reverse
  implicit none
...
contains
  ...
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    write(*,*) 'Testing CCOPY (Vector Reverse, n =', n, ')'

    ! Set ISIZE globals required by COPY bv routine
    call set_ISIZE1OFCx(n)

    call ccopy_bv(nsize, x, xb, incx_val, y, yb, incy_val, nbdirs)
  ...
  subroutine check_vjp_numerically(n, nbdirs, nsize, incx_val, incy_val, x_orig, y_orig, yb_orig, xb, yb, passed)
    ...
    do k = 1, nbdirs
      ...
      x = x_orig + h * x_dir
      y = y_orig + h * y_dir
      call ccopy(nsize, x, incx_val, y, incy_val)
      y_plus = y
      x = x_orig - h * x_dir
      y = y_orig - h * y_dir
      call ccopy(nsize, x, incx_val, y, incy_val)
...
end program test_ccopy_vector_reverse