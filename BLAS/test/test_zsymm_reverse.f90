! Test program for ZSYMM reverse (BLAS3 outlined)
program test_zsymm_reverse
  implicit none
  external :: zsymm
  external :: zsymm_b
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing ZSYMM (multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 3
...
contains
  subroutine run_test_for_size(n, passed)
    ...
    complex(8), dimension(n,n) :: a_dir, b_dir, c_dir, a_fd, b_fd
    real(8), parameter :: h = 1.0e-7
    real(8) :: vjp_fd, vjp_ad, abs_error, ref_c, relative_error, abs_reference
    real(8) :: vjp_ad_alpha, vjp_ad_beta, vjp_ad_a, vjp_ad_b, vjp_ad_c
    integer :: ii, jj
    real(4) :: tr, ti
    msize = n
    nsize = n
    ...
    call zsymm_b(side, uplo, msize, nsize, alpha, alphab, a, ab, lda_val, b, bb, ldb_val, beta, betab, c, cb, ldc_val)
    ...
    call zsymm(side, uplo, msize, nsize, alpha + h*alpha_dir, a_fd, lda_val, b_fd, ldb_val, beta + h*beta_dir, c_plus, ldc_val)
    a_fd = a - h * a_dir
    b_fd = b - h * b_dir
    c_minus = c_orig - h * c_dir
    call zsymm(side, uplo, msize, nsize, alpha - h*alpha_dir, a_fd, lda_val, b_fd, ldb_val, beta - h*beta_dir, c_minus, ldc_val)
    vjp_fd = 0.0d0
    do jj = 1, n
      do ii = 1, n
...
end program test_zsymm_reverse