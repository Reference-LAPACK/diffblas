...
program test_zhbmv_vector_reverse
  ...
  integer :: ksize
  complex(8) :: alpha
  complex(8), dimension(max_size,max_size) :: a  ! Band storage
  integer :: lda_val
  ...
  complex(8), dimension(nbdirs,max_size,max_size) :: ab  ! Band storage
  complex(8), dimension(nbdirs,max_size) :: xb
  complex(8), dimension(nbdirs) :: betab
  complex(8), dimension(nbdirs,max_size) :: yb

  ! Storage for original cotangents (for INOUT parameters in VJP verification)
  complex(8), dimension(nbdirs,max_size) :: yb_orig
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing ZHBMV (Vector Reverse, multi-size: n = 4)'
  all_passed = .true.
  do itest = 1, 1
    n = test_sizes(itest)
    write(*,*) 'Testing ZHBMV (Vector Reverse, n =', n, ')'
