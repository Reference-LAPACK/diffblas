! Test program for ZTRMM vector forward (BLAS3 outlined)
program test_ztrmm_vector_forward
  implicit none
  external :: ztrmm
  external :: ztrmm_dv
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing ZTRMM (Vector Forward, multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
    nbdirs = n_test
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    transa = 'N'
    diag = 'N'
    write(*,*) 'Testing ZTRMM (Vector Forward, n =', n, ')'
    ...
    b_orig = b
    b_dv_seed = b_dv
    call ztrmm_dv(side, uplo, transa, diag, msize, nsize, alpha, alpha_dv, a, a_dv, lda_val, b, b_dv, ldb_val, nbdirs)
    ...
    do k = 1, nbdirs
      ...
      call ztrmm(side, uplo, transa, diag, msize, nsize, alpha + h*alpha_dv(k), a_t, lda_val, b_plus, ldb_val)
      a_t = a - h * a_dv(k,:,:)
      b_minus = b_orig - h * b_dv_seed(k,:,:)
      call ztrmm(side, uplo, transa, diag, msize, nsize, alpha - h*alpha_dv(k), a_t, lda_val, b_minus, ldb_val)
      max_err = 0.0d0
      do jj = 1, n
        do ii = 1, n
...
end program test_ztrmm_vector_forward