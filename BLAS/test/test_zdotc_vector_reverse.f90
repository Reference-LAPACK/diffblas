! Test program for ZDOTC vector reverse mode differentiation
! Generated automatically by run_tapenade_blas.py
! Using REAL*8 precision with nbdirs=n
! Multi-size test with outlined run_test_for_size(n) - arrays declared to size n

program test_zdotc_vector_reverse
  implicit none
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing ZDOTC (Vector Reverse, multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
    nbdirs = test_sizes(i)
...
contains
  ...
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    complex(8), dimension(nbdirs) :: result_b, result_b_seed
    ...
    call zdotc_bv(nsize, x, xb, incx_val, y, yb, incy_val, result_b, nbdirs)
  ...
  subroutine check_vjp_numerically(n, nbdirs, nsize, incx_val, incy_val, x_orig, y_orig, result_b_seed, xb, yb, passed)
    ...
    complex(8), intent(in) :: result_b_seed(nbdirs)
    ...
    complex(8) :: result_forward, result_backward, result_central_diff
    complex(8), dimension(n) :: x, y
    integer :: i, k
    real(4) :: temp_real, temp_imag
    ...
    do k = 1, nbdirs
      ...
      x = x_orig + h * x_dir
      y = y_orig + h * y_dir
      result_forward = zdotc(nsize, x, incx_val, y, incy_val)
      x = x_orig - h * x_dir
      y = y_orig - h * y_dir
      result_backward = zdotc(nsize, x, incx_val, y, incy_val)
...
end program test_zdotc_vector_reverse