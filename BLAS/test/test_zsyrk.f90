! Test program for ZSYRK differentiation (BLAS3 outlined)
...
program test_zsyrk
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing ZSYRK (multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
...
contains
  subroutine run_test_for_size(n, passed)
    ...
    complex(8), dimension(n,n) :: a, a_d, c, c_d
    ...
    c_d = 0.0d0
    c_orig = c
    call zsyrk_d(uplo, transa, nsize, ksize, alpha, alpha_d, a, a_d, lda_val, beta, beta_d, c, c_d, ldc_val)
    ! Finite-difference check: (output(alpha+h) - output(alpha-h))/(2h) vs derivative
    c_plus = c_orig
    call zsyrk(uplo, transa, nsize, ksize, alpha + h, a, lda_val, beta, c_plus, ldc_val)
    c_minus = c_orig
    call zsyrk(uplo, transa, nsize, ksize, alpha - h, a, lda_val, beta, c_minus, ldc_val)
    max_err = 0.0d0
    do jj = 1, n
      do ii = 1, n
