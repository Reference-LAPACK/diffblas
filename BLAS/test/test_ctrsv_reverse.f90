! Test program for CTRSV reverse (adjoint) mode differentiation
...
program test_ctrsv_reverse
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing CTRSV (reverse mode)'
  all_passed = .true.
  do id = 1, 2
    if (id == 1) then
      diag = 'N'
...
contains
  ...
  subroutine run_test_for_size(n, diag, passed)
    ...
    complex(4), dimension(n,n) :: a, ab
    complex(4), dimension(n) :: x, xb
    complex(4), dimension(n,n) :: a_orig
    complex(4), dimension(n) :: x_orig, xb_orig
    integer :: i
    ...
    write(*,*) 'Testing CTRSV (n =', n, ', diag = ', diag, ')'

    call ctrsv_b(uplo, trans, diag, nsize, a, ab, lda_val, x, xb, incx)
  ...
  subroutine check_vjp_numerically(n, uplo, trans, diag, nsize, lda_val, &
    ...
    complex(4), intent(in) :: a_orig(n,n), x_orig(n), xb_orig(n)
    complex(4), intent(in) :: ab(n,n), xb(n)
    logical, intent(out) :: passed
    ...
    integer :: i, j
    complex(4), dimension(n,n) :: a_dir, a
    complex(4), dimension(n) :: x_dir, x, x_plus, x_minus, x_cdiff
    ...
    a = a_orig + h * a_dir
    x = x_orig + h * x_dir
    call ctrsv(uplo, trans, diag, nsize, a, lda_val, x, incx)
    x_plus = x
...
end program test_ctrsv_reverse
