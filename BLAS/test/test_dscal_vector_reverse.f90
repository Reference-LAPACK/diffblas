! Test program for DSCAL vector reverse mode differentiation
! Generated automatically by run_tapenade_blas.py
! Using REAL*8 precision with nbdirs=n
! Multi-size test with outlined run_test_for_size(n) - arrays declared to size n

program test_dscal_vector_reverse
  implicit none
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing DSCAL (Vector Reverse, multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
    nbdirs = test_sizes(i)
...
contains
  ...
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    real(8), dimension(nbdirs,n) :: xb_orig
    integer :: k, i
    ...
    write(*,*) 'Testing DSCAL (Vector Reverse, n =', n, ')'

    call dscal_bv(nsize, alpha, alphab, x, xb, incx_val, nbdirs)
  ...
  subroutine check_vjp_numerically(n, nbdirs, nsize, incx_val, alpha_orig, x_orig, xb_orig, alphab, xb, passed)
    ...
    real(8), intent(in) :: xb_orig(nbdirs,n)
    real(8), intent(in) :: alphab(nbdirs)
    real(8), intent(in) :: xb(nbdirs,n)
    logical, intent(out) :: passed
    ...
    real(8) :: alpha
    real(8), dimension(n) :: x, x_plus, x_minus, x_central_diff
    ...
    do k = 1, nbdirs
      ...
      x = x_orig + h * x_dir
      call dscal(nsize, alpha, x, incx_val)
      x_plus = x
      alpha = alpha_orig - h * alpha_dir
      x = x_orig - h * x_dir
      call dscal(nsize, alpha, x, incx_val)
...
end program test_dscal_vector_reverse