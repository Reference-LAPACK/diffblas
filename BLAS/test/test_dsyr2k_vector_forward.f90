! Test program for DSYR2K vector forward mode differentiation
! Generated automatically by run_tapenade_blas.py
! Using REAL*8 precision with nbdirs=4

program test_dsyr2k_vector_forward
  implicit none
  ...
  real(8), dimension(nbdirs) :: beta_dv
  real(8), dimension(nbdirs,max_size,max_size) :: c_dv
  ...
  real(8) :: beta_orig
  real(8), dimension(nbdirs) :: beta_dv_orig
  real(8), dimension(max_size,max_size) :: c_orig
  real(8), dimension(nbdirs,max_size,max_size) :: c_dv_orig

  test_sizes = (/ 4 /)
  write(*,*) 'Testing DSYR2K (Vector Forward, multi-size: n = 4)'
  all_passed = .true.
  do itest = 1, 1
    n = test_sizes(itest)
    write(*,*) 'Testing DSYR2K (Vector Forward, n =', n, ')'
  ...
  write(*,*) 'Testing DSYR2K (Vector Forward Mode)'
  ...
  call dsyr2k_dv(uplo, trans, nsize, ksize, alpha, alpha_dv, a, a_dv, lda_val, b, b_dv, ldb_val, beta, beta_dv, c, c_dv, ldc_val, nbdirs)
...
end program test_dsyr2k_vector_forward