! Test program for ZHEMM reverse (BLAS3 outlined)
program test_zhemm_reverse
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing ZHEMM (multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 3
...
contains
  subroutine run_test_for_size(n, passed)
    ...
    call zhemm(side, uplo, msize, nsize, alpha + h*alpha_dir, a_fd, lda_val, b_fd, ldb_val, beta + h*beta_dir, c_plus, ldc_val)
    a_fd = a - h * a_dir
    b_fd = b - h * b_dir
    c_minus = c_orig - h * c_dir
    call zhemm(side, uplo, msize, nsize, alpha - h*alpha_dir, a_fd, lda_val, b_fd, ldb_val, beta - h*beta_dir, c_minus, ldc_val)
    vjp_fd = 0.0d0
    do jj = 1, n
      do ii = 1, n
