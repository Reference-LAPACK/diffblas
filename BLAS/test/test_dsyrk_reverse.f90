! Test program for DSYRK reverse (BLAS3 outlined)
program test_dsyrk_reverse
  implicit none
  external :: dsyrk
  external :: dsyrk_b
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing DSYRK (multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 3
...
contains
  subroutine run_test_for_size(n, passed)
    ...
    real(8), dimension(n,n) :: a, ab, c, cb
    ...
    cb = cb * 2.0d0 - 1.0d0
    cb_seed = cb
    write(*,*) 'Testing DSYRK (n =', n, ')'
    alphab = 0.0d0
    betab = 0.0d0
    ab = 0.0d0
    call set_ISIZE2OFA(n)
    call dsyrk_b(uplo, transa, nsize, ksize, alpha, alphab, a, ab, lda_val, beta, betab, c, cb, ldc_val)
    ...
    c_plus = c
    call dsyrk(uplo, transa, nsize, ksize, alpha + h*alphab, a + h*ab, lda_val, beta + h*betab, c_plus, ldc_val)
    c_minus = c
    call dsyrk(uplo, transa, nsize, ksize, alpha - h*alphab, a - h*ab, lda_val, beta - h*betab, c_minus, ldc_val)
    vjp_fd = 0.0d0
    do jj = 1, n
      do ii = 1, n
...
end program test_dsyrk_reverse