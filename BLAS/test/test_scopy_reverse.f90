! Test program for SCOPY reverse mode (adjoint) differentiation
! Generated automatically by run_tapenade_blas.py
! Using REAL*4 precision
! Multi-size test with outlined run_test_for_size(n) - arrays declared to size n

program test_scopy_reverse
  implicit none
...
contains
  ...
  subroutine run_test_for_size(n, passed)
    ...
    call scopy_b(nsize, sx, sxb, incx_val, sy, syb, incy_val)

    call set_ISIZE1OFSx(-1)

    call check_vjp_numerically(n, nsize, incx_val, incy_val, sx_orig, sy_orig, syb_orig, sxb, syb, passed)

  end subroutine run_test_for_size

  subroutine check_vjp_numerically(n, nsize, incx_val, incy_val, sx_orig, sy_orig, syb_orig, sxb, syb, passed)
    implicit none
    integer, intent(in) :: n
...
end program test_scopy_reverse