! Test program for ZTRMM reverse (BLAS3 outlined)
program test_ztrmm_reverse
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing ZTRMM (multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 3
...
contains
  subroutine run_test_for_size(n, passed)
    ...
    call ztrmm(side, uplo, transa, diag, msize, nsize, alpha + h*alpha_dir, a_fd, lda_val, b_plus, ldb_val)
    a_fd = a - h * a_dir
    b_minus = b_orig - h * b_dir
    call ztrmm(side, uplo, transa, diag, msize, nsize, alpha - h*alpha_dir, a_fd, lda_val, b_minus, ldb_val)
    vjp_fd = 0.0d0
    do jj = 1, n
      do ii = 1, n
