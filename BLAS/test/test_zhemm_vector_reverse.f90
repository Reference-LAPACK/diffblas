! Test program for ZHEMM vector reverse (BLAS3 outlined)
program test_zhemm_vector_reverse
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing ZHEMM (Vector Reverse, multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
    nbdirs = n_test
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    write(*,*) 'Testing ZHEMM (Vector Reverse, n =', n, ')'
    ...
    do k = 1, nbdirs
      ...
      call zhemm(side, uplo, msize, nsize, alpha + h*alpha_dir, a_t, lda_val, b_t, ldb_val, beta + h*beta_dir, c_plus, ldc_val)
      a_t = a - h * a_dir
      b_t = b - h * b_dir
      c_minus = c_orig - h * c_dir
      call zhemm(side, uplo, msize, nsize, alpha - h*alpha_dir, a_t, lda_val, b_t, ldb_val, beta - h*beta_dir, c_minus, ldc_val)
      vjp_fd = 0.0d0
      do jj = 1, n
        do ii = 1, n
...
end program test_zhemm_vector_reverse