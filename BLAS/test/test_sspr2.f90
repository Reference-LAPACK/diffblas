! Test program for SSPR2 differentiation
...
contains
  subroutine run_test_for_size(n, passed)
    ...
    real(4) :: alpha, alpha_d
    real(4), dimension(n) :: x, x_d
    real(4), allocatable :: ap(:), ap_d(:), ap_d_seed(:), ap_orig(:)
    real(4), dimension(n) :: y, y_d
    integer :: ii
    uplo = 'U'
    nsize = n
    incx_val = 1
    ...
    call check_derivatives_numerically(n, npack, uplo, nsize, incx_val, incy_val, alpha, alpha_d, x, x_d, y, y_d, ap_orig, ap_d, ap_d_seed, passed)
    deallocate(ap, ap_d, ap_d_seed, ap_orig)
  end subroutine run_test_for_size

  subroutine check_derivatives_numerically(n, npack, uplo, nsize, incx_val, incy_val, alpha, alpha_d, x, x_d, y, y_d, ap_orig, ap_d, ap_d_seed, passed)
    implicit none
    integer, intent(in) :: n, npack
    character, intent(in) :: uplo
    integer, intent(in) :: nsize, incx_val
    integer, intent(in) :: incy_val
    real(4), intent(in) :: alpha, alpha_d
    real(4), intent(in) :: x(n), x_d(n)
    real(4), intent(in) :: y(n), y_d(n)
    real(4), intent(in) :: ap_orig(npack), ap_d(npack), ap_d_seed(npack)
    logical, intent(out) :: passed
