! Test program for DTRMV reverse mode (adjoint) differentiation
! Generated automatically by run_tapenade_blas.py
! Using REAL*8 precision
! Multi-size test with outlined run_test_for_size(n) - arrays declared to size n

program test_dtrmv_reverse
  implicit none
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing DTRMV (multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
...
contains
  ...
  subroutine run_test_for_size(n, passed)
    ...
    call dtrmv_b(uplo, trans, diag, nsize, a, ab, lda_val, x, xb, incx_val)
  ...
  subroutine check_vjp_numerically(n, uplo, trans, diag, nsize, lda_val, incx_val, a_orig, x_orig, xb_orig, ab, xb, passed)
    ...
    real(8), dimension(n) :: x_plus, x_minus, x_central_diff
...
end program test_dtrmv_reverse