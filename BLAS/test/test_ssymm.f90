! Test program for SSYMM differentiation (BLAS3 outlined)
...
program test_ssymm
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing SSYMM (multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
...
contains
  subroutine run_test_for_size(n, passed)
    ...
    c_d = 0.0d0
    c_orig = c
    call ssymm_d(side, uplo, msize, nsize, alpha, alpha_d, a, a_d, lda_val, b, b_d, ldb_val, beta, beta_d, c, c_d, ldc_val)
    write(*,*) 'Testing SSYMM (n =', n, ')'
    write(*,*) 'Function calls completed successfully'
    ! Finite-difference check: (output(alpha+h) - output(alpha-h))/(2h) vs derivative
    c_plus = c_orig
    call ssymm(side, uplo, msize, nsize, alpha + h, a, lda_val, b, ldb_val, beta, c_plus, ldc_val)
    c_minus = c_orig
    call ssymm(side, uplo, msize, nsize, alpha - h, a, lda_val, b, ldb_val, beta, c_minus, ldc_val)
    max_err = 0.0d0
    do jj = 1, n
      do ii = 1, n
