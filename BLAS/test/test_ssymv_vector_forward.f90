...
program test_ssymv_vector_forward
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing SSYMV (Vector Forward, multi-size: n = 4)'
  all_passed = .true.
  do itest = 1, 1
    n = test_sizes(itest)
    write(*,*) 'Testing SSYMV (Vector Forward, n =', n, ')'
  ...
  call ssymv_dv(uplo, nsize, alpha, alpha_dv, a, a_dv, lda_val, x, x_dv, incx_val, beta, beta_dv, y, y_dv, incy_val, nbdirs)
