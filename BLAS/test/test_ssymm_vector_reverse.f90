! Test program for SSYMM vector reverse (BLAS3 outlined)
program test_ssymm_vector_reverse
  implicit none
  external :: ssymm
  external :: ssymm_bv
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing SSYMM (Vector Reverse, multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
    nbdirs = n_test
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    real(4), dimension(n,n) :: a_dir, b_dir, c_dir
    ...
    call ssymm_bv(side, uplo, msize, nsize, alpha, alphab, a, ab, lda_val, b, bb, ldb_val, beta, betab, c, cb, ldc_val, nbdirs)
    call set_ISIZE2OFA(-1)
    call set_ISIZE2OFB(-1)
    write(*,*) 'Testing SSYMM (Vector Reverse, n =', n, ')'
    ...
    do k = 1, nbdirs
      ...
      call ssymm(side, uplo, msize, nsize, alpha + h*alpha_dir, a_t, lda_val, b_t, ldb_val, beta + h*beta_dir, c_plus, ldc_val)
      a_t = a - h * a_dir
      b_t = b - h * b_dir
      c_minus = c_orig - h * c_dir
      call ssymm(side, uplo, msize, nsize, alpha - h*alpha_dir, a_t, lda_val, b_t, ldb_val, beta - h*beta_dir, c_minus, ldc_val)
      vjp_fd = 0.0d0
...
end program test_ssymm_vector_reverse