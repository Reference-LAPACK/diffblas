...
program test_sgemm_vector_reverse
  ...
  real(4), dimension(nbdirs,max_size,max_size) :: cb

  ! Storage for original cotangents (for INOUT parameters in VJP verification)
  real(4), dimension(nbdirs,max_size,max_size) :: cb_orig
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing SGEMM (Vector Reverse, multi-size: n = 4)'
  all_passed = .true.
  do itest = 1, 1
    n = test_sizes(itest)
...
contains
  ...
  subroutine check_vjp_numerically(passed)
    ...
    real(4), dimension(max_size,max_size) :: c_plus, c_minus, c_central_diff
