! Test program for SGEMM vector reverse mode differentiation
! Generated automatically by run_tapenade_blas.py
! Using REAL*4 precision with nbdirs=n
! Multi-size test with outlined run_test_for_size(n) - arrays declared to size n

program test_sgemm_vector_reverse
  implicit none
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing SGEMM (Vector Reverse, multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
    nbdirs = test_sizes(i)
...
contains
  ...
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    real(4), dimension(nbdirs,n,n) :: cb_orig
    integer :: k, ii, jj
    ...
    write(*,*) 'Testing SGEMM (Vector Reverse, n =', n, ')'

    call set_ISIZE2OFA(n)
    call set_ISIZE2OFB(n)

    call sgemm_bv(transa, transb, msize, nsize, ksize, alpha, alphab, a, ab, lda_val, b, bb, ldb_val, beta, betab, c, cb, ldc_val, nbdirs)
  ...
  subroutine check_vjp_numerically(n, nbdirs, transa, transb, msize, nsize, ksize, lda_val, ldb_val, ldc_val, alpha_orig, a_orig, b_orig, beta_orig, c_orig, cb_orig, alphab, ab, bb, betab, cb, passed)
    ...
    real(4), intent(in) :: a_orig(n,n), b_orig(n,n), c_orig(n,n)
    real(4), intent(in) :: cb_orig(nbdirs,n,n)
    real(4), intent(in) :: alphab(nbdirs), betab(nbdirs)
    real(4), intent(in) :: ab(nbdirs,n,n), bb(nbdirs,n,n), cb(nbdirs,n,n)
    logical, intent(out) :: passed
...
end program test_sgemm_vector_reverse