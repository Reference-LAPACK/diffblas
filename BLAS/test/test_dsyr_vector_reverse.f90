...
program test_dsyr_vector_reverse
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing DSYR (Vector Reverse, multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
    nbdirs = test_sizes(i)
...
contains
  ...
  subroutine check_vjp_syr_syr2(n, nbdirs, uplo, nsize, lda_val, incx_val, incy_val, alpha, x, a, ab_orig, alphab, xb, ab, passed, y, yb)
    ...
    do k = 1, nbdirs
      ...
      x_t = x + h * x_dir
      if (present(y)) then
        call dsyr(uplo, nsize, alpha + h*alpha_dir, x_t, incx_val, y_t, incy_val, a_t, lda_val)
      else
        call dsyr(uplo, nsize, alpha + h*alpha_dir, x_t, incx_val, a_t, lda_val)
      end if
      a_plus = a_t
      a_t = a - h * a_dir
      x_t = x - h * x_dir
