...
program test_zdotu_vector_forward
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing ZDOTU (Vector Forward, multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
    nbdirs = test_sizes(i)
...
contains
  ...
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    complex(8), dimension(nbdirs) :: result_dv
    ...
    result_val = zdotu(nsize, x, incx_val, y, incy_val)

    write(*,*) 'Testing ZDOTU (Vector Forward, n =', n, ')'

    call zdotu_dv(nsize, x, x_dv, incx_val, y, y_dv, incy_val, result_val, result_dv, nbdirs)
  ...
  subroutine check_derivatives_numerically(n, nbdirs, nsize, incx_val, incy_val, x_orig, x_dv_orig, y_orig, y_dv_orig, result_dv, passed)
    ...
    complex(8), intent(in) :: result_dv(nbdirs)
    logical, intent(out) :: passed

    real(8), parameter :: h = 1.0e-7
    real(8) :: relative_error, max_error, abs_error, abs_reference, error_bound
    complex(8) :: central_diff, ad_result, result_forward, result_backward
    ...
    do idir = 1, nbdirs
      ...
      result_forward = zdotu(nsize, x, incx_val, y, incy_val)
      x = x_orig - h * x_dv_orig(idir,:)
      y = y_orig - h * y_dv_orig(idir,:)
      result_backward = zdotu(nsize, x, incx_val, y, incy_val)
