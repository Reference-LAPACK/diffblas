...
program test_ztpmv_reverse
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing ZTPMV (multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
...
contains
  subroutine run_test_for_size(n, passed)
    ...
    complex(8), allocatable :: ap(:), apb(:), x(:), xb(:)
    complex(8), allocatable :: ap_orig(:), ap_plus(:), ap_minus(:), x_orig(:), x_plus(:), x_minus(:), xb_dir(:), apb_dir(:)
    integer :: ii
    real(4) :: tr, ti
    write(*,*) 'Testing ZTPMV (n =', n, ')'
    uplo = 'U'
    trans = 'N'
    diag = 'N'
    nsize = n
    ...
    call ztpmv_b(uplo, trans, diag, nsize, ap, apb, x, xb, incx_val)
  ...
  subroutine check_vjp_numerically(n, npack, uplo, trans, diag, nsize, incx_val, ap_orig, x_orig, xb_dir, apb_dir, xb_adj, apb_adj, passed)
    ...
    complex(8), intent(in) :: ap_orig(npack), x_orig(n), xb_dir(n), apb_dir(npack), xb_adj(n), apb_adj(npack)
    logical, intent(out) :: passed
    real(8), parameter :: h = 1.0e-7
    real(8) :: vjp_fd, vjp_ad, abs_error, abs_reference, error_bound, relative_error
    complex(8) :: ap_t(npack), x_t(n), x_plus(n), x_minus(n)
    integer :: i, j
    vjp_fd = 0.0d0
    do i = 1, n
      x_t = x_orig
...
end program test_ztpmv_reverse