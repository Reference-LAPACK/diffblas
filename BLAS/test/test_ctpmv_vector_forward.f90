...
program test_ctpmv_vector_forward
  ...
  complex(4), dimension(nbdirs,(max_size*(max_size+1))/2) :: ap_dv
  complex(4), dimension(nbdirs,max_size) :: x_dv
  ! Declare variables for storing original values
  complex(4), dimension((max_size*(max_size+1))/2) :: ap_orig
  complex(4), dimension(nbdirs,(max_size*(max_size+1))/2) :: ap_dv_orig
...
contains
  ...
  subroutine run_test_for_size(n, passed)
    ...
    write(*,*) 'Testing CTPMV (Vector Forward Mode)'
  ...
  subroutine check_derivatives_numerically(passed)
    ...
    do idir = 1, nbdirs
      ...
      ap = ap_orig + cmplx(h, 0.0) * ap_dv_orig(idir,:)
