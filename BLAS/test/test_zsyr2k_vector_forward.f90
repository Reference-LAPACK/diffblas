! Test program for ZSYR2K vector forward (BLAS3 outlined)
program test_zsyr2k_vector_forward
  implicit none
  external :: zsyr2k
  external :: zsyr2k_dv
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    uplo = 'L'
    transa = 'N'
    write(*,*) 'Testing ZSYR2K (Vector Forward, n =', n, ')'
    ...
    do k = 1, nbdirs
      ...
      call zsyr2k(uplo, transa, nsize, ksize, alpha + h*alpha_dv(k), a_t, lda_val, b_t, ldb_val, beta + h*beta_dv(k), c_plus, ldc_val)
      a_t = a - h * a_dv(k,:,:)
      b_t = b - h * b_dv(k,:,:)
      c_minus = c_orig - h * c_dv_seed(k,:,:)
      call zsyr2k(uplo, transa, nsize, ksize, alpha - h*alpha_dv(k), a_t, lda_val, b_t, ldb_val, beta - h*beta_dv(k), c_minus, ldc_val)
      max_err = 0.0d0
      do jj = 1, n
        do ii = 1, n
