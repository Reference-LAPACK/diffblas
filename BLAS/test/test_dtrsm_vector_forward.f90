! Test program for DTRSM vector forward (BLAS3 outlined)
program test_dtrsm_vector_forward
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing DTRSM (Vector Forward, multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
    nbdirs = n_test
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    real(8), dimension(nbdirs,n,n) :: b_dv_seed
    ...
    do k = 1, nbdirs
      ...
      call dtrsm(side, uplo, transa, diag, msize, nsize, alpha + h*alpha_dv(k), a_t, lda_val, b_plus, ldb_val)
      a_t = a - h * a_dv(k,:,:)
      b_minus = b_orig - h * b_dv_seed(k,:,:)
      call dtrsm(side, uplo, transa, diag, msize, nsize, alpha - h*alpha_dv(k), a_t, lda_val, b_minus, ldb_val)
      max_err = 0.0d0
      do jj = 1, n
        do ii = 1, n
