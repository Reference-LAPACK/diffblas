...
contains
  subroutine run_test_for_size(n, passed)
    ...
    real(8), allocatable :: ap(:), apb(:), x(:), xb(:)
    real(8), allocatable :: ap_orig(:), ap_plus(:), ap_minus(:), x_orig(:), x_plus(:), x_minus(:), xb_dir(:), apb_dir(:)
    integer :: ii
    uplo = 'U'
    trans = 'N'
    diag = 'N'
    ...
    call dtpmv_b(uplo, trans, diag, nsize, ap, apb, x, xb, incx_val)
  ...
  subroutine check_vjp_numerically(n, npack, uplo, trans, diag, nsize, incx_val, ap_orig, x_orig, xb_dir, apb_dir, xb_adj, apb_adj, passed)
    ...
    real(8), intent(in) :: ap_orig(npack), x_orig(n), xb_dir(n), apb_dir(npack), xb_adj(n), apb_adj(npack)
    logical, intent(out) :: passed
...
end program test_dtpmv_reverse