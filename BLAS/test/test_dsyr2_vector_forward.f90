...
program test_dsyr2_vector_forward
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing DSYR2 (Vector Forward, multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
    nbdirs = test_sizes(i)
...
contains
  ...
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    real(8) :: alpha_orig
    real(8), dimension(nbdirs) :: alpha_dv_seed
    real(8), dimension(n) :: x_orig
    real(8), dimension(nbdirs,n) :: x_dv_seed
    real(8), dimension(n) :: y
    real(8), dimension(nbdirs,n) :: y_dv
    real(8), dimension(n) :: y_orig
    real(8), dimension(nbdirs,n) :: y_dv_seed
    real(8), dimension(n,n) :: a_orig
    real(8), dimension(nbdirs,n,n) :: a_dv_seed
    integer :: ii, jj, idir
    ...
    write(*,*) 'Testing DSYR2 (Vector Forward, n =', n, ')'
    alpha_orig = alpha
  ...
  subroutine check_derivatives_numerically(n, nbdirs, uplo, nsize, lda_val, incx_val, incy_val, alpha_orig, alpha_dv_seed, x_orig, x_dv_seed, y_orig, y_dv_seed, a_orig, a_dv_seed, a_dv, passed)
    ...
    real(8), intent(in) :: alpha_dv_seed(nbdirs), x_orig(n), x_dv_seed(nbdirs,n)
    real(8), intent(in) :: y_orig(n), y_dv_seed(nbdirs,n)
    ...
    do idir = 1, nbdirs
      ...
      call dsyr2(uplo, nsize, alpha_t, x_t, incx_val, y_t, incy_val, a_t, lda_val)
      a_fwd = a_t
      alpha_t = alpha_orig - h * alpha_dv_seed(idir)
      x_t = x_orig - h * x_dv_seed(idir,:)
      y_t = y_orig - h * y_dv_seed(idir,:)
...
end program test_dsyr2_vector_forward