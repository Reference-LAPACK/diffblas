...
program test_cgemv_vector_reverse
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing CGEMV (Vector Reverse, multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
    nbdirs = test_sizes(i)
...
contains
  ...
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    complex(4), dimension(nbdirs,n) :: yb_orig
    integer :: k, ii, jj
  ...
  subroutine check_vjp_numerically(n, nbdirs, trans, msize, nsize, lda_val, incx_val, incy_val, alpha_orig, a_orig, x_orig, beta_orig, y_orig, yb_orig, alphab, ab, xb, betab, yb, passed)
    ...
    complex(4), intent(in) :: yb_orig(nbdirs,n)
    complex(4), intent(in) :: alphab(nbdirs), betab(nbdirs)
    complex(4), intent(in) :: ab(nbdirs,n,n), xb(nbdirs,n), yb(nbdirs,n)
    logical, intent(out) :: passed
    ...
    complex(4) :: alpha, beta
    complex(4), dimension(n,n) :: a
    complex(4), dimension(n) :: x, y, y_plus, y_minus, y_central_diff
