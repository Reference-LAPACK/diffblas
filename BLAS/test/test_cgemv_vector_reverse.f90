! Test program for CGEMV vector reverse mode differentiation
! Generated automatically by run_tapenade_blas.py
! Using REAL*4 precision with nbdirs=4

program test_cgemv_vector_reverse
  implicit none
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing CGEMV (Vector Reverse, multi-size: n = 4)'
  all_passed = .true.
  do itest = 1, 1
    n = test_sizes(itest)
    write(*,*) 'Testing CGEMV (Vector Reverse, n =', n, ')'
  ...
  call cgemv_bv(trans, msize, nsize, alpha, alphab, a, ab, lda_val, x, xb, incx_val, beta, betab, y, yb, incy_val, nbdirs)
...
contains
  ...
  subroutine check_vjp_numerically(passed)
    ...
    do k = 1, nbdirs
      ...
      call cgemv(trans, msize, nsize, alpha, a, lda_val, x, incx_val, beta, y, incy_val)
      y_plus = y
...
end program test_cgemv_vector_reverse