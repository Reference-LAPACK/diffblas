! Test program for CSYR2K differentiation
...
program test_csyr2k
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing CSYR2K (multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
...
contains
  ...
  subroutine run_test_for_size(n, passed)
    ...
    call random_number(temp_re)
    call random_number(temp_im)
    b = cmplx(temp_re * 2.0 - 1.0, temp_im * 2.0 - 1.0, kind=4)
    call random_number(temp_re)
    call random_number(temp_im)
    beta = cmplx(temp_re * 2.0 - 1.0, temp_im * 2.0 - 1.0, kind=4)
    call random_number(temp_re)
    call random_number(temp_im)
    c = cmplx(temp_re * 2.0 - 1.0, temp_im * 2.0 - 1.0, kind=4)
    ...
    write(*,*) 'Testing CSYR2K (n =', n, ')'
    c_orig = c

    ! Call the differentiated function
    call csyr2k_d(uplo, trans, nsize, ksize, alpha, alpha_d, a, a_d, lda_val, b, b_d, ldb_val, beta, beta_d, c, c_d, ldc_val)

    write(*,*) 'Function calls completed successfully'

    ! Numerical differentiation check
    call check_derivatives_numerically(n, trans, uplo, nsize, ksize, lda_val, ldb_val, ldc_val, a_orig, alpha_orig, c_orig, b_orig, beta_orig, a_d_orig, alpha_d_orig, c_d_orig, b_d_orig, beta_d_orig, c_d, passed)

  end subroutine run_test_for_size

  subroutine check_derivatives_numerically(n, trans, uplo, nsize, ksize, lda_val, ldb_val, ldc_val, a_orig, alpha_orig, c_orig, b_orig, beta_orig, a_d_orig, alpha_d_orig, c_d_orig, b_d_orig, beta_d_orig, c_d, passed)
    implicit none
    integer, intent(in) :: n
    ...
    call csyr2k(uplo, trans, nsize, ksize, alpha, a, lda_val, b, ldb_val, beta, c, ldc_val)
    c_forward = c
