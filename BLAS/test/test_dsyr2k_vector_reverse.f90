! Test program for DSYR2K vector reverse (BLAS3 outlined)
program test_dsyr2k_vector_reverse
  implicit none
  external :: dsyr2k
  external :: dsyr2k_bv
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing DSYR2K (Vector Reverse, multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
    nbdirs = n_test
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    real(8), dimension(n,n) :: c_plus, c_minus
    ...
    call dsyr2k_bv(uplo, transa, nsize, ksize, alpha, alphab, a, ab, lda_val, b, bb, ldb_val, beta, betab, c, cb, ldc_val, nbdirs)
    call set_ISIZE2OFA(-1)
    call set_ISIZE2OFB(-1)
    write(*,*) 'Testing DSYR2K (Vector Reverse, n =', n, ')'
    ...
    do k = 1, nbdirs
      ...
      b_t = b + h * bb(k,:,:)
      c_plus = c
      call dsyr2k(uplo, transa, nsize, ksize, alpha + h*alphab(k), a_t, lda_val, b_t, ldb_val, beta + h*betab(k), c_plus, ldc_val)
      a_t = a - h * ab(k,:,:)
      b_t = b - h * bb(k,:,:)
      c_minus = c
      call dsyr2k(uplo, transa, nsize, ksize, alpha - h*alphab(k), a_t, lda_val, b_t, ldb_val, beta - h*betab(k), c_minus, ldc_val)
      vjp_fd = 0.0d0
...
end program test_dsyr2k_vector_reverse