! Test program for ZTRMM differentiation (BLAS3 outlined)
...
program test_ztrmm
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing ZTRMM (multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
...
contains
  subroutine run_test_for_size(n, passed)
    ...
    b_d = 0.0d0
    b_orig = b
    call ztrmm_d(side, uplo, transa, diag, msize, nsize, alpha, alpha_d, a, a_d, lda_val, b, b_d, ldb_val)
    write(*,*) 'Testing ZTRMM (n =', n, ')'
    write(*,*) 'Function calls completed successfully'
    ! Finite-difference check: (output(alpha+h) - output(alpha-h))/(2h) vs derivative
    b_plus = b_orig
    call ztrmm(side, uplo, transa, diag, msize, nsize, alpha + h, a, lda_val, b_plus, ldb_val)
    b_minus = b_orig
    call ztrmm(side, uplo, transa, diag, msize, nsize, alpha - h, a, lda_val, b_minus, ldb_val)
    max_err = 0.0d0
    do jj = 1, n
      do ii = 1, n
