! Test program for DGER vector reverse mode differentiation
! Generated automatically by run_tapenade_blas.py
! Using REAL*8 precision with nbdirs=4

program test_dger_vector_reverse
  implicit none
  ...
  real(8), dimension(nbdirs,max_size,max_size) :: ab_orig
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing DGER (Vector Reverse, multi-size: n = 4)'
  all_passed = .true.
  do itest = 1, 1
    n = test_sizes(itest)
    write(*,*) 'Testing DGER (Vector Reverse, n =', n, ')'
...
contains
  ...
  subroutine run_test_for_size(n, passed)
    ...
    call dger_bv(msize, nsize, alpha, alphab, x, xb, incx_val, y, yb, incy_val, a, ab, lda_val, nbdirs)
  ...
  subroutine check_vjp_numerically(passed)
    ...
    real(8), dimension(max_size,max_size) :: a_plus, a_minus, a_central_diff
    ...
    do k = 1, nbdirs
      ...
      y = y_orig + h * y_dir
      a = a_orig + h * a_dir
      call dger(msize, nsize, alpha, x, incx_val, y, incy_val, a, lda_val)
      a_plus = a
      ...
      do j = 1, n
        do i = 1, n
          ...
          temp_products(n_products) = ab_orig(k,i,j) * a_central_diff(i,j)
        end do
      end do
...
end program test_dger_vector_reverse