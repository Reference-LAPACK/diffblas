! Test program for CTRSM forward (tangent) mode differentiation
...
program test_ctrsm
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing CTRSM (forward mode)'
  all_passed = .true.
  do id = 1, 2
    if (id == 1) then
      diag = 'N'
...
contains
  ...
  subroutine run_test_for_size(n, diag, passed)
    ...
    complex(4) :: alpha, alpha_d
    complex(4), dimension(n,n) :: a, b, a_d, b_d
    complex(4) :: alpha_orig, alpha_d_orig
    complex(4), dimension(n,n) :: a_orig, a_d_orig, b_orig, b_d_orig
    integer :: i
    ...
    call ctrsm_d(side, uplo, transa, diag, msize, nsize, alpha, alpha_d, &
  ...
  subroutine check_derivatives_numerically(n, side, uplo, transa, diag, &
    ...
    complex(4), intent(in) :: b_orig(n,n), b_d_orig(n,n), b_d(n,n)
    logical, intent(out) :: passed
    ...
    complex(4), dimension(n,n) :: b_forward, b_backward, a, b
    complex(4) :: alpha
    integer :: i, j
