...
program test_dtpmv_vector_reverse
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing DTPMV (Vector Reverse, multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
    nbdirs = test_sizes(i)
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    real(8), allocatable :: apb(:,:), xb(:,:)
    real(8), allocatable :: ap_orig(:), x_orig(:), xb_orig(:,:)
    integer :: idir, ii
    real(4) :: tr, ti
    uplo = 'L'
    trans = 'N'
    ...
    xb_orig = xb
    apb = 0.0d0
    write(*,*) 'Testing DTPMV (Vector Reverse, n =', n, ')'
  ...
  subroutine check_vjp_numerically(n, npack, nbdirs, uplo, trans, diag, nsize, incx_val, ap_orig, x_orig, xb_orig, apb, xb, passed)
    ...
    real(8), intent(in) :: ap_orig(npack), x_orig(n), xb_orig(nbdirs,n)
    real(8), intent(in) :: apb(nbdirs,npack), xb(nbdirs,n)
    logical, intent(out) :: passed
    real(8), parameter :: h = 1.0e-7
    real(8) :: vjp_ad, vjp_fd, relative_error, max_error, abs_error, abs_reference, error_bound
    real(8), allocatable :: ap(:), x(:), ap_dir(:), x_dir(:), x_plus(:), x_minus(:)
...
end program test_dtpmv_vector_reverse