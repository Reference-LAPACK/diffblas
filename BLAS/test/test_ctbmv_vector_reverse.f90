! Test program for CTBMV vector reverse - BLAS2 band
! Generated automatically by run_tapenade_blas.py
! Multi-size outlined run_test_for_size(n, passed, nbdirs)

program test_ctbmv_vector_reverse
  implicit none
  external :: ctbmv
  external :: ctbmv_bv
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing CTBMV (Vector Reverse band, multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
    nbdirs = test_sizes(i)
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    complex(4), dimension(:,:), allocatable :: xb, yb, xb_seed, yb_seed
    integer :: band_row, j
    ...
    do j = 1, n
      do band_row = 1, nbdirs
        call random_number(temp_real)
        call random_number(temp_imag)
        xb(band_row, j) = cmplx(temp_real*2.0-1.0, temp_imag*2.0-1.0, kind=kind(xb))
      end do
    end do
    xb_seed = xb
    write(*,*) 'Testing CTBMV (Vector Reverse band, n =', n, ')'
    call set_ISIZE2OFA(n)
    call ctbmv_bv(uplo, trans, diag, nsize, ksize, a, ab, lda_val, x, xb, incx_val, nbdirs)
  ...
  subroutine check_vjp_numerically_band_vec(n, nbdirs, lda_val, ksize, uplo, trans, diag, nsize, incx_val, a, ab, x, xb_seed, xb, passed)
    ...
    complex(4), intent(in) :: a(lda_val, n), ab(nbdirs, lda_val, n), x(n), xb_seed(nbdirs, n), xb(nbdirs, n)
    logical, intent(out) :: passed
    ...
    n_products = n
    do i = 1, n
      temp_products(i) = real(conjg(xb_seed(k,i)) * ((x_plus(i) - x_minus(i)) / (2.0d0 * h)))
    end do
...
end program test_ctbmv_vector_reverse