! Test program for CGBMV reverse mode (adjoint) differentiation
...
program test_cgbmv_reverse
  ...
  complex(4) :: betab
  complex(4), dimension(max_size) :: yb
  ...
  complex(4), dimension(max_size) :: y_plus, y_minus

  ! Saved cotangents (output adjoints) for VJP verification
  complex(4), dimension(max_size) :: yb_orig
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing CGBMV (multi-size: n = 4)'
  all_passed = .true.
  do itest = 1, 1
    n = test_sizes(itest)
    write(*,*) 'Testing CGBMV (n =', n, ')'
  ...
  call cgbmv_b(trans, msize, nsize, kl, ku, alpha, alphab, a, ab, lda_val, x, xb, incx_val, beta, betab, y, yb, incy_val)
...
contains
  ...
  subroutine check_vjp_numerically(passed)
    ...
    complex(4), dimension(max_size) :: y_central_diff
...
end program test_cgbmv_reverse