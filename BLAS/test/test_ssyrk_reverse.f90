! Test program for SSYRK reverse (BLAS3 outlined)
program test_ssyrk_reverse
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing SSYRK (multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 1
...
contains
  subroutine run_test_for_size(n, passed)
    ...
    real(4), dimension(n,n) :: a, ab, c, cb
    ...
    call ssyrk_b(uplo, transa, nsize, ksize, alpha, alphab, a, ab, lda_val, beta, betab, c, cb, ldc_val)
    ...
    c_plus = c
    call ssyrk(uplo, transa, nsize, ksize, alpha + h*alphab, a + h*ab, lda_val, beta + h*betab, c_plus, ldc_val)
    c_minus = c
    call ssyrk(uplo, transa, nsize, ksize, alpha - h*alphab, a - h*ab, lda_val, beta - h*betab, c_minus, ldc_val)
    vjp_fd = 0.0d0
    do jj = 1, n
      do ii = 1, n
...
end program test_ssyrk_reverse