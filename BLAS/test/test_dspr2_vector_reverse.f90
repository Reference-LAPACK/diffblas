...
program test_dspr2_vector_reverse
  ...
  real(8), dimension(nbdirsmax,(n*(n+1))/2) :: apb_orig
  ...
  call dspr2_bv(uplo, nsize, alpha, alphab, x, xb, incx_val, y, yb, incy_val, ap, apb, nbdirsmax)
...
contains
  ...
  subroutine check_vjp_numerically()
    ...
    real(8), dimension((n*(n+1))/2) :: ap_dir
    real(8), dimension((n*(n+1))/2) :: ap_plus, ap_minus, ap_central_diff
