! Test program for DDOT differentiation
...
program test_ddot
  ...
  real(8) :: ddot_result, ddot_d_result
  real(8) :: ddot_forward, ddot_backward

  ! Variables for storing original derivative values
  real(8), dimension(4) :: dy_d_orig
  ...
  ddot_result = ddot(nsize, dx, incx_val, dy, incy_val)
  ...
  incx_val = 1
  ! dy already has correct value from original call
  incy_val = 1

  ! Call the differentiated function
  ddot_d_result = ddot_d(nsize, dx, dx_d, incx_val, dy, dy_d, incy_val, ddot_result)
...
contains
  ...
  subroutine check_derivatives_numerically()
    ...
    ! ddot_forward already captured above
    ...
    ! ddot_backward already captured above
    
    ! Compute central differences and compare with AD results
    ! Check derivatives for function DDOT
    ! Central difference: (f(x+h) - f(x-h)) / (2h)
    central_diff = (ddot_forward - ddot_backward) / (2.0e0 * h)
    ! AD result
    ad_result = ddot_d_result
    ...
    if (abs_error > error_bound) then
      ...
      write(*,*) 'Large error in function DDOT:'
