! Test program for DTRMM reverse mode (adjoint) differentiation
...
program test_dtrmm_reverse
  ...
  real(8), dimension(max_size,max_size) :: b_plus, b_minus

  ! Saved cotangents (output adjoints) for VJP verification
  real(8), dimension(max_size,max_size) :: bb_orig
...
contains
  ...
  subroutine check_vjp_numerically()
    ...
    real(8), dimension(max_size,max_size) :: b_central_diff
