...
program test_sswap_vector_forward
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing SSWAP (Vector Forward, multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
    nbdirs = test_sizes(i)
...
contains
  ...
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    write(*,*) 'Testing SSWAP (Vector Forward, n =', n, ')'

    call sswap_dv(nsize, x, x_dv, incx_val, y, y_dv, incy_val, nbdirs)
  ...
  subroutine check_derivatives_numerically(n, nbdirs, nsize, incx_val, incy_val, x_orig, x_dv_orig, y_orig, y_dv_orig, y_dv, passed)
    ...
    do idir = 1, nbdirs
      ...
      y = y_orig
      call sswap(nsize, x, incx_val, y, incy_val)
      y_forward = y
      x = x_orig - h * x_dv_orig(idir,:)
      y = y_orig
      call sswap(nsize, x, incx_val, y, incy_val)
...
end program test_sswap_vector_forward