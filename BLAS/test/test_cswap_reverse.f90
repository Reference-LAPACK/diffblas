...
program test_cswap_reverse
  ...
  complex(4), dimension(max_size) :: cyb
  ...
  do i = 1, max_size
    call random_number(temp_real_init)
    call random_number(temp_imag_init)
    cy(i) = cmplx(temp_real_init, temp_imag_init) * (2.0,2.0) - (1.0,1.0)
  end do
  incy_val = 1
  ...
  do i = 1, max_size
    call random_number(temp_real_init)
    call random_number(temp_imag_init)
    cyb(i) = cmplx(temp_real_init, temp_imag_init) * (2.0,2.0) - (1.0,1.0)
  end do
  ...
  call cswap_b(nsize, cx, cxb, incx_val, cy, cyb, incy_val)
...
contains
  ...
  subroutine check_vjp_numerically()
    ...
    complex(4), dimension(max_size) :: cy_central_diff
    ...
    do i = 1, max_size
      call random_number(temp_real)
      call random_number(temp_imag)
      cy_dir(i) = cmplx(temp_real, temp_imag) * (2.0,2.0) - (1.0,1.0)
    end do
    ...
    cy_central_diff = (cy_plus - cy_minus) / (2.0d0 * h)
...
end program test_cswap_reverse