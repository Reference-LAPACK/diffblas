! Test program for CTRSV forward (tangent) mode differentiation
! Hand-written driver following the structure of test_zgemv.f90.
! COMPLEX*8. Multi-size, sweeps DIAG in {'N','U'}.
...
program test_ctrsv
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing CTRSV (forward mode)'
  all_passed = .true.
  do id = 1, 2
    if (id == 1) then
      diag = 'N'
...
contains
  ...
  subroutine run_test_for_size(n, diag, passed)
    ...
    complex(4), dimension(n,n) :: a, a_d
    complex(4), dimension(n) :: x, x_d
    ...
    call ctrsv_d(uplo, trans, diag, nsize, a, a_d, lda_val, x, x_d, incx)
  ...
  subroutine check_derivatives_numerically(n, uplo, trans, diag, nsize, &
    ...
    complex(4), intent(in) :: x_orig(n), x_d_orig(n), x_d(n)
    logical, intent(out) :: passed

    real(8), parameter :: h = 1.0d-3
    real(8) :: max_error, abs_error, abs_reference, error_bound
    complex(4) :: central_diff
    logical :: has_large_errors
    complex(4), dimension(n,n) :: a
    complex(4), dimension(n) :: x, x_forward, x_backward
    integer :: i
    ...
    do i = 1, n
      ...
      error_bound = 1.0d-2 + 1.0d-2 * abs_reference
