! Test program for CTPMV vector reverse mode differentiation
! Multi-size outlined run_test_for_size(n) - TPMV/TPSV packed triangular

program test_ctpmv_vector_reverse
  implicit none
  external :: ctpmv
  external :: ctpmv_bv
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing CTPMV (Vector Reverse, multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
    nbdirs = test_sizes(i)
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    complex(4), allocatable :: apb(:,:), xb(:,:)
    complex(4), allocatable :: ap_orig(:), x_orig(:), xb_orig(:,:)
    integer :: idir, ii
    real(4) :: tr, ti
    uplo = 'L'
    trans = 'N'
  ...
  subroutine check_vjp_numerically(n, npack, nbdirs, uplo, trans, diag, nsize, incx_val, ap_orig, x_orig, xb_orig, apb, xb, passed)
    ...
    complex(4), intent(in) :: ap_orig(npack), x_orig(n), xb_orig(nbdirs,n)
    complex(4), intent(in) :: apb(nbdirs,npack), xb(nbdirs,n)
    logical, intent(out) :: passed
    real(4), parameter :: h = 1.0e-3
    real(4) :: vjp_ad, vjp_fd, relative_error, max_error, abs_error, abs_reference, error_bound
    complex(4), allocatable :: ap(:), x(:), ap_dir(:), x_dir(:), x_plus(:), x_minus(:)
...
end program test_ctpmv_vector_reverse