...
contains
  ...
  subroutine check_vjp_spr_spr2(n, npack, nbdirs, uplo, nsize, incx_val, incy_val, alpha, x, ap, apb_orig, alphab, xb, apb, passed, y, yb)
    ...
    do k = 1, nbdirs
      ...
      x_t = x + h * x_dir
      if (present(y)) then
        call sspr(uplo, nsize, alpha + h*alpha_dir, x_t, incx_val, y_t, incy_val, ap_t)
      else
        call sspr(uplo, nsize, alpha + h*alpha_dir, x_t, incx_val, ap_t)
      end if
      ap_plus = ap_t
      ap_t = ap - h * ap_dir
      x_t = x - h * x_dir
...
end program test_sspr_vector_reverse