! Test program for ZHBMV vector forward - BLAS2 band
! Generated automatically by run_tapenade_blas.py
! Multi-size outlined run_test_for_size(n, passed, nbdirs) - band

program test_zhbmv_vector_forward
  implicit none
  external :: zhbmv
  external :: zhbmv_dv
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing ZHBMV (Vector Forward band, multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
    nbdirs = test_sizes(i)
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    write(*,*) 'Testing ZHBMV (Vector Forward band, n =', n, ')'
    a_orig = a
    x_orig = x
    a_dv_seed = a_dv
    x_dv_seed = x_dv
    ...
    call zhbmv_dv(uplo, nsize, ksize, alpha, alpha_dv, a, a_dv, lda_val, x, x_dv, incx_val, beta, beta_dv, y, y_dv, incy_val, nbdirs)
...
end program test_zhbmv_vector_forward