! Test program for ZDOTU reverse mode (adjoint) differentiation
! Generated automatically by run_tapenade_blas.py
! Using REAL*8 precision
! Multi-size test with outlined run_test_for_size(n) - arrays declared to size n

program test_zdotu_reverse
  implicit none
...
contains
  ...
  subroutine run_test_for_size(n, passed)
    ...
    call random_number(temp_re)
    call random_number(temp_im)
    zdotub = cmplx(temp_re * 2.0 - 1.0, temp_im * 2.0 - 1.0, kind=4)
    zdotub_orig = zdotub
    ...
    call zdotu_b(nsize, zx, zxb, incx_val, zy, zyb, incy_val, zdotub)

    call set_ISIZE1OFZx(-1)
    call set_ISIZE1OFZy(-1)

    call check_vjp_numerically(n, nsize, incx_val, incy_val, zx_orig, zy_orig, zxb, zyb, zdotub_orig, passed)

  end subroutine run_test_for_size

  subroutine check_vjp_numerically(n, nsize, incx_val, incy_val, zx_orig, zy_orig, zxb, zyb, zdotub_orig, passed)
    implicit none
    integer, intent(in) :: n
    ...
    complex(8), intent(in) :: zdotub_orig
    logical, intent(out) :: passed
    ...
    vjp_fd = real(conjg(zdotub_orig) * (zdotu_plus - zdotu_minus) / (2.0 * h))
...
end program test_zdotu_reverse