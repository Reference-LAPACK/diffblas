...
program test_zdotu_reverse
  ...
  call random_number(temp_real_init)
  call random_number(temp_imag_init)
  zdotub = cmplx(temp_real_init, temp_imag_init) * (2.0,2.0) - (1.0,1.0)
...
contains
  ...
  subroutine check_vjp_numerically()
    ...
    zdotu_central_diff = (zdotu_plus - zdotu_minus) / (2.0d0 * h)
    ...
    vjp_fd = 0.0d0
    vjp_fd = vjp_fd + real(conjg(zdotub_orig) * zdotu_central_diff)
