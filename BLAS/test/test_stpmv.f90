! Test program for STPMV differentiation
...
contains
  subroutine run_test_for_size(n, passed)
    ...
    real(4), allocatable :: ap(:), ap_d(:), x(:), x_d(:)
    real(4), allocatable :: ap_t(:), x_t(:), x_plus(:), x_minus(:)
    real(4), allocatable :: ap_d_seed(:), x_d_seed(:)
  ...
  subroutine check_derivatives_numerically(n, npack, uplo, trans, diag, nsize, incx_val, ap, ap_d_seed, x, x_d_seed, x_d, passed)
    ...
    real(4), intent(in) :: ap(npack), ap_d_seed(npack), x(n), x_d_seed(n), x_d(n)
    logical, intent(out) :: passed
