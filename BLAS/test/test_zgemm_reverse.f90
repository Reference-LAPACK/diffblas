! Test program for ZGEMM reverse mode (adjoint) differentiation
! Generated automatically by run_tapenade_blas.py
! Using REAL*8 precision
! Multi-size test with outlined run_test_for_size(n) - arrays declared to size n

program test_zgemm_reverse
  implicit none
...
contains
  ...
  subroutine run_test_for_size(n, passed)
    ...
    complex(8) :: alphab, betab
    complex(8), dimension(n,n) :: ab, bb, cb
    complex(8) :: alpha_orig, beta_orig
    complex(8), dimension(n,n) :: a_orig, b_orig, c_orig, cb_orig
    ...
    do j = 1, n
      do i = 1, n
        call random_number(temp_re)
        call random_number(temp_im)
        a(i,j) = cmplx(temp_re * 2.0 - 1.0, temp_im * 2.0 - 1.0, kind=8)
      end do
    end do
    do j = 1, n
      do i = 1, n
        call random_number(temp_re)
        call random_number(temp_im)
        b(i,j) = cmplx(temp_re * 2.0 - 1.0, temp_im * 2.0 - 1.0, kind=8)
      end do
    end do
    call random_number(temp_re)
    call random_number(temp_im)
    ...
    do j = 1, n
      do i = 1, n
        call random_number(temp_re)
        call random_number(temp_im)
        c(i,j) = cmplx(temp_re * 2.0 - 1.0, temp_im * 2.0 - 1.0, kind=8)
      end do
    end do
    ...
    do j = 1, n
      do i = 1, n
        call random_number(temp_re)
        call random_number(temp_im)
        cb(i,j) = cmplx(temp_re * 2.0 - 1.0, temp_im * 2.0 - 1.0, kind=8)
      end do
    end do
    cb_orig = cb
    ...
    call zgemm_b(transa, transb, msize, nsize, ksize, alpha, alphab, a, ab, lda_val, b, bb, ldb_val, beta, betab, c, cb, ldc_val)
  ...
  subroutine check_vjp_numerically(n, transa, transb, msize, nsize, ksize, lda_val, ldb_val, ldc_val, alpha_orig, a_orig, b_orig, beta_orig, c_orig, cb_orig, alphab, ab, bb, betab, cb, passed)
    ...
    complex(8), intent(in) :: a_orig(n,n), b_orig(n,n), c_orig(n,n), cb_orig(n,n)
    complex(8), intent(in) :: alphab, betab
    complex(8), intent(in) :: ab(n,n), bb(n,n), cb(n,n)
    logical, intent(out) :: passed
    ...
    call random_number(temp_re)
    call random_number(temp_im)
    alpha_dir = cmplx(temp_re * 2.0 - 1.0, temp_im * 2.0 - 1.0, kind=8)
    do j = 1, n
      do i = 1, n
        call random_number(temp_re)
        call random_number(temp_im)
        a_dir(i,j) = cmplx(temp_re * 2.0 - 1.0, temp_im * 2.0 - 1.0, kind=8)
      end do
    end do
    do j = 1, n
      do i = 1, n
        call random_number(temp_re)
        call random_number(temp_im)
        b_dir(i,j) = cmplx(temp_re * 2.0 - 1.0, temp_im * 2.0 - 1.0, kind=8)
      end do
    end do
    call random_number(temp_re)
    call random_number(temp_im)
    beta_dir = cmplx(temp_re * 2.0 - 1.0, temp_im * 2.0 - 1.0, kind=8)
    do j = 1, n
      do i = 1, n
        call random_number(temp_re)
        call random_number(temp_im)
        c_dir(i,j) = cmplx(temp_re * 2.0 - 1.0, temp_im * 2.0 - 1.0, kind=8)
      end do
    end do
...
end program test_zgemm_reverse