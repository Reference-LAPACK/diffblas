! Test program for DTRMV differentiation
...
contains
  ...
  subroutine run_test_for_size(n, passed)
    ...
    call dtrmv_d(uplo, trans, diag, nsize, a, a_d, lda_val, x, x_d, 1)
    a_d = a_d_orig
  ...
  subroutine check_derivatives_numerically(n, trans, uplo, diag, nsize, lda_val, x_orig, a_orig, x_d_orig, a_d_orig, x_d, passed)
    ...
    x = x_orig + h * x_d_orig
    a = a_orig + h * a_d_orig
    call dtrmv(uplo, trans, diag, nsize, a, lda_val, x, 1)
    x_forward = x
