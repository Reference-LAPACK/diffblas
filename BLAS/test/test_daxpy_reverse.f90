! Test program for DAXPY reverse mode (adjoint) differentiation
! Generated automatically by run_tapenade_blas.py
! Using REAL*8 precision
! Multi-size test with outlined run_test_for_size(n) - arrays declared to size n

program test_daxpy_reverse
  implicit none
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing DAXPY (multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
...
contains
  ...
  subroutine run_test_for_size(n, passed)
    ...
    call daxpy_b(nsize, da, dab, dx, dxb, incx_val, dy, dyb, incy_val)

    call set_ISIZE1OFDx(-1)

    call check_vjp_numerically(n, nsize, incx_val, incy_val, da_orig, dx_orig, dy_orig, dyb_orig, dab, dxb, dyb, passed)

  end subroutine run_test_for_size

  subroutine check_vjp_numerically(n, nsize, incx_val, incy_val, da_orig, dx_orig, dy_orig, dyb_orig, dab, dxb, dyb, passed)
    implicit none
    integer, intent(in) :: n
...
end program test_daxpy_reverse