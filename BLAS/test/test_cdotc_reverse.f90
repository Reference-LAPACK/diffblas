...
program test_cdotc_reverse
  ...
  call random_number(temp_real_init)
  call random_number(temp_imag_init)
  cdotcb = cmplx(temp_real_init, temp_imag_init) * (2.0,2.0) - (1.0,1.0)
...
contains
  ...
  subroutine check_vjp_numerically()
    ...
    cdotc_central_diff = (cdotc_plus - cdotc_minus) / (2.0d0 * h)
    ...
    vjp_fd = 0.0
    vjp_fd = vjp_fd + real(conjg(cdotcb_orig) * cdotc_central_diff)
