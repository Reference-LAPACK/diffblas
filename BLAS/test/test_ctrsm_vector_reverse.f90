! Test program for CTRSM vector reverse (BLAS3 outlined)
program test_ctrsm_vector_reverse
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing CTRSM (Vector Reverse, multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
    nbdirs = n_test
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    call ctrsm_bv(side, uplo, transa, diag, msize, nsize, alpha, alphab, a, ab, lda_val, b, bb, ldb_val, nbdirs)
    call set_ISIZE2OFA(-1)
    write(*,*) 'Testing CTRSM (Vector Reverse, n =', n, ')'
