! Test program for ZSYRK vector reverse mode differentiation
...
program test_zsyrk_vector_reverse
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing ZSYRK (Vector Reverse, multi-size: n = 4)'
  all_passed = .true.
  do itest = 1, 1
    n = test_sizes(itest)
  ...
  call zsyrk_bv(uplo, trans, nsize, ksize, alpha, alphab, a, ab, lda_val, beta, betab, c, cb, ldc_val, nbdirs)
