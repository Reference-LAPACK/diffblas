! Test program for CHEMM reverse mode (adjoint) differentiation
...
program test_chemm_reverse
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing CHEMM (multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
...
contains
  ...
  subroutine check_vjp_numerically(n, side, uplo, msize, nsize, lda_val, ldb_val, ldc_val, alpha_orig, a_orig, b_orig, beta_orig, c_orig, cb_orig, alphab, ab, bb, betab, cb, passed)
    ...
    call chemm(side, uplo, msize, nsize, alpha, a, lda_val, b, ldb_val, beta, c, ldc_val)
    c_plus = c
