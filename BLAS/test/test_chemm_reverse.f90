! Test program for CHEMM reverse (BLAS3 outlined)
program test_chemm_reverse
  implicit none
  external :: chemm
  external :: chemm_b
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing CHEMM (multi-size: n =', test_sizes(1), ')'
  all_passed = .true.
  do i = 1, 3
...
contains
  subroutine run_test_for_size(n, passed)
    ...
    complex(4), dimension(n,n) :: a, ab, b, bb, c, cb
    complex(4), dimension(n,n) :: cb_seed, c_plus, c_minus
    complex(4), dimension(n,n) :: c_orig
    complex(4) :: alpha_dir, beta_dir
    complex(4), dimension(n,n) :: a_dir, b_dir, c_dir, a_fd, b_fd
    ...
    ! Initialize a as Hermitian matrix (matches BLAS/test)
    do ii = 1, n
      call random_number(tr)
      a(ii,ii) = cmplx(tr*2.0-1.0, 0.0, kind=kind(a))
    end do
    do jj = 1, n
      do ii = jj+1, n
        call random_number(tr)
        call random_number(ti)
        a(jj,ii) = cmplx(tr*2.0-1.0, ti*2.0-1.0, kind=kind(a))
    ...
    call chemm_b(side, uplo, msize, nsize, alpha, alphab, a, ab, lda_val, b, bb, ldb_val, beta, betab, c, cb, ldc_val)
    ...
    call chemm(side, uplo, msize, nsize, alpha + h*alpha_dir, a_fd, lda_val, b_fd, ldb_val, beta + h*beta_dir, c_plus, ldc_val)
    a_fd = a - h * a_dir
    b_fd = b - h * b_dir
    c_minus = c_orig - h * c_dir
    call chemm(side, uplo, msize, nsize, alpha - h*alpha_dir, a_fd, lda_val, b_fd, ldb_val, beta - h*beta_dir, c_minus, ldc_val)
    vjp_fd = 0.0d0
    do jj = 1, n
      do ii = 1, n
    ...
    vjp_ad_b     = 0.0d0
    vjp_ad_c     = 0.0d0
    vjp_ad_alpha = real(conjg(alpha_dir) * alphab)
    vjp_ad_beta  = real(conjg(beta_dir) * betab)
    vjp_ad = vjp_ad + vjp_ad_alpha + vjp_ad_beta
    do ii = 1, n
      a_dir(ii,ii) = cmplx(real(a_dir(ii,ii)), 0.0, kind=kind(a_dir))
    end do
    do jj = 1, n
      do ii = jj+1, n
    ...
    vjp_ad_a = sum(real(conjg(a_dir) * ab))
    vjp_ad_b = sum(real(conjg(b_dir) * bb))
    vjp_ad_c = sum(real(conjg(c_dir) * cb))
...
end program test_chemm_reverse