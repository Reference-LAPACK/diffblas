...
program test_ssbmv_reverse
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing SSBMV (multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 1
    n_test = test_sizes(i)
...
contains
  subroutine run_test_for_size(n, passed)
    ...
    real(4) :: alpha, alphab
    real(4) :: beta, betab
    real(4), dimension(:,:), allocatable :: a, ab
    real(4), dimension(:), allocatable :: x, xb
    real(4), dimension(:), allocatable :: y, yb
    integer :: band_row, j
    real(4) :: temp_real
    ksize = max(0, n - 1)
    nsize = n
    ...
    ab = 0.0d0
    yb = 0.0d0
    write(*,*) 'Testing SSBMV (n =', n, ')'
    call set_ISIZE1OFX(n)
    call set_ISIZE2OFA(lda_val)
    call ssbmv_b(uplo, nsize, ksize, alpha, alphab, a, ab, lda_val, x, xb, incx_val, beta, betab, y, yb, incy_val)
  ...
  subroutine check_vjp_numerically_band(n, lda_val, ksize, uplo, nsize, incx_val, incy_val, alpha, alphab, beta, betab, a, ab, x, xb, y, yb, passed)
    ...
    real(4), intent(in) :: alpha, alphab, beta, betab
    real(4), intent(in) :: a(lda_val, n), ab(lda_val, n), x(n), xb(n), y(n), yb(n)
    logical, intent(out) :: passed
    real(4), parameter :: h = 1.0e-7
    real(4) :: vjp_fd, vjp_ad, abs_error, abs_ref, err_bound
    real(4), dimension(n) :: y_plus, y_minus, y_t
    real(4) :: alpha_t
    ...
    x_t = x + h * xb
    y_t = y + h * yb
    call ssbmv(uplo, nsize, ksize, alpha_t, a_t, lda_val, x_t, incx_val, beta, y_t, incy_val)
    y_plus = y_t
    alpha_t = alpha - h * alphab
    a_t = a - h * ab
    x_t = x - h * xb
    ...
    n_products = n
    do i = 1, n
      temp_products(i) = yb(i) * ((y_plus(i) - y_minus(i)) / (2.0d0 * h))
    end do
    ...
    vjp_ad = 0.0d0
    vjp_ad = vjp_ad + alphab * alphab
    do i = 1, n
      vjp_ad = vjp_ad + xb(i) * xb(i)
    end do
    do i = 1, n
      vjp_ad = vjp_ad + yb(i) * yb(i)
    end do
    n_products = 0
    do j = 1, n
      do band_row = max(1, ksize+2-j), ksize+1
...
end program test_ssbmv_reverse