! Test program for SSBMV reverse mode (adjoint) differentiation
...
program test_ssbmv_reverse
  ...
  real(4) :: alphab
  real(4), dimension(max_size,max_size) :: ab  ! Band storage
  ...
  real(4), dimension(max_size) :: y_plus, y_minus

  ! Saved cotangents (output adjoints) for VJP verification
  real(4), dimension(max_size) :: yb_orig
...
contains
  ...
  subroutine check_vjp_numerically(passed)
    ...
    real(4) :: alpha_dir
    real(4), dimension(max_size,max_size) :: a_dir  ! Band storage
    real(4), dimension(max_size) :: x_dir
    real(4) :: beta_dir
    real(4), dimension(max_size) :: y_dir
    
    real(4), dimension(max_size) :: y_central_diff
    ...
    y = y_orig + h * y_dir
    call ssbmv(uplo, nsize, ksize, alpha, a, lda_val, x, incx_val, beta, y, incy_val)
    y_plus = y
