...
contains
  subroutine run_test_for_size(n, passed)
    ...
    real(4), dimension(:), allocatable :: y, yb, yb_seed
    integer :: band_row, j
    real(4) :: temp_real
    ksize = max(0, n - 1)
    nsize = n
  ...
  subroutine check_vjp_numerically_band(n, lda_val, ksize, uplo, nsize, incx_val, incy_val, alpha, alphab, beta, betab, a, ab, x, xb, y, yb_seed, yb, passed)
    ...
    real(4), intent(in) :: a(lda_val, n), ab(lda_val, n), x(n), xb(n), y(n), yb_seed(n), yb(n)
    logical, intent(out) :: passed
    ...
    do i = 1, n
      n_products = n_products + 1
      temp_products(n_products) = x_dir(i) * xb(i)
    end do
    do i = 1, n
      n_products = n_products + 1
      temp_products(n_products) = y_dir(i) * yb(i)
    end do
    call sort_array(temp_products, n_products)
    do i = 1, n_products
