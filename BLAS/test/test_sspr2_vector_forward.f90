! Test program for SSPR2 vector forward mode differentiation
! Generated automatically by run_tapenade_blas.py
! Using REAL*4 precision with nbdirs=4

program test_sspr2_vector_forward
  implicit none
  ...
  real(4), dimension(nbdirs,(max_size*(max_size+1))/2) :: ap_dv
  ...
  real(4), dimension(nbdirs,(max_size*(max_size+1))/2) :: ap_dv_orig

  test_sizes = (/ 4 /)
  write(*,*) 'Testing SSPR2 (Vector Forward, multi-size: n = 4)'
  all_passed = .true.
  do itest = 1, 1
    n = test_sizes(itest)
    write(*,*) 'Testing SSPR2 (Vector Forward, n =', n, ')'
...
contains
  ...
  subroutine run_test_for_size(n, passed)
    ...
    write(*,*) 'Testing SSPR2 (Vector Forward Mode)'
    ...
    call sspr2_dv(uplo, nsize, alpha, alpha_dv, x, x_dv, incx_val, y, y_dv, incy_val, ap, ap_dv, nbdirs)
  ...
  subroutine check_derivatives_numerically(passed)
    ...
    real(4), dimension((max_size*(max_size+1))/2) :: ap_forward, ap_backward
...
end program test_sspr2_vector_forward