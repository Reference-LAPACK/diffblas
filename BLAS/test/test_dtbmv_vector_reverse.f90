! Test program for DTBMV vector reverse mode differentiation
...
program test_dtbmv_vector_reverse
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing DTBMV (Vector Reverse, multi-size: n = 4)'
  all_passed = .true.
  do itest = 1, 1
    n = test_sizes(itest)
    write(*,*) 'Testing DTBMV (Vector Reverse, n =', n, ')'
...
contains
  ...
  subroutine check_vjp_numerically(passed)
    ...
    do k = 1, nbdirs
      ...
      a = a_orig + h * a_dir
      x = x_orig + h * x_dir
      call dtbmv(uplo, trans, diag, nsize, ksize, a, lda_val, x, incx_val)
      x_plus = x
