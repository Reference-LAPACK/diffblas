! Test program for DTBMV vector reverse - BLAS2 band
! Generated automatically by run_tapenade_blas.py
! Multi-size outlined run_test_for_size(n, passed, nbdirs)

program test_dtbmv_vector_reverse
  implicit none
  external :: dtbmv
  external :: dtbmv_bv
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing DTBMV (Vector Reverse band, multi-size: n = 4)'
  all_passed = .true.
  do i = 1, 3
    n_test = test_sizes(i)
    nbdirs = test_sizes(i)
...
contains
  subroutine run_test_for_size(n, passed, nbdirs)
    ...
    xb = xb * 2.0d0 - 1.0d0
    xb_seed = xb
    write(*,*) 'Testing DTBMV (Vector Reverse band, n =', n, ')'
    call set_ISIZE2OFA(n)
    call dtbmv_bv(uplo, trans, diag, nsize, ksize, a, ab, lda_val, x, xb, incx_val, nbdirs)
  ...
  subroutine check_vjp_numerically_band_vec(n, nbdirs, lda_val, ksize, uplo, trans, diag, nsize, incx_val, a, ab, x, xb_seed, xb, passed)
    ...
    real(8), intent(in) :: a(lda_val, n), ab(nbdirs, lda_val, n), x(n), xb_seed(nbdirs, n), xb(nbdirs, n)
    logical, intent(out) :: passed
    real(8), parameter :: h = 1.0e-7
    real(8) :: vjp_fd, vjp_ad, abs_error, abs_ref, err_bound, relative_error, max_re
    real(8), dimension(n) :: x_plus, x_minus, x_t, x_dir
...
end program test_dtbmv_vector_reverse