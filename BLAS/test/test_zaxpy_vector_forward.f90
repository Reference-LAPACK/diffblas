...
program test_zaxpy_vector_forward
  ...
  test_sizes = (/ 4 /)
  write(*,*) 'Testing ZAXPY (Vector Forward, multi-size: n = 4)'
  all_passed = .true.
  do itest = 1, 1
    n = test_sizes(itest)
    write(*,*) 'Testing ZAXPY (Vector Forward, n =', n, ')'
  ...
  call zaxpy_dv(nsize, za, za_dv, zx, zx_dv, incx_val, zy, zy_dv, incy_val, nbdirs)
