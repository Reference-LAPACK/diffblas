! Test program for ZTRSV vector reverse (adjoint) mode differentiation
...
program test_ztrsv_vector_reverse
  ...
  test_sizes = (/ 4, 10, 25 /)
  write(*,*) 'Testing ZTRSV (vector reverse mode)'
  all_passed = .true.
  do id = 1, 2
    if (id == 1) then
      diag = 'N'
...
contains
  ...
  subroutine run_test_for_size(n, nbdirs, diag, passed)
    ...
    complex(8) :: ab(nbdirs,n,n), xb(nbdirs,n), xb_seed(nbdirs,n)
    complex(8) :: a_dir(n,n), x_dir(n)
    ...
    call ztrsv_bv(uplo, trans, diag, nsize, a, ab, lda_val, x, xb, incx, nbdirs)
