...
program test_csyr2k_reverse
  ...
  complex(4) :: alphab
  complex(4), dimension(max_size,max_size) :: ab
  complex(4), dimension(max_size,max_size) :: bb
  complex(4) :: betab
  complex(4), dimension(max_size,max_size) :: cb
  ...
  complex(4), dimension(max_size,max_size) :: c_plus, c_minus

  ! Saved cotangents (output adjoints) for VJP verification
  complex(4), dimension(max_size,max_size) :: cb_orig
  ...
  do j = 1, max_size
    do i = 1, max_size
      call random_number(temp_real_init)
      call random_number(temp_imag_init)
      c(i,j) = cmplx(temp_real_init, temp_imag_init) * (2.0,2.0) - (1.0,1.0)
    end do
  end do
  ldc_val = ldc
  ...
  do j = 1, max_size
    do i = 1, max_size
      call random_number(temp_real_init)
      call random_number(temp_imag_init)
      cb(i,j) = cmplx(temp_real_init, temp_imag_init) * (2.0,2.0) - (1.0,1.0)
    end do
  end do
...
contains
  ...
  subroutine check_vjp_numerically()
    ...
    complex(4), dimension(max_size,max_size) :: c_central_diff
    ...
    do j = 1, max_size
      do i = 1, max_size
        call random_number(temp_real)
        call random_number(temp_imag)
        c_dir(i,j) = cmplx(temp_real, temp_imag) * (2.0,2.0) - (1.0,1.0)
      end do
    end do
