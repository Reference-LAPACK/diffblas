...
C
C  Differentiation of ctpmv in forward (tangent) mode:
...
C>
C  =====================================================================
      SUBROUTINE CTPMV_D(uplo, trans, diag, n, ap, apd, x, xd, incx)
      IMPLICIT NONE
C
          ...
          IF (incx .EQ. 1) THEN
            DO j=n,1,-1
              ...
              ELSE
                IF (nounit) THEN
                  temp0 = CONJG(ap(kk))
                  tempd = temp0*tempd + temp*CONJG(apd(kk))
                  temp = temp*temp0
                END IF
                DO i=j-1,1,-1
                  temp0 = CONJG(ap(k))
                  tempd = tempd + x(i)*CONJG(apd(k)) + temp0*xd(i)
                  temp = temp + temp0*x(i)
                  k = k - 1
                ENDDO
              END IF
            ...
            DO j=n,1,-1
              ...
              ELSE
                ...
                DO k=kk-1,kk-j+1,-1
                  ix = ix - incx
                  temp0 = CONJG(ap(k))
                  tempd = tempd + x(ix)*CONJG(apd(k)) + temp0*xd(ix)
