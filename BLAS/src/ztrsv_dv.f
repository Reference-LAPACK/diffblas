...
C
C  Differentiation of ztrsv in forward (tangent) mode (with options multiDirectional):
...
C>
C  =====================================================================
C      SUBROUTINE ZTRSV_DV(uplo, trans, diag, n, a, ad, lda, x, xd, incx
C     +                    , nbdirs)
C      IMPLICIT NONE
...
CC     .. Array Arguments ..
C      COMPLEX*16 a(lda, *), x(*)
C      COMPLEX*16 ad(nbdirsmax, lda, *), xd(nbdirsmax, *)
CC     ..
CC
CC  =====================================================================
CC     ..
...
C              ELSE
C                DO i=1,j-1
C                  temp1 = DCONJG(a(i, j))
C                  DO nd=1,nbdirs
C                    tempd(nd) = tempd(nd) - x(i)*DCONJG(ad(nd, i, j)) - 
C     +                temp1*xd(nd, i)
C                  ENDDO
C                  temp = temp - temp1*x(i)
C                ENDDO
C                IF (nounit) THEN
C                  temp1 = DCONJG(a(j, j))
C                  DO nd=1,nbdirs
C                    tempd(nd) = (tempd(nd)-temp*DCONJG(ad(nd, j, j))/
C     +                temp1)/temp1
C                  ENDDO
C                  temp = temp/temp1
C                END IF
...
C                  temp1 = DCONJG(a(i, j))
C                  DO nd=1,nbdirs
C                    tempd(nd) = tempd(nd) - x(ix)*DCONJG(ad(nd, i, j)) -
C     +                temp1*xd(nd, ix)
C                  ENDDO
C                  temp = temp - temp1*x(ix)
C                  ix = ix + incx
C                ENDDO
C                IF (nounit) THEN
C                  temp1 = DCONJG(a(j, j))
...
C                temp1 = DCONJG(a(j, j))
C                DO nd=1,nbdirs
C                  tempd(nd) = (tempd(nd)-temp*DCONJG(ad(nd, j, j))/temp1
C     +              )/temp1
C                ENDDO
C                temp = temp/temp1
C              END IF
...
C      END
      ...
      SUBROUTINE ZTRSV_DV(UPLO, TRANS, DIAG, N, A, AD, LDA, X, XD,
     +                    INCX, NBDIRS)
      IMPLICIT NONE
      ...
      COMPLEX*16 X(*), XD(NBDIRSMAX,*)

      COMPLEX*16 S(N), T(N), W(N), AD_SLICE(LDA,LDA)
      INTEGER ND, I, J, KX
     ...
     +      ' must be in 1..nbdirsmax=', NBDIRSMAX, '. Stopping.'
      ...
      DO ND = 1, NBDIRS
         ...
         DO I = 1, N
            T(I) = S(I)
         END DO
         CALL ZTRMV(UPLO, TRANS, DIAG, N, AD_SLICE, LDA, T, 1)
