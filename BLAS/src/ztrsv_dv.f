...
C>
C  =====================================================================
      SUBROUTINE ZTRSV_DV(uplo, trans, diag, n, a, ad, lda, x, xd, incx
     +                    , nbdirs)
      IMPLICIT NONE
              ...
              DO j=n,1,-1
                IF (x(jx) .NE. zero) THEN
                  IF (nounit) THEN
                    temp0 = x(jx)/a(j, j)
                    DO nd=1,nbdirs
                      xd(nd, jx) = (xd(nd, jx)-temp0*ad(nd, j, j))/a(j, 
     +                  j)
                    ENDDO
                    x(jx) = temp0
                  END IF
...
C
     ...
     +                temp1*xd(nd, i)
                ...
                IF (nounit) THEN
                  temp1 = DCONJG(a(j, j))
                  DO nd=1,nbdirs
                    tempd(nd) = (tempd(nd)-temp*DCONJG(ad(nd, j, j))/
     +                temp1)/temp1
                  ENDDO
                  temp = temp/temp1
                END IF
     ...
     +              temp1*xd(nd, i)
              ...
              IF (nounit) THEN
                temp1 = DCONJG(a(j, j))
                DO nd=1,nbdirs
                  tempd(nd) = (tempd(nd)-temp*DCONJG(ad(nd, j, j))/temp1
     +              )/temp1
                ENDDO
                temp = temp/temp1
              END IF
