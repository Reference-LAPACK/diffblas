...
C
C  Differentiation of ztrsm in forward (tangent) mode:
...
C>
C  =====================================================================
C      SUBROUTINE ZTRSM_D(side, uplo, transa, diag, m, n, alpha, alphad, 
C     +                   a, ad, lda, b, bd, ldb)
C      IMPLICIT NONE
CC
...
C                    tempd = tempd - b(k, j)*DCONJG(ad(k, i)) - temp1*bd(
C     +                k, j)
C                    temp = temp - temp1*b(k, j)
C                  ENDDO
C                  IF (nounit) THEN
C                    temp1 = DCONJG(a(i, i))
C                    tempd = (tempd-temp*DCONJG(ad(i, i))/temp1)/temp1
C                    temp = temp/temp1
C                  END IF
C                END IF
C                bd(i, j) = tempd
...
C                  temp0 = b(i, k)/temp1
C                  bd(i, k) = (bd(i, k)-temp0*DCONJG(ad(k, k)))/temp1
C                  b(i, k) = temp0
C                ENDDO
C              END IF
C            END IF
...
C      END
      ...
      SUBROUTINE ZTRSM_D(SIDE, UPLO, TRANSA, DIAG, M, N,
     +                   ALPHA, ALPHAD, A, AD, LDA, B, BD, LDB)
      IMPLICIT NONE
      CHARACTER SIDE, UPLO, TRANSA, DIAG
      INTEGER M, N, LDA, LDB
      COMPLEX*16 ALPHA, ALPHAD
      COMPLEX*16 A(LDA,*), AD(LDA,*)
      COMPLEX*16 B(LDB,*), BD(LDB,*)
