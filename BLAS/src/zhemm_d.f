...
C
C  Differentiation of zhemm in forward (tangent) mode:
...
C>
C  =====================================================================
      SUBROUTINE ZHEMM_D(side, uplo, m, n, alpha, alphad, a, ad, lda, b
            ...
            DO j=1,n
              DO i=1,m
                ...
                DO k=1,i-1
                  ...
                  temp = DCONJG(a(k, i))
                  temp2d = temp2d + temp*bd(k, j) + b(k, j)*DCONJG(ad(k
     +              , i))
                  temp2 = temp2 + b(k, j)*temp
                ENDDO
                IF (beta .EQ. zero) THEN
                  temp0 = DBLE(a(i, i))
                  cd(i, j) = temp0*temp1d + temp1*DBLE(ad(i, i)) + temp2
     +              *alphad + alpha*temp2d
                  c(i, j) = temp1*temp0 + alpha*temp2
                ELSE
                  temp0 = DBLE(a(i, i))
                  cd(i, j) = c(i, j)*betad + beta*cd(i, j) + temp0*
     +              temp1d + temp1*DBLE(ad(i, i)) + temp2*alphad + alpha
     +              *temp2d
...
C
          DO j=1,n
            temp0 = DBLE(a(j, j))
            temp1d = temp0*alphad + alpha*DBLE(ad(j, j))
            temp1 = alpha*temp0
            IF (beta .EQ. zero) THEN
              DO i=1,m
     ...
     +            temp1d + temp1*bd(i, j)
            ...
            DO k=1,j-1
              ...
              ELSE
                temp = DCONJG(a(j, k))
                temp1d = temp*alphad + alpha*DCONJG(ad(j, k))
                temp1 = alpha*temp
              END IF
              DO i=1,m
