...
C
C  Differentiation of dtrsm in forward (tangent) mode:
...
C>
C  =====================================================================
C      SUBROUTINE DTRSM_D(side, uplo, transa, diag, m, n, alpha, alphad, 
C     +                   a, ad, lda, b, bd, ldb)
C      IMPLICIT NONE
CC
...
CC     .. Scalar Arguments ..
C      DOUBLE PRECISION alpha
C      DOUBLE PRECISION alphad
C      INTEGER lda, ldb, m, n
C      CHARACTER diag, side, transa, uplo
CC     ..
CC     .. Array Arguments ..
C      DOUBLE PRECISION a(lda, *), b(ldb, *)
C      DOUBLE PRECISION ad(lda, *), bd(ldb, *)
C      EXTERNAL LSAME
CC     ..
CC
CC  =====================================================================
...
CC
C      END IF
C      END SUBROUTINE DTRSM_D

      SUBROUTINE DTRSM_D(SIDE, UPLO, TRANSA, DIAG, M, N,
...
C
      ...
      INTEGER M, N, LDA, LDB
      DOUBLE PRECISION ALPHA, ALPHAD
      DOUBLE PRECISION A(LDA,*), AD(LDA,*)
      DOUBLE PRECISION B(LDB,*), BD(LDB,*)
...
C     Step 2: T = op(Ad)*X  (SIDE='L') or X*op(Ad)  (SIDE='R')
      DO J = 1, N
         ...
         END DO
      END DO
      CALL DTRMM(SIDE, UPLO, TRANSA, DIAG, M, N, 1.0D0, AD, LDA, T, M)

C     Step 3: build RHS = alphad*B + alpha*Bd - T, in place in BD
      DO J = 1, N
         DO I = 1, M
...
C     Step 4: solve the SAME triangular system again -> dX, overwrites BD
      CALL DTRSM(SIDE, UPLO, TRANSA, DIAG, M, N, 1.0D0, A, LDA, BD, LDB)

      RETURN
      END SUBROUTINE DTRSM_D
