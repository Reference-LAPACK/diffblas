...
C
C  Differentiation of ssyr2k in forward (tangent) mode:
...
C>
C  =====================================================================
      SUBROUTINE SSYR2K_D(uplo, trans, n, k, alpha, alphad, a, ad, lda, 
     +                    b, bd, ldb, beta, betad, c, cd, ldc)
      IMPLICIT NONE
C
            ...
            DO j=1,n
              ...
              DO l=1,k
                IF (a(j, l) .NE. zero .OR. b(j, l) .NE. zero) THEN
                  temp1d = b(j, l)*alphad + alpha*bd(j, l)
                  temp1 = alpha*b(j, l)
                  temp2d = a(j, l)*alphad + alpha*ad(j, l)
                  temp2 = alpha*a(j, l)
                  DO i=1,j
                    cd(i, j) = cd(i, j) + temp1*ad(i, l) + a(i, l)*
     +                temp1d + temp2*bd(i, l) + b(i, l)*temp2d
...
C
          DO j=1,n
            DO i=1,j
              ...
              temp2d = 0.0
              DO l=1,k
                temp1d = temp1d + b(l, j)*ad(l, i) + a(l, i)*bd(l, j)
                temp1 = temp1 + a(l, i)*b(l, j)
                temp2d = temp2d + a(l, j)*bd(l, i) + b(l, i)*ad(l, j)
                temp2 = temp2 + b(l, i)*a(l, j)
              ENDDO
              IF (beta .EQ. zero) THEN
                cd(i, j) = (temp1+temp2)*alphad + alpha*temp1d + alpha*
     +            temp2d
                c(i, j) = alpha*temp1 + alpha*temp2
              ELSE
                cd(i, j) = c(i, j)*betad + beta*cd(i, j) + (temp1+temp2)
     +            *alphad + alpha*temp1d + alpha*temp2d
