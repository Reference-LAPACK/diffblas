...
C
C  Differentiation of ctbmv in forward (tangent) mode (with options multiDirectional):
...
C>
C  =====================================================================
      SUBROUTINE CTBMV_DV(uplo, trans, diag, n, k, a, ad, lda, x, xd, 
     +                    incx, nbdirs)
      IMPLICIT NONE
...
C
            ...
            IF (incx .EQ. 1) THEN
              DO j=1,n
                IF (x(j) .NE. zero) THEN
                  ...
                  DO i=max1,j-1
                    DO nd=1,nbdirs
                      xd(nd, i) = xd(nd, i) + a(l+i, j)*tempd(nd) + temp
     +                  *ad(nd, l+i, j)
                    ENDDO
                  ...
                  IF (nounit) THEN
                    DO nd=1,nbdirs
                      xd(nd, j) = a(kplus1, j)*xd(nd, j) + x(j)*ad(nd, 
     +                  kplus1, j)
                    ENDDO
                    x(j) = x(j)*a(kplus1, j)
                  END IF
            ...
            ELSE
              ...
              DO j=1,n
                IF (x(jx) .NE. zero) THEN
                  ...
                  DO i=max2,j-1
                    DO nd=1,nbdirs
                      xd(nd, ix) = xd(nd, ix) + a(l+i, j)*tempd(nd) + 
     +                  temp*ad(nd, l+i, j)
                    ENDDO
                  ...
                  IF (nounit) THEN
                    DO nd=1,nbdirs
                      xd(nd, jx) = a(kplus1, j)*xd(nd, jx) + x(jx)*ad(nd
     +                  , kplus1, j)
                    ENDDO
     ...
     +                ad(nd, l+i, j)
                ...
                IF (nounit) THEN
                  DO nd=1,nbdirs
                    xd(nd, j) = a(1, j)*xd(nd, j) + x(j)*ad(nd, 1, j)
                  ENDDO
                  x(j) = x(j)*a(1, j)
                END IF
              END IF
          ...
          ELSE
            ...
            DO j=n,1,-1
              IF (x(jx) .NE. zero) THEN
                ...
                DO i=min2,j+1,-1
                  DO nd=1,nbdirs
                    xd(nd, ix) = xd(nd, ix) + a(l+i, j)*tempd(nd) + temp
     +                *ad(nd, l+i, j)
                  ENDDO
                  x(ix) = x(ix) + temp*a(l+i, j)
                  ix = ix - incx
                ...
                IF (nounit) THEN
                  DO nd=1,nbdirs
                    xd(nd, jx) = a(1, j)*xd(nd, jx) + x(jx)*ad(nd, 1, j)
                  ENDDO
                  x(jx) = x(jx)*a(1, j)
                END IF
              END IF
...
C
          ...
          IF (incx .EQ. 1) THEN
            DO j=n,1,-1
              ...
              IF (noconj) THEN
                IF (nounit) THEN
                  DO nd=1,nbdirs
                    tempd(nd) = a(kplus1, j)*tempd(nd) + temp*ad(nd, 
     +                kplus1, j)
                  ENDDO
                ...
                DO i=j-1,max3,-1
                  DO nd=1,nbdirs
                    tempd(nd) = tempd(nd) + x(i)*ad(nd, l+i, j) + a(l+i
     +                , j)*xd(nd, i)
                  ENDDO
              ...
              ELSE
                IF (nounit) THEN
                  temp0 = CONJG(a(kplus1, j))
                  DO nd=1,nbdirs
                    tempd(nd) = temp0*tempd(nd) + temp*CONJG(ad(nd, 
     +                kplus1, j))
                  ENDDO
                  temp = temp*temp0
                END IF
                ...
                DO i=j-1,max4,-1
                  temp0 = CONJG(a(l+i, j))
                  DO nd=1,nbdirs
                    tempd(nd) = tempd(nd) + x(i)*CONJG(ad(nd, l+i, j)) +
     +                temp0*xd(nd, i)
                  ENDDO
                  temp = temp + temp0*x(i)
                ENDDO
     ...
     +                kplus1, j)
                ...
                DO i=j-1,max5,-1
                  DO nd=1,nbdirs
                    tempd(nd) = tempd(nd) + x(ix)*ad(nd, l+i, j) + a(l+i
     +                , j)*xd(nd, ix)
                  ENDDO
     ...
     +                kplus1, j))
                ...
                DO i=j-1,max6,-1
                  temp0 = CONJG(a(l+i, j))
                  DO nd=1,nbdirs
                    tempd(nd) = tempd(nd) + x(ix)*CONJG(ad(nd, l+i, j)) 
     +                + temp0*xd(nd, ix)
                  ENDDO
        ...
        ELSE IF (incx .EQ. 1) THEN
          DO j=1,n
            ...
            IF (noconj) THEN
              IF (nounit) THEN
                DO nd=1,nbdirs
                  tempd(nd) = a(1, j)*tempd(nd) + temp*ad(nd, 1, j)
                ENDDO
                temp = temp*a(1, j)
              END IF
              IF (n .GT. j + k) THEN
              ...
              DO i=j+1,min3
                DO nd=1,nbdirs
                  tempd(nd) = tempd(nd) + x(i)*ad(nd, l+i, j) + a(l+i, j
     +              )*xd(nd, i)
                ENDDO
                temp = temp + a(l+i, j)*x(i)
              ENDDO
            ELSE
              IF (nounit) THEN
                temp0 = CONJG(a(1, j))
                DO nd=1,nbdirs
                  tempd(nd) = temp0*tempd(nd) + temp*CONJG(ad(nd, 1, j))
                ENDDO
                temp = temp*temp0
              END IF
              IF (n .GT. j + k) THEN
