...
C
C  Differentiation of zsyr2k in reverse (adjoint) mode (with options multiDirectional):
...
C  ===========
C
C       SUBROUTINE ZSYR2K(UPLO,TRANS,N,K,ALPHA,A,LDA,B,LDB,BETA,C,LDC)
C
C       .. Scalar Arguments ..
...
C>
C  =====================================================================
      SUBROUTINE ZSYR2K_BV(uplo, trans, n, k, alpha, alphab, a, ab, lda
            ...
            DO j=n,1,-1
              DO l=k,1,-1
                ...
                IF (branch .EQ. 0) THEN
                  ...
                  DO i=ad_to3,1,-1
                    DO nd=1,nbdirs
                      ab(nd, i, l) = ab(nd, i, l) + CONJG(temp1)*cb(nd, 
     +                  i, j)
                      temp1b(nd) = temp1b(nd) + CONJG(a(i, l))*cb(nd, i
     +                  , j)
                      bb(nd, i, l) = bb(nd, i, l) + CONJG(temp2)*cb(nd, 
     +                  i, j)
                      temp2b(nd) = temp2b(nd) + CONJG(b(i, l))*cb(nd, i
     +                  , j)
                    ENDDO
                  ENDDO
                  ...
                  DO nd=1,nbdirs
                    alphab(nd) = alphab(nd) + CONJG(a(j, l))*temp2b(nd) 
     +                + CONJG(b(j, l))*temp1b(nd)
                    ab(nd, j, l) = ab(nd, j, l) + CONJG(alpha)*temp2b(nd
     +                )
                    bb(nd, j, l) = bb(nd, j, l) + CONJG(alpha)*temp1b(nd
     +                )
                  ENDDO
                END IF
              ENDDO
...
C
          ...
          DO j=n,1,-1
            ...
            DO i=ad_to4,1,-1
              ...
              IF (branch .EQ. 0) THEN
                DO nd=1,nbdirs
                  alphab(nd) = alphab(nd) + (CONJG(temp1)+CONJG(temp2))*
     +              cb(nd, i, j)
                  temp1b(nd) = CONJG(alpha)*cb(nd, i, j)
     ...
     +              cb(nd, i, j)
              ...
              DO l=k,1,-1
                DO nd=1,nbdirs
                  bb(nd, l, i) = bb(nd, l, i) + CONJG(a(l, j))*temp2b(nd
     +              )
                  ab(nd, l, j) = ab(nd, l, j) + CONJG(b(l, i))*temp2b(nd
     +              )
                  ab(nd, l, i) = ab(nd, l, i) + CONJG(b(l, j))*temp1b(nd
     +              )
                  bb(nd, l, j) = bb(nd, l, j) + CONJG(a(l, i))*temp1b(nd
     +              )
                ENDDO
              ENDDO
              CALL POPCOMPLEX16(temp2)
