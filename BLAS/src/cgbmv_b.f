...
C
C  Differentiation of cgbmv in reverse (adjoint) mode:
...
C>
C  =====================================================================
      SUBROUTINE CGBMV_B(trans, m, n, kl, ku, alpha, alphab, a, ab, lda
              ...
              IF (incy .EQ. 1) THEN
                ...
                DO j=n,1,-1
                  ...
                  DO i=ad_to,ad_from,-1
                    tempb = tempb + CONJG(a(k+i, j))*yb(i)
                    ab(k+i, j) = ab(k+i, j) + CONJG(temp)*yb(i)
                  ENDDO
                  alphab = alphab + CONJG(x(jx))*tempb
                  xb(jx) = xb(jx) + CONJG(alpha)*tempb
                ENDDO
              ELSE
                DO j=1,n
                  iy = ky
                ...
                DO j=n,1,-1
                  ...
                  DO i=ad_to0,ad_from0,-1
                    CALL POPINTEGER4(iy)
                    tempb = tempb + CONJG(a(k+i, j))*yb(iy)
                    ab(k+i, j) = ab(k+i, j) + CONJG(temp)*yb(iy)
                  ENDDO
                  alphab = alphab + CONJG(x(jx))*tempb
                  xb(jx) = xb(jx) + CONJG(alpha)*tempb
                ENDDO
...
C
              ...
              IF (incx .EQ. 1) THEN
                ...
                DO j=n,1,-1
                  CALL POPINTEGER4(jy)
                  alphab = alphab + CONJG(temp)*yb(jy)
                  tempb = CONJG(alpha)*yb(jy)
                  k = kup1 - j
                  CALL POPCONTROL1B(branch)
                  IF (branch .EQ. 0) THEN
                    ...
                    DO i=ad_to1,ad_from1,-1
                      ab(k+i, j) = ab(k+i, j) + CONJG(x(i))*tempb
                      xb(i) = xb(i) + CONJG(a(k+i, j))*tempb
                    ENDDO
                  ELSE
                    CALL POPINTEGER4(ad_from2)
                    CALL POPINTEGER4(ad_to2)
                    DO i=ad_to2,ad_from2,-1
                      ab(k+i, j) = ab(k+i, j) + CONJG(CONJG(x(i))*tempb)
                      xb(i) = xb(i) + CONJG(CONJG(a(k+i, j)))*tempb
                    ENDDO
                  END IF
                ...
                DO j=n,1,-1
                  ...
                  IF (branch .EQ. 0) THEN
                    ...
                    DO i=ad_to3,ad_from3,-1
                      CALL POPINTEGER4(ix)
                      ab(k+i, j) = ab(k+i, j) + CONJG(x(ix))*tempb
                      xb(ix) = xb(ix) + CONJG(a(k+i, j))*tempb
                    ENDDO
                  ELSE
                    ...
                    DO i=ad_to4,ad_from4,-1
                      CALL POPINTEGER4(ix)
                      ab(k+i, j) = ab(k+i, j) + CONJG(CONJG(x(ix))*tempb
     +                  )
                      xb(ix) = xb(ix) + CONJG(CONJG(a(k+i, j)))*tempb
                    ENDDO
                  END IF
                  CALL POPCOMPLEX8(temp)
                ENDDO
