...
C>
C  =====================================================================
C      SUBROUTINE CTRSM_DV(side, uplo, transa, diag, m, n, alpha, alphad
...
CC     .. Scalar Arguments ..
C      COMPLEX alpha
C      COMPLEX alphad(nbdirsmax)
C      INTEGER lda, ldb, m, n
C      CHARACTER diag, side, transa, uplo
CC     ..
CC     .. Array Arguments ..
C      COMPLEX a(lda, *), b(ldb, *)
C      COMPLEX ad(nbdirsmax, lda, *), bd(nbdirsmax, ldb, *)
C      EXTERNAL LSAME
CC     ..
CC
CC  =====================================================================
...
C                    temp0 = CONJG(a(k, i))
C                    DO nd=1,nbdirs
C                      tempd(nd) = tempd(nd) - b(k, j)*CONJG(ad(nd, k, i)
C     +                  ) - temp0*bd(nd, k, j)
C                    ENDDO
C                    temp = temp - temp0*b(k, j)
C                  ENDDO
C                  IF (nounit) THEN
C                    temp0 = CONJG(a(i, i))
C                    DO nd=1,nbdirs
C                      tempd(nd) = (tempd(nd)-temp*CONJG(ad(nd, i, i))/
C     +                  temp0)/temp0
C                    ENDDO
C                    temp = temp/temp0
C                  END IF
...
C                  temp1 = b(i, k)/temp0
C                  DO nd=1,nbdirs
C                    bd(nd, i, k) = (bd(nd, i, k)-temp1*CONJG(ad(nd, k, k
C     +                )))/temp0
C                  ENDDO
C                  b(i, k) = temp1
C                ENDDO
...
C              IF (noconj) THEN
C                DO nd=1,nbdirs
C                  tempd(nd) = ad(nd, j, k)
C                ENDDO
C                temp = a(j, k)
C              ELSE
C                DO nd=1,nbdirs
C                  tempd(nd) = CONJG(ad(nd, j, k))
C                ENDDO
C                temp = CONJG(a(j, k))
C              END IF
C              DO i=1,m
C                DO nd=1,nbdirs
C                  bd(nd, i, j) = bd(nd, i, j) - b(i, k)*tempd(nd) - temp
C     +              *bd(nd, i, k)
C                ENDDO
...
C                  temp1 = b(i, k)/a(k, k)
C                  DO nd=1,nbdirs
C                    bd(nd, i, k) = (bd(nd, i, k)-temp1*ad(nd, k, k))/a(k
C     +                , k)
C                  ENDDO
C                  b(i, k) = temp1
C                ENDDO
...
C                  temp0 = b(i, k)/temp1
C                  DO nd=1,nbdirs
C                    bd(nd, i, k) = (bd(nd, i, k)-temp0*CONJG(ad(nd, k, k
C     +                )))/temp1
C                  ENDDO
C                  b(i, k) = temp0
C                ENDDO
...
!
      ...
      COMPLEX A(LDA,*), AD(NBDIRSMAX,LDA,*)
      COMPLEX B(LDB,*), BD(NBDIRSMAX,LDB,*)
     ...
     +      ' must be in 1..nbdirsmax=', NBDIRSMAX, '. Stopping.'
      ...
      DO ND = 1, NBDIRS
         ...
         DO J = 1, N
            ...
            END DO
         END DO
         CALL CTRMM(SIDE, UPLO, TRANSA, DIAG, M, N, (1.0E0,0.0E0),
     +              AD_SLICE, LDA, T, M)
