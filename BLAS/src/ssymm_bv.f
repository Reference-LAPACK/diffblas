...
C
C  Differentiation of ssymm in reverse (adjoint) mode (with options multiDirectional):
...
C  ===========
C
C       SUBROUTINE SSYMM(SIDE,UPLO,M,N,ALPHA,A,LDA,B,LDB,BETA,C,LDC)
C
C       .. Scalar Arguments ..
C       REAL ALPHA,BETA
...
C>
C  =====================================================================
      SUBROUTINE SSYMM_BV(side, uplo, m, n, alpha, alphab, a, ab, lda, b
          ...
          IF (upper) THEN
            DO j=1,n
              DO i=1,m
                ...
                temp2 = zero
                DO k=1,i-1
                  CALL PUSHREAL4(c(k, j))
                  c(k, j) = c(k, j) + temp1*a(k, i)
                  temp2 = temp2 + b(k, j)*a(k, i)
                ENDDO
                CALL PUSHINTEGER4(k - 1)
                IF (beta .EQ. zero) THEN
                  CALL PUSHREAL4(c(i, j))
            ...
            DO j=n,1,-1
              DO i=m,1,-1
                ...
                IF (branch .EQ. 0) THEN
                  ...
                  CALL POPREAL4(c(i, j))
                  DO nd=1,nbdirs
                    temp1b(nd) = a(i, i)*cb(nd, i, j)
                    ab(nd, i, i) = ab(nd, i, i) + temp1*cb(nd, i, j)
                    alphab(nd) = alphab(nd) + temp2*cb(nd, i, j)
                ...
                DO k=ad_to,1,-1
                  DO nd=1,nbdirs
                    bb(nd, k, j) = bb(nd, k, j) + a(k, i)*temp2b(nd)
                    ab(nd, k, i) = ab(nd, k, i) + b(k, j)*temp2b(nd) + 
     +                temp1*cb(nd, k, j)
                    temp1b(nd) = temp1b(nd) + a(k, i)*cb(nd, k, j)
                  ENDDO
                  CALL POPREAL4(c(k, j))
                ENDDO
                CALL POPREAL4(temp2)
                DO nd=1,nbdirs
                  alphab(nd) = alphab(nd) + b(i, j)*temp1b(nd)
                  bb(nd, i, j) = bb(nd, i, j) + alpha*temp1b(nd)
                ENDDO
              ENDDO
            ENDDO
          ELSE
...
C
          ...
          DO j=n,1,-1
            ...
            DO k=n,ad_from0,-1
              ...
              DO i=m,1,-1
                DO nd=1,nbdirs
                  temp1b(nd) = temp1b(nd) + b(i, k)*cb(nd, i, j)
                  bb(nd, i, k) = bb(nd, i, k) + temp1*cb(nd, i, j)
                ENDDO
              ENDDO
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                CALL POPREAL4(temp1)
                DO nd=1,nbdirs
                  alphab(nd) = alphab(nd) + a(k, j)*temp1b(nd)
                  ab(nd, k, j) = ab(nd, k, j) + alpha*temp1b(nd)
                ENDDO
              ELSE
                CALL POPREAL4(temp1)
                DO nd=1,nbdirs
                  alphab(nd) = alphab(nd) + a(j, k)*temp1b(nd)
                  ab(nd, j, k) = ab(nd, j, k) + alpha*temp1b(nd)
                ENDDO
              END IF
            ENDDO
            ...
            IF (branch .EQ. 0) THEN
              ...
              DO i=m,1,-1
                DO nd=1,nbdirs
                  betab(nd) = betab(nd) + c(i, j)*cb(nd, i, j)
                  temp1b(nd) = temp1b(nd) + b(i, j)*cb(nd, i, j)
                  bb(nd, i, j) = bb(nd, i, j) + temp1*cb(nd, i, j)
                  cb(nd, i, j) = beta*cb(nd, i, j)
                ENDDO
              ENDDO
            ELSE
            ...
            CALL POPREAL4(temp1)
            DO nd=1,nbdirs
              alphab(nd) = alphab(nd) + a(j, j)*temp1b(nd)
              ab(nd, j, j) = ab(nd, j, j) + alpha*temp1b(nd)
            ENDDO
          ENDDO
        END IF
      END IF
