...
C
C  Differentiation of ctpmv in forward (tangent) mode (with options multiDirectional):
...
C>
C  =====================================================================
      SUBROUTINE CTPMV_DV(uplo, trans, diag, n, ap, apd, x, xd, incx, 
     +                    nbdirs)
      IMPLICIT NONE
...
C     .. Array Arguments ..
      COMPLEX ap(*), x(*)
      COMPLEX apd(nbdirsmax, *), xd(nbdirsmax, *)
C     ..
C
C  =====================================================================
C
...
C
          ...
          IF (incx .EQ. 1) THEN
            DO j=n,1,-1
              ...
              IF (noconj) THEN
                ...
                DO i=j-1,1,-1
                  DO nd=1,nbdirs
                    tempd(nd) = tempd(nd) + x(i)*apd(nd, k) + ap(k)*xd(
     +                nd, i)
                  ENDDO
                  temp = temp + ap(k)*x(i)
                  k = k - 1
              ...
              ELSE
                IF (nounit) THEN
                  temp0 = CONJG(ap(kk))
                  DO nd=1,nbdirs
                    tempd(nd) = temp0*tempd(nd) + temp*CONJG(apd(nd, kk)
     +                )
                  ENDDO
                  temp = temp*temp0
                END IF
                DO i=j-1,1,-1
                  temp0 = CONJG(ap(k))
                  DO nd=1,nbdirs
                    tempd(nd) = tempd(nd) + x(i)*CONJG(apd(nd, k)) + 
     +                temp0*xd(nd, i)
                  ENDDO
                  temp = temp + temp0*x(i)
                  k = k - 1
            ...
            DO j=n,1,-1
              ...
              IF (noconj) THEN
                ...
                DO k=kk-1,kk-j+1,-1
                  ix = ix - incx
                  DO nd=1,nbdirs
                    tempd(nd) = tempd(nd) + x(ix)*apd(nd, k) + ap(k)*xd(
     +                nd, ix)
                  ENDDO
                  temp = temp + ap(k)*x(ix)
                ENDDO
     ...
     +                )
                ...
                DO k=kk-1,kk-j+1,-1
                  ...
                  temp0 = CONJG(ap(k))
                  DO nd=1,nbdirs
                    tempd(nd) = tempd(nd) + x(ix)*CONJG(apd(nd, k)) + 
     +                temp0*xd(nd, ix)
                  ENDDO
