...
C
C  Differentiation of dger in reverse (adjoint) mode (with options multiDirectional):
...
C>
C  =====================================================================
      SUBROUTINE DGER_BV(m, n, alpha, alphab, x, xb, incx, y, yb, incy, 
     +                   a, ab, lda, nbdirs)
      IMPLICIT NONE
          ...
          IF (incx .EQ. 1) THEN
            ...
            DO j=n,1,-1
              ...
              IF (branch .EQ. 0) THEN
                ...
                DO i=m,1,-1
                  DO nd=1,nbdirs
                    xb(nd, i) = xb(nd, i) + temp*ab(nd, i, j)
                    tempb(nd) = tempb(nd) + x(i)*ab(nd, i, j)
                  ENDDO
                ENDDO
                CALL POPREAL8(temp)
                DO nd=1,nbdirs
                  alphab(nd) = alphab(nd) + y(jy)*tempb(nd)
                  yb(nd, jy) = yb(nd, jy) + alpha*tempb(nd)
                ENDDO
              END IF
            ENDDO
          ELSE
            ...
            DO j=n,1,-1
              ...
              IF (branch .EQ. 0) THEN
                ...
                DO i=m,1,-1
                  CALL POPINTEGER4(ix)
                  DO nd=1,nbdirs
                    xb(nd, ix) = xb(nd, ix) + temp*ab(nd, i, j)
                    tempb(nd) = tempb(nd) + x(ix)*ab(nd, i, j)
                  ENDDO
                ENDDO
                CALL POPREAL8(temp)
                DO nd=1,nbdirs
