...
C
C  Differentiation of ctpmv in reverse (adjoint) mode (with options multiDirectional):
...
C  ===========
C
C       SUBROUTINE CTPMV(UPLO,TRANS,DIAG,N,AP,X,INCX)
C
C       .. Scalar Arguments ..
C       INTEGER INCX,N
...
C> \param[in] AP
C> \verbatim
C>          AP is COMPLEX array, dimension at least
C>           ( ( n*( n + 1 ) )/2 ).
...
C>
C  =====================================================================
      SUBROUTINE CTPMV_BV(uplo, trans, diag, n, ap, apb, x, xb, incx, 
     +                    nbdirs)
      IMPLICIT NONE
              ...
              DO j=1,n,1
                ...
                ELSE
                  ...
                  DO i=1,ad_from2,1
                    CALL POPINTEGER4(k)
                    DO nd=1,nbdirs
                      apb(nd, k) = apb(nd, k) + CONJG(CONJG(x(i))*tempb(
     +                  nd))
                      xb(nd, i) = xb(nd, i) + CONJG(CONJG(ap(k)))*tempb(
     +                  nd)
                    ENDDO
                  ENDDO
                  CALL POPCONTROL1B(branch)
                  IF (branch .NE. 0) THEN
                    CALL POPCOMPLEX8(temp)
                    DO nd=1,nbdirs
                      apb(nd, kk) = apb(nd, kk) + CONJG(CONJG(temp)*
     +                  tempb(nd))
                      tempb(nd) = CONJG(CONJG(ap(kk)))*tempb(nd)
                    ENDDO
                  END IF
                END IF
                DO nd=1,nbdirs
              ...
              DO j=1,n,1
                ...
                ELSE
                  ...
                  DO k=ad_to4,ad_from4,1
                    DO nd=1,nbdirs
                      apb(nd, k) = apb(nd, k) + CONJG(CONJG(x(ix))*tempb
     +                  (nd))
                      xb(nd, ix) = xb(nd, ix) + CONJG(CONJG(ap(k)))*
     +                  tempb(nd)
                    ENDDO
                    CALL POPINTEGER4(ix)
                  ENDDO
