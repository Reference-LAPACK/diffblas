...
C
C  Differentiation of ztbmv in forward (tangent) mode (with options multiDirectional):
...
C>
C  =====================================================================
      SUBROUTINE ZTBMV_DV(uplo, trans, diag, n, k, a, ad, lda, x, xd, 
     +                    incx, nbdirs)
      IMPLICIT NONE
...
C     .. Array Arguments ..
      COMPLEX*16 a(lda, *), x(*)
      COMPLEX*16 ad(nbdirsmax, lda, *), xd(nbdirsmax, *)
C     ..
C
C  =====================================================================
C
...
C
     ...
     +                , j)*xd(nd, i)
              ...
              ELSE
                IF (nounit) THEN
                  temp0 = DCONJG(a(kplus1, j))
                  DO nd=1,nbdirs
                    tempd(nd) = temp0*tempd(nd) + temp*DCONJG(ad(nd, 
     +                kplus1, j))
                  ENDDO
                  temp = temp*temp0
                END IF
                ...
                END IF
                DO i=j-1,max4,-1
                  temp0 = DCONJG(a(l+i, j))
                  DO nd=1,nbdirs
                    tempd(nd) = tempd(nd) + x(i)*DCONJG(ad(nd, l+i, j)) 
     +                + temp0*xd(nd, i)
                  ENDDO
                  temp = temp + temp0*x(i)
                ENDDO
     ...
     +                kplus1, j))
                ...
                DO i=j-1,max6,-1
                  temp0 = DCONJG(a(l+i, j))
                  DO nd=1,nbdirs
                    tempd(nd) = tempd(nd) + x(ix)*DCONJG(ad(nd, l+i, j))
     +                + temp0*xd(nd, ix)
                  ENDDO
     ...
     +              )*xd(nd, i)
            ...
            ELSE
              IF (nounit) THEN
                temp0 = DCONJG(a(1, j))
                DO nd=1,nbdirs
                  tempd(nd) = temp0*tempd(nd) + temp*DCONJG(ad(nd, 1, j)
     +              )
                ENDDO
                temp = temp*temp0
              END IF
