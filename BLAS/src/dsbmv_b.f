...
C
C  Differentiation of dsbmv in reverse (adjoint) mode:
...
C>
C  =====================================================================
      SUBROUTINE DSBMV_B(uplo, n, k, alpha, alphab, a, ab, lda, x, xb, 
...
C
            ...
            IF (incx .EQ. 1 .AND. incy .EQ. 1) THEN
              ...
              DO j=n,1,-1
                temp1 = alpha*x(j)
                temp1b = a(kplus1, j)*yb(j)
                ab(kplus1, j) = ab(kplus1, j) + temp1*yb(j)
                ...
                DO i=ad_to,ad_from,-1
                  ab(l+i, j) = ab(l+i, j) + x(i)*temp2b + temp1*yb(i)
                  xb(i) = xb(i) + a(l+i, j)*temp2b
                  temp1b = temp1b + a(l+i, j)*yb(i)
                ENDDO
                CALL POPREAL8(temp2)
            ...
            ELSE
              ...
              DO j=n,1,-1
                ...
                CALL POPINTEGER4(jx)
                temp1 = alpha*x(jx)
                temp1b = a(kplus1, j)*yb(jy)
                ab(kplus1, j) = ab(kplus1, j) + temp1*yb(jy)
                ...
                DO i=ad_to0,ad_from0,-1
                  CALL POPINTEGER4(iy)
                  CALL POPINTEGER4(ix)
                  ab(l+i, j) = ab(l+i, j) + x(ix)*temp2b + temp1*yb(iy)
                  xb(ix) = xb(ix) + a(l+i, j)*temp2b
                  temp1b = temp1b + a(l+i, j)*yb(iy)
                ENDDO
                CALL POPREAL8(temp2)
...
C
            ...
            DO j=n,1,-1
              ...
              DO i=ad_to1,ad_from1,-1
                ...
                temp1b = temp1b + a(l+i, j)*yb(i)
              ENDDO
              temp1b = temp1b + a(1, j)*yb(j)
              ab(1, j) = ab(1, j) + temp1*yb(j)
              CALL POPREAL8(temp2)
              alphab = alphab + x(j)*temp1b
              xb(j) = xb(j) + alpha*temp1b
            ENDDO
          ELSE
            ...
            DO j=n,1,-1
              ...
              CALL POPINTEGER4(iy)
              CALL POPINTEGER4(ix)
              temp1b = temp1b + a(1, j)*yb(jy)
              ab(1, j) = ab(1, j) + temp1*yb(jy)
              CALL POPREAL8(temp2)
              alphab = alphab + x(jx)*temp1b
              xb(jx) = xb(jx) + alpha*temp1b
            ENDDO
