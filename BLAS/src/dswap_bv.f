...
C
C  Differentiation of dswap in reverse (adjoint) mode (with options multiDirectional):
...
C>
C  =====================================================================
      SUBROUTINE DSWAP_BV(n, dx, dxb, incx, dy, dyb, incy, nbdirs)
      IMPLICIT NONE
...
C
          ...
          DO i=n-MOD(n-mp1, 3),mp1,-3
            DO nd=1,nbdirs
              dtempb(nd) = dyb(nd, i+2)
              dyb(nd, i+2) = dxb(nd, i+2)
              dxb(nd, i+2) = dtempb(nd)
              dtempb(nd) = dyb(nd, i+1)
              dyb(nd, i+1) = dxb(nd, i+1)
              dxb(nd, i+1) = dtempb(nd)
              dtempb(nd) = dyb(nd, i)
              dyb(nd, i) = dxb(nd, i)
              dxb(nd, i) = dtempb(nd)
            ENDDO
