...
C
C  Differentiation of ztrsv in reverse (adjoint) mode (with options multiDirectional):
...
C>
C  =====================================================================
C      SUBROUTINE ZTRSV_BV(uplo, trans, diag, n, a, ab, lda, x, xb, incx
C     +                    , nbdirs)
C      IMPLICIT NONE
...
CC     .. Array Arguments ..
C      COMPLEX*16 a(lda, *), x(*)
C      COMPLEX*16 ab(nbdirsmax, lda, *), xb(nbdirsmax, *)
CC     ..
CC
CC  =====================================================================
CC     ..
...
C                    CALL POPCOMPLEX16(temp)
C                    temp0 = DCONJG(a(j, j))
C                    DO nd=1,nbdirs
C                      ab(nd, j, j) = ab(nd, j, j) + DCONJG(CONJG(-(temp/
C     +                  temp0**2))*tempb(nd))
...
C                  DO i=ad_to0,1,-1
C                    DO nd=1,nbdirs
C                      ab(nd, i, j) = ab(nd, i, j) + DCONJG(CONJG(-x(i))*
C     +                  tempb(nd))
C                      xb(nd, i) = xb(nd, i) + CONJG(-DCONJG(a(i, j)))*
C     +                  tempb(nd)
C                    ENDDO
C                  ENDDO
C                END IF
...
C                    CALL POPINTEGER4(ix)
C                    DO nd=1,nbdirs
C                      ab(nd, i, j) = ab(nd, i, j) + DCONJG(CONJG(-x(ix))
C     +                  *tempb(nd))
C                      xb(nd, ix) = xb(nd, ix) + CONJG(-DCONJG(a(i, j)))*
C     +                  tempb(nd)
C                    ENDDO
C                  ENDDO
C                END IF
...
C      END
      ...
      SUBROUTINE ZTRSV_BV(UPLO, TRANS, DIAG, N, A, AB, LDA, X, XB,
     +                    INCX, NBDIRS)
      IMPLICIT NONE
      ...
      COMPLEX*16 X(*), XB(NBDIRSMAX,*)

      COMPLEX*16 BLOC(N), S(N), YB(N), AB_SLICE(LDA,LDA)
            ...
            DO I = 1, N
               IF ((UPPER.AND.I.LE.J).OR.(.NOT.UPPER.AND.I.GE.J)) THEN
                  IF (.NOT.(UNIT.AND.I.EQ.J)) THEN
                     IF (ISN) THEN
                        AB_SLICE(I,J) = -YB(I)*DCONJG(S(J))
                     ELSE IF (IST) THEN
                        AB_SLICE(I,J) = -YB(J)*DCONJG(S(I))
                     ELSE
                        AB_SLICE(I,J) = -DCONJG(YB(J))*S(I)
                     END IF
                  END IF
               END IF
            END DO
