...
C
C  Differentiation of ztpmv in reverse (adjoint) mode:
...
C>
C  =====================================================================
      SUBROUTINE ZTPMV_B(uplo, trans, diag, n, ap, apb, x, xb, incx)
      IMPLICIT NONE
            ...
            IF (incx .EQ. 1) THEN
              ...
              DO j=1,n,1
                ...
                ELSE
                  ...
                  DO i=1,ad_from2,1
                    CALL POPINTEGER4(k)
                    apb(k) = apb(k) + DCONJG(CONJG(x(i))*tempb)
                    xb(i) = xb(i) + CONJG(DCONJG(ap(k)))*tempb
                  ENDDO
                  CALL POPCONTROL1B(branch)
                  IF (branch .NE. 0) THEN
                    CALL POPCOMPLEX16(temp)
                    apb(kk) = apb(kk) + DCONJG(CONJG(temp)*tempb)
                    tempb = CONJG(DCONJG(ap(kk)))*tempb
                  END IF
                END IF
                xb(j) = xb(j) + tempb
              ENDDO
              ...
              DO j=1,n,1
                ...
                ELSE
                  ...
                  DO k=ad_to4,ad_from4,1
                    apb(k) = apb(k) + DCONJG(CONJG(x(ix))*tempb)
                    xb(ix) = xb(ix) + CONJG(DCONJG(ap(k)))*tempb
                    CALL POPINTEGER4(ix)
                  ENDDO
                  CALL POPCONTROL1B(branch)
                  IF (branch .NE. 0) THEN
