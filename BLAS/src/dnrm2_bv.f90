...
!
!  Differentiation of dnrm2 in reverse (adjoint) mode (with options multiDirectional):
...
!>
!  =====================================================================
SUBROUTINE DNRM2_BV(n, x, xb, incx, dnrm2b, nbdirs)
  USE DIFFSIZES
!  Hint: nbdirsmax should be the maximum number of differentiation directions
  ...
  REAL(wp) :: dnrm2
  REAL(wp), DIMENSION(nbdirsmax) :: dnrm2b
!
...
!
  IF (n .LE. 0) THEN
    xb(1:nbdirsmax, 1:1+(n-1)*abs(incx)) = 0.0_8
  ELSE
!
!
...
!
    ...
    DO nd=1,nbdirs
      IF (sumsq .EQ. 0.0) THEN
        sumsqb(nd) = 0.0_8
      ELSE
        sumsqb(nd) = scl*dnrm2b(nd)/(2.0*SQRT(sumsq))
      END IF
    END DO
    ...
    END IF
    abigb = 0.0_8
 100 xb(1:nbdirsmax, 1:1+(n-1)*abs(incx)) = 0.0_8
    DO i=n,1,-1
      CALL POPINTEGER4(ix)
      CALL POPCONTROL2B(branch)
      IF (branch .LT. 2) THEN
