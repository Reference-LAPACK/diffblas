...
C
C  Differentiation of dtrsm in reverse (adjoint) mode:
...
C>
C  =====================================================================
C      SUBROUTINE DTRSM_B(side, uplo, transa, diag, m, n, alpha, alphab, 
C     +                   a, ab, lda, b, bb, ldb)
C      IMPLICIT NONE
...
CC     .. Scalar Arguments ..
C      DOUBLE PRECISION alpha
C      DOUBLE PRECISION alphab
C      INTEGER lda, ldb, m, n
C      CHARACTER diag, side, transa, uplo
CC     ..
CC     .. Array Arguments ..
C      DOUBLE PRECISION a(lda, *), b(ldb, *)
C      DOUBLE PRECISION ab(lda, *), bb(ldb, *)
C      EXTERNAL LSAME
CC     ..
CC
CC  =====================================================================
...
C      DOUBLE PRECISION tmpb
C      DOUBLE PRECISION tmp0
C      DOUBLE PRECISION tmpb0
C      DOUBLE PRECISION tmp1
C      DOUBLE PRECISION tmpb1
C      DOUBLE PRECISION tmp2
C      DOUBLE PRECISION tmpb2
C      DOUBLE PRECISION tmp3
C      DOUBLE PRECISION tmpb3
C      DOUBLE PRECISION tmp4
C      DOUBLE PRECISION tmpb4
C      INTEGER ad_to
C      INTEGER*4 branch
C      INTEGER ad_from
C      INTEGER ad_to0
...
C      END
      ...
      SUBROUTINE DTRSM_B(SIDE, UPLO, TRANSA, DIAG, M, N,
     +                   ALPHA, ALPHAB, A, AB, LDA, B, BB, LDB)
C
C Reverse-mode (adjoint) derivative of DTRSM via the Giles/black-box
C method: differentiates op(A)*X = alpha*B directly, using only the
C original, undifferentiated DTRSM -- never the substitution algorithm.
C
C CONVENTION (matches Tapenade's own generated dtrsm_b.f):
...
C
      ...
      INTEGER M, N, LDA, LDB
      DOUBLE PRECISION ALPHA, ALPHAB
      DOUBLE PRECISION A(LDA,*), AB(LDA,*)
      DOUBLE PRECISION B(LDB,*), BB(LDB,*)
...
C             Overwrites BB in place: held the seed on entry, holds Yb after.
      CALL DTRSM(SIDE, UPLO, TRANSA_T, DIAG, M, N, 1.0D0, A, LDA, BB, LDB)
...
C     Step 4: Ab, restricted to the triangle DTRSM actually reads
      DO J = 1, LDA
         DO I = 1, LDA
            AB(I,J) = 0.0D0
         END DO
