...
C>
C  =====================================================================
C      SUBROUTINE ZTRSV_D(uplo, trans, diag, n, a, ad, lda, x, xd, incx)
C      IMPLICIT NONE
CC
...
C                  tempd = tempd - x(i)*DCONJG(ad(i, j)) - temp1*xd(i)
C                  temp = temp - temp1*x(i)
C                ENDDO
C                IF (nounit) THEN
C                  temp1 = DCONJG(a(j, j))
C                  tempd = (tempd-temp*DCONJG(ad(j, j))/temp1)/temp1
C                  temp = temp/temp1
C                END IF
C              END IF
C              xd(j) = tempd
...
C                DO i=1,j-1
C                  temp1 = DCONJG(a(i, j))
C                  tempd = tempd - x(ix)*DCONJG(ad(i, j)) - temp1*xd(ix)
...
C      END
      ...
      DO I = 1, N
         W(I) = S(I)
      END DO
      CALL ZTRMV(UPLO, TRANS, DIAG, N, AD, LDA, W, 1)
