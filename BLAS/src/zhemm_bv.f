...
C
C  Differentiation of zhemm in reverse (adjoint) mode (with options multiDirectional):
...
C>
C  =====================================================================
      SUBROUTINE ZHEMM_BV(side, uplo, m, n, alpha, alphab, a, ab, lda, b
     +                    , bb, ldb, beta, betab, c, cb, ldc, nbdirs)
      IMPLICIT NONE
...
C     .. Array Arguments ..
      ...
      COMPLEX*16 ab(nbdirs, lda, *), bb(nbdirs, ldb, *), cb(
     +           nbdirs, ldc, *)
      EXTERNAL LSAME
C     ..
C
...
C
          IF (upper) THEN
            DO j=1,n
              DO i=1,m
                ...
                temp2 = zero
                DO k=1,i-1
                  CALL PUSHCOMPLEX16(c(k, j))
                  c(k, j) = c(k, j) + temp1*a(k, i)
                  temp2 = temp2 + b(k, j)*DCONJG(a(k, i))
                ENDDO
                CALL PUSHINTEGER4(k - 1)
                IF (beta .EQ. zero) THEN
                  CALL PUSHCOMPLEX16(c(i, j))
                  c(i, j) = temp1*DBLE(a(i, i)) + alpha*temp2
                  CALL PUSHCONTROL1B(0)
                ELSE
                  CALL PUSHCOMPLEX16(c(i, j))
                  c(i, j) = beta*c(i, j) + temp1*DBLE(a(i, i)) + alpha*
     +              temp2
            ...
            DO j=n,1,-1
              DO i=m,1,-1
                ...
                IF (branch .EQ. 0) THEN
                  temp1 = alpha*b(i, j)
                  CALL POPCOMPLEX16(c(i, j))
                  DO nd=1,nbdirs
                    temp1b(nd) = DBLE(a(i, i))*cb(nd, i, j)
                    ab(nd, i, i) = ab(nd, i, i) + CONJG(temp1)*cb(nd, i
     +                , j)
                    alphab(nd) = alphab(nd) + CONJG(temp2)*cb(nd, i, j)
                    temp2b(nd) = CONJG(alpha)*cb(nd, i, j)
                    cb(nd, i, j) = (0.0,0.0)
                  ENDDO
                ELSE
                  temp1 = alpha*b(i, j)
     ...
     +                temp2b(nd)) + CONJG(temp1)*cb(nd, k, j)
                    temp1b(nd) = temp1b(nd) + CONJG(a(k, i))*cb(nd, k, j
     +                )
                  ENDDO
                  CALL POPCOMPLEX16(c(k, j))
                ENDDO
                CALL POPCOMPLEX16(temp2)
                DO nd=1,nbdirs
                  alphab(nd) = alphab(nd) + CONJG(b(i, j))*temp1b(nd)
                  bb(nd, i, j) = bb(nd, i, j) + CONJG(alpha)*temp1b(nd)
                ENDDO
              ENDDO
            ENDDO
          ELSE
...
C
          ...
          DO j=n,1,-1
            ...
            DO k=n,ad_from0,-1
              ...
              DO i=m,1,-1
                DO nd=1,nbdirs
                  temp1b(nd) = temp1b(nd) + CONJG(b(i, k))*cb(nd, i, j)
                  bb(nd, i, k) = bb(nd, i, k) + CONJG(temp1)*cb(nd, i, j
     +              )
                ENDDO
              ENDDO
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                CALL POPCOMPLEX16(temp1)
                DO nd=1,nbdirs
                  alphab(nd) = alphab(nd) + CONJG(a(k, j))*temp1b(nd)
                  ab(nd, k, j) = ab(nd, k, j) + CONJG(alpha)*temp1b(nd)
                ENDDO
              ELSE
                CALL POPCOMPLEX16(temp1)
                DO nd=1,nbdirs
                  alphab(nd) = alphab(nd) + CONJG(DCONJG(a(j, k)))*
     +              temp1b(nd)
                  ab(nd, j, k) = ab(nd, j, k) + DCONJG(CONJG(alpha)*
     +              temp1b(nd))
                ENDDO
              END IF
            ENDDO
     ...
     +              temp1b(nd))
            ...
            IF (branch .EQ. 0) THEN
              ...
              DO i=m,1,-1
                DO nd=1,nbdirs
                  betab(nd) = betab(nd) + CONJG(c(i, j))*cb(nd, i, j)
                  temp1b(nd) = temp1b(nd) + CONJG(b(i, j))*cb(nd, i, j)
                  bb(nd, i, j) = bb(nd, i, j) + CONJG(temp1)*cb(nd, i, j
     +              )
                  cb(nd, i, j) = CONJG(beta)*cb(nd, i, j)
                ENDDO
              ENDDO
     ...
     +              )
            ...
            CALL POPCOMPLEX16(temp1)
            DO nd=1,nbdirs
              alphab(nd) = alphab(nd) + DBLE(a(j, j))*temp1b(nd)
              ab(nd, j, j) = ab(nd, j, j) + CONJG(alpha)*temp1b(nd)
            ENDDO
          ENDDO
        END IF
      END IF
