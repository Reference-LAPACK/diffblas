...
C
C  Differentiation of csyrk in reverse (adjoint) mode (with options multiDirectional):
...
C>
C  =====================================================================
      SUBROUTINE CSYRK_BV(uplo, trans, n, k, alpha, alphab, a, ab, lda, 
...
C     .. Array Arguments ..
      COMPLEX a(lda, *), c(ldc, *)
      COMPLEX ab(nbdirs, lda, *), cb(nbdirs, ldc, *)
      EXTERNAL LSAME
C     ..
C
C  =====================================================================
...
C
     ...
     +                  , j)
                  ...
                  DO nd=1,nbdirs
                    alphab(nd) = alphab(nd) + CONJG(a(j, l))*tempb(nd)
                    ab(nd, j, l) = ab(nd, j, l) + CONJG(alpha)*tempb(nd)
                  ENDDO
                END IF
              ENDDO
...
C
          ...
          DO j=n,1,-1
            ...
            DO i=ad_to4,1,-1
              ...
              DO l=k,1,-1
                DO nd=1,nbdirs
                  ab(nd, l, i) = ab(nd, l, i) + CONJG(a(l, j))*tempb(nd)
                  ab(nd, l, j) = ab(nd, l, j) + CONJG(a(l, i))*tempb(nd)
                ENDDO
              ENDDO
              CALL POPCOMPLEX8(temp)
            ENDDO
