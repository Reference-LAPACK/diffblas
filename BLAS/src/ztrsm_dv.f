...
C>
C  =====================================================================
C      SUBROUTINE ZTRSM_DV(side, uplo, transa, diag, m, n, alpha, alphad
...
CC     .. Scalar Arguments ..
C      COMPLEX*16 alpha
C      COMPLEX*16 alphad(nbdirsmax)
C      INTEGER lda, ldb, m, n
C      CHARACTER diag, side, transa, uplo
CC     ..
CC     .. Array Arguments ..
C      COMPLEX*16 a(lda, *), b(ldb, *)
C      COMPLEX*16 ad(nbdirsmax, lda, *), bd(nbdirsmax, ldb, *)
C      EXTERNAL LSAME
CC     ..
CC
CC  =====================================================================
...
C                ELSE
C                  DO k=1,i-1
C                    temp1 = DCONJG(a(k, i))
C                    DO nd=1,nbdirs
C                      tempd(nd) = tempd(nd) - b(k, j)*DCONJG(ad(nd, k, i
C     +                  )) - temp1*bd(nd, k, j)
C                    ENDDO
C                    temp = temp - temp1*b(k, j)
C                  ENDDO
C                  IF (nounit) THEN
C                    temp1 = DCONJG(a(i, i))
C                    DO nd=1,nbdirs
C                      tempd(nd) = (tempd(nd)-temp*DCONJG(ad(nd, i, i))/
C     +                  temp1)/temp1
C                    ENDDO
C                    temp = temp/temp1
C                  END IF
...
C              ELSE
C                DO i=1,m
C                  temp1 = DCONJG(a(k, k))
C                  temp0 = b(i, k)/temp1
C                  DO nd=1,nbdirs
C                    bd(nd, i, k) = (bd(nd, i, k)-temp0*DCONJG(ad(nd, k, 
C     +                k)))/temp1
C                  ENDDO
C                  b(i, k) = temp0
C                ENDDO
...
C      END
      ...
      SUBROUTINE ZTRSM_DV(SIDE, UPLO, TRANSA, DIAG, M, N,
     +                    ALPHA, ALPHAD, A, AD, LDA, B, BD, LDB, NBDIRS)
      ...
      COMPLEX*16 A(LDA,*), AD(NBDIRSMAX,LDA,*)
      COMPLEX*16 B(LDB,*), BD(NBDIRSMAX,LDB,*)

      COMPLEX*16 X(M,N), T(M,N)
      COMPLEX*16 AD_SLICE(LDA,LDA), BD_SLICE(LDB,N)
      INTEGER ND, I, J
     ...
     +      ' must be in 1..nbdirsmax=', NBDIRSMAX, '. Stopping.'
      ...
      DO ND = 1, NBDIRS
         ...
         DO J = 1, N
            ...
            END DO
         END DO
         CALL ZTRMM(SIDE, UPLO, TRANSA, DIAG, M, N, (1.0D0,0.0D0),
     +              AD_SLICE, LDA, T, M)
     ...
     +                         - T(I,J)
         ...
         CALL ZTRSM(SIDE, UPLO, TRANSA, DIAG, M, N, (1.0D0,0.0D0),
     +              A, LDA, BD_SLICE, LDB)
