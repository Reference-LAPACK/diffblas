...
C
C  Differentiation of dtbmv in forward (tangent) mode (with options multiDirectional):
...
C>
C  =====================================================================
      SUBROUTINE DTBMV_DV(uplo, trans, diag, n, k, a, ad, lda, x, xd, 
     +                    incx, nbdirs)
      IMPLICIT NONE
     ...
     +            xd(nd, i)
        ...
        ELSE
          ...
          DO j=1,n
            ...
            DO i=j+1,min4
              DO nd=1,nbdirs
                tempd(nd) = tempd(nd) + x(ix)*ad(nd, l+i, j) + a(l+i, j)
     +            *xd(nd, ix)
              ENDDO
              temp = temp + a(l+i, j)*x(ix)
              ix = ix + incx
