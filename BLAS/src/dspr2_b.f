...
C
C  Differentiation of dspr2 in reverse (adjoint) mode:
...
C>
C  =====================================================================
      SUBROUTINE DSPR2_B(uplo, n, alpha, alphab, x, xb, incx, y, yb, 
     +                   incy, ap, apb)
      IMPLICIT NONE
            ...
            IF (incx .EQ. 1 .AND. incy .EQ. 1) THEN
              ...
              DO j=n,1,-1
                ...
                IF (branch .EQ. 0) THEN
                  ...
                  DO i=ad_to,1,-1
                    CALL POPINTEGER4(k)
                    xb(i) = xb(i) + temp1*apb(k)
                    temp1b = temp1b + x(i)*apb(k)
                    yb(i) = yb(i) + temp2*apb(k)
                    temp2b = temp2b + y(i)*apb(k)
                  ENDDO
                  CALL POPREAL8(temp2)
              ...
              DO j=n,1,-1
                ...
                IF (branch .EQ. 0) THEN
                  ...
                  DO k=ad_to0,ad_from,-1
                    CALL POPINTEGER4(iy)
                    CALL POPINTEGER4(ix)
                    xb(ix) = xb(ix) + temp1*apb(k)
                    temp1b = temp1b + x(ix)*apb(k)
                    yb(iy) = yb(iy) + temp2*apb(k)
                    temp2b = temp2b + y(iy)*apb(k)
                  ENDDO
                  CALL POPREAL8(temp2)
