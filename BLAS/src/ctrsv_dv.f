...
C
C  Differentiation of ctrsv in forward (tangent) mode (with options multiDirectional):
...
C>
C  =====================================================================
C      SUBROUTINE CTRSV_DV(uplo, trans, diag, n, a, ad, lda, x, xd, incx
C     +                    , nbdirs)
C      IMPLICIT NONE
...
CC     .. Array Arguments ..
C      COMPLEX a(lda, *), x(*)
C      COMPLEX ad(nbdirsmax, lda, *), xd(nbdirsmax, *)
CC     ..
CC
CC  =====================================================================
CC     ..
...
C                DO i=1,j-1
C                  DO nd=1,nbdirs
C                    tempd(nd) = tempd(nd) - x(i)*ad(nd, i, j) - a(i, j)*
C     +                xd(nd, i)
C                  ENDDO
...
C                  temp0 = CONJG(a(i, j))
C                  DO nd=1,nbdirs
C                    tempd(nd) = tempd(nd) - x(i)*CONJG(ad(nd, i, j)) - 
C     +                temp0*xd(nd, i)
C                  ENDDO
...
C                  temp0 = CONJG(a(j, j))
C                  DO nd=1,nbdirs
C                    tempd(nd) = (tempd(nd)-temp*CONJG(ad(nd, j, j))/
C     +                temp0)/temp0
C                  ENDDO
C                  temp = temp/temp0
C                END IF
...
C                  temp0 = CONJG(a(i, j))
C                  DO nd=1,nbdirs
C                    tempd(nd) = tempd(nd) - x(ix)*CONJG(ad(nd, i, j)) - 
C     +                temp0*xd(nd, ix)
C                  ENDDO
...
C                temp0 = CONJG(a(j, j))
C                DO nd=1,nbdirs
C                  tempd(nd) = (tempd(nd)-temp*CONJG(ad(nd, j, j))/temp0)
C     +              /temp0
C                ENDDO
C                temp = temp/temp0
C              END IF
...
C      END
      ...
      SUBROUTINE CTRSV_DV(UPLO, TRANS, DIAG, N, A, AD, LDA, X, XD,
     +                    INCX, NBDIRS)
!
! Vector forward-mode derivative of CTRSV, black-box/Giles-style.
! Self-contained -- does not call CTRSV_D. Passing the SAME trans flag
! to CTRMV as the primal call automatically handles TRANS='C' conjugation.
!
      IMPLICIT NONE
      ...
      COMPLEX X(*), XD(NBDIRSMAX,*)

      COMPLEX S(N), T(N), W(N), AD_SLICE(LDA,LDA)
      INTEGER ND, I, J, KX
...
C        T = op(Ad)*S (local copy, S stays untouched)
         DO I = 1, N
            T(I) = S(I)
         END DO
         CALL CTRMV(UPLO, TRANS, DIAG, N, AD_SLICE, LDA, T, 1)
