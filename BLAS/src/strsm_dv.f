...
C
C  Differentiation of strsm in forward (tangent) mode (with options multiDirectional):
...
C>
C  =====================================================================
C      SUBROUTINE STRSM_DV(side, uplo, transa, diag, m, n, alpha, alphad
...
CC     .. Scalar Arguments ..
C      REAL alpha
C      REAL alphad(nbdirsmax)
C      INTEGER lda, ldb, m, n
C      CHARACTER diag, side, transa, uplo
CC     ..
CC     .. Array Arguments ..
C      REAL a(lda, *), b(ldb, *)
C      REAL ad(nbdirsmax, lda, *), bd(nbdirsmax, ldb, *)
C      EXTERNAL LSAME
CC     ..
CC
CC  =====================================================================
...
C
      ...
      REAL B(LDB,*), BD(NBDIRSMAX,LDB,*)

      REAL X(M,N), T(M,N)
      REAL AD_SLICE(LDA,N), BD_SLICE(LDB,N)
      INTEGER ND, I, J
...
C        T = op(Ad)*X (SIDE='L') or X*op(Ad) (SIDE='R')
         DO J = 1, N
            ...
            END DO
         END DO
         CALL STRMM(SIDE, UPLO, TRANSA, DIAG, M, N, 1.0E0,
     +              AD_SLICE, LDA, T, M)
...
C        Solve the same triangular system again -> dX for this direction
         CALL STRSM(SIDE, UPLO, TRANSA, DIAG, M, N, 1.0E0,
     +              A, LDA, BD_SLICE, LDB)
