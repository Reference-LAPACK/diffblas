...
C
C  Differentiation of zsymm in reverse (adjoint) mode (with options multiDirectional):
...
C>
C  =====================================================================
      SUBROUTINE ZSYMM_BV(side, uplo, m, n, alpha, alphab, a, ab, lda, b
...
C
            ...
            DO j=n,1,-1
              DO i=m,1,-1
                ...
                IF (branch .EQ. 0) THEN
                  ...
                  DO nd=1,nbdirs
                    temp1b(nd) = CONJG(a(i, i))*cb(nd, i, j)
     ...
     +                , j)
                ...
                DO k=ad_to,1,-1
                  DO nd=1,nbdirs
                    bb(nd, k, j) = bb(nd, k, j) + CONJG(a(k, i))*temp2b(
     +                nd)
                    ab(nd, k, i) = ab(nd, k, i) + CONJG(b(k, j))*temp2b(
     +                nd) + CONJG(temp1)*cb(nd, k, j)
...
C
     ...
     +              )
              ...
              ELSE
                CALL POPCOMPLEX16(temp1)
                DO nd=1,nbdirs
                  alphab(nd) = alphab(nd) + CONJG(a(j, k))*temp1b(nd)
                  ab(nd, j, k) = ab(nd, j, k) + CONJG(alpha)*temp1b(nd)
                ENDDO
              END IF
            ENDDO
     ...
     +              )
            ...
            CALL POPCOMPLEX16(temp1)
            DO nd=1,nbdirs
              alphab(nd) = alphab(nd) + CONJG(a(j, j))*temp1b(nd)
