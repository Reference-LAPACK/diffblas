...
C
C  Differentiation of ctrsm in forward (tangent) mode:
...
C>
C  =====================================================================
C      SUBROUTINE CTRSM_D(side, uplo, transa, diag, m, n, alpha, alphad, 
C     +                   a, ad, lda, b, bd, ldb)
C      IMPLICIT NONE
CC
...
C                  DO k=1,i-1
C                    temp0 = CONJG(a(k, i))
C                    tempd = tempd - b(k, j)*CONJG(ad(k, i)) - temp0*bd(k
C     +                , j)
C                    temp = temp - temp0*b(k, j)
C                  ENDDO
C                  IF (nounit) THEN
C                    temp0 = CONJG(a(i, i))
C                    tempd = (tempd-temp*CONJG(ad(i, i))/temp0)/temp0
C                    temp = temp/temp0
C                  END IF
C                END IF
C                bd(i, j) = tempd
...
C                  temp0 = CONJG(a(k, k))
C                  temp1 = b(i, k)/temp0
C                  bd(i, k) = (bd(i, k)-temp1*CONJG(ad(k, k)))/temp0
C                  b(i, k) = temp1
C                ENDDO
C              END IF
C            END IF
...
C                temp = a(j, k)
C              ELSE
C                tempd = CONJG(ad(j, k))
C                temp = CONJG(a(j, k))
C              END IF
C              DO i=1,m
C                bd(i, j) = bd(i, j) - b(i, k)*tempd - temp*bd(i, k)
...
C                DO i=1,m
C                  temp1 = b(i, k)/a(k, k)
C                  bd(i, k) = (bd(i, k)-temp1*ad(k, k))/a(k, k)
C                  b(i, k) = temp1
C                ENDDO
C              ELSE
C                DO i=1,m
C                  temp1 = CONJG(a(k, k))
C                  temp0 = b(i, k)/temp1
C                  bd(i, k) = (bd(i, k)-temp0*CONJG(ad(k, k)))/temp1
C                  b(i, k) = temp0
C                ENDDO
C              END IF
C            END IF
...
C      END
      ...
      SUBROUTINE CTRSM_D(SIDE, UPLO, TRANSA, DIAG, M, N,
     +                   ALPHA, ALPHAD, A, AD, LDA, B, BD, LDB)
!
! Forward-mode derivative of CTRSM, black-box/Giles-style. ALPHA complex.
! Passing the SAME transa flag to CTRMM/CTRSM as the primal call handles
! TRANSA='C' conjugation automatically.
!
      IMPLICIT NONE
      CHARACTER SIDE, UPLO, TRANSA, DIAG
      INTEGER M, N, LDA, LDB
      COMPLEX ALPHA, ALPHAD
      COMPLEX A(LDA,*), AD(LDA,*)
      COMPLEX B(LDB,*), BD(LDB,*)
