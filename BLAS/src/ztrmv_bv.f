...
C
C  Differentiation of ztrmv in reverse (adjoint) mode (with options multiDirectional):
...
C>
C  =====================================================================
      SUBROUTINE ZTRMV_BV(uplo, trans, diag, n, a, ab, lda, x, xb, incx
     +                    , nbdirs)
      IMPLICIT NONE
...
C
     ...
     +                  tempb(nd))
                      tempb(nd) = CONJG(DCONJG(a(j, j)))*tempb(nd)
                    ENDDO
                  END IF
                END IF
                DO nd=1,nbdirs
              ...
              ELSE
                ...
                DO i=n,ad_from4,-1
                  DO nd=1,nbdirs
                    ab(nd, i, j) = ab(nd, i, j) + DCONJG(CONJG(x(i))*
     +                tempb(nd))
                    xb(nd, i) = xb(nd, i) + CONJG(DCONJG(a(i, j)))*tempb
     +                (nd)
                  ENDDO
                ENDDO
                CALL POPCONTROL1B(branch)
