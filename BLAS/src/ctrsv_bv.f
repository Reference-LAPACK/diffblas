...
C
C  Differentiation of ctrsv in reverse (adjoint) mode (with options multiDirectional):
...
C>
C  =====================================================================
C      SUBROUTINE CTRSV_BV(uplo, trans, diag, n, a, ab, lda, x, xb, incx
C     +                    , nbdirs)
C      IMPLICIT NONE
...
CC     .. Array Arguments ..
C      COMPLEX a(lda, *), x(*)
C      COMPLEX ab(nbdirsmax, lda, *), xb(nbdirsmax, *)
CC     ..
CC
CC  =====================================================================
CC     ..
...
C                  DO i=1,ad_from,1
C                    CALL POPCOMPLEX8(x(i))
C                    DO nd=1,nbdirs
C                      tempb(nd) = tempb(nd) + CONJG(-a(i, j))*xb(nd, i)
C                      ab(nd, i, j) = ab(nd, i, j) + CONJG(-temp)*xb(nd, 
C     +                  i)
C                    ENDDO
C                  ENDDO
...
C                    CALL POPCOMPLEX8(x(j))
C                    DO nd=1,nbdirs
C                      tempb0(nd) = CONJG(1.0/a(j, j))*xb(nd, j)
C                      xb(nd, j) = tempb0(nd)
C                      ab(nd, j, j) = ab(nd, j, j) + CONJG(-(x(j)/a(j, j)
C     +                  ))*tempb0(nd)
C                    ENDDO
C                  END IF
C                ENDDO
...
C                  DO i=1,ad_from0,1
C                    DO nd=1,nbdirs
C                      tempb(nd) = tempb(nd) + CONJG(-a(i, j))*xb(nd, ix)
C                      ab(nd, i, j) = ab(nd, i, j) + CONJG(-temp)*xb(nd, 
C     +                  ix)
C                    ENDDO
C                    CALL POPCOMPLEX8(x(ix))
...
C                  CALL POPCONTROL1B(branch)
C                  IF (branch .EQ. 0) THEN
C                    CALL POPCOMPLEX8(x(jx))
C                    DO nd=1,nbdirs
C                      tempb0(nd) = CONJG(1.0/a(j, j))*xb(nd, jx)
C                      xb(nd, jx) = tempb0(nd)
C                      ab(nd, j, j) = ab(nd, j, j) + CONJG(-(x(jx)/a(j, j
C     +                  )))*tempb0(nd)
C                    ENDDO
C                  END IF
C                ENDDO
...
C                  DO nd=1,nbdirs
C                    tempb(nd) = tempb(nd) + CONJG(-a(i, j))*xb(nd, i)
C                    ab(nd, i, j) = ab(nd, i, j) + CONJG(-temp)*xb(nd, i)
C                  ENDDO
C                ENDDO
C                CALL POPCOMPLEX8(temp)
C                DO nd=1,nbdirs
...
C                  DO nd=1,nbdirs
C                    tempb(nd) = tempb(nd) + CONJG(-a(i, j))*xb(nd, ix)
C                    ab(nd, i, j) = ab(nd, i, j) + CONJG(-temp)*xb(nd, ix
C     +                )
C                  ENDDO
C                  CALL POPCOMPLEX8(x(ix))
C                  CALL POPINTEGER4(ix)
...
C                      ab(nd, j, j) = ab(nd, j, j) + CONJG(-(temp/a(j, j)
C     +                  ))*tempb0(nd)
C                    ENDDO
C                  END IF
...
C                  DO i=ad_to,1,-1
C                    DO nd=1,nbdirs
C                      ab(nd, i, j) = ab(nd, i, j) + CONJG(-x(i))*tempb(
C     +                  nd)
C                      xb(nd, i) = xb(nd, i) + CONJG(-a(i, j))*tempb(nd)
C                    ENDDO
C                  ENDDO
C                ELSE
...
C                    temp0 = CONJG(a(j, j))
C                    DO nd=1,nbdirs
C                      ab(nd, j, j) = ab(nd, j, j) + CONJG(CONJG(-(temp/
C     +                  temp0**2))*tempb(nd))
...
C                  DO i=ad_to0,1,-1
C                    DO nd=1,nbdirs
C                      ab(nd, i, j) = ab(nd, i, j) + CONJG(CONJG(-x(i))*
C     +                  tempb(nd))
C                      xb(nd, i) = xb(nd, i) + CONJG(-CONJG(a(i, j)))*
C     +                  tempb(nd)
C                    ENDDO
C                  ENDDO
C                END IF
...
C                    CALL POPINTEGER4(ix)
C                    DO nd=1,nbdirs
C                      ab(nd, i, j) = ab(nd, i, j) + CONJG(-x(ix))*tempb(
C     +                  nd)
C                      xb(nd, ix) = xb(nd, ix) + CONJG(-a(i, j))*tempb(nd
C     +                  )
C                    ENDDO
C                  ENDDO
C                ELSE
...
C                    CALL POPINTEGER4(ix)
C                    DO nd=1,nbdirs
C                      ab(nd, i, j) = ab(nd, i, j) + CONJG(CONJG(-x(ix))*
C     +                  tempb(nd))
C                      xb(nd, ix) = xb(nd, ix) + CONJG(-CONJG(a(i, j)))*
C     +                  tempb(nd)
C                    ENDDO
C                  ENDDO
C                END IF
...
C                DO i=ad_to3,n,1
C                  DO nd=1,nbdirs
C                    ab(nd, i, j) = ab(nd, i, j) + CONJG(-x(i))*tempb(nd)
C                    xb(nd, i) = xb(nd, i) + CONJG(-a(i, j))*tempb(nd)
C                  ENDDO
C                ENDDO
C              ELSE
...
C                  ENDDO
C                END IF
C                CALL POPINTEGER4(ad_to4)
C                DO i=ad_to4,n,1
C                  DO nd=1,nbdirs
C                    ab(nd, i, j) = ab(nd, i, j) + CONJG(CONJG(-x(i))*
C     +                tempb(nd))
C                    xb(nd, i) = xb(nd, i) + CONJG(-CONJG(a(i, j)))*tempb
C     +                (nd)
C                  ENDDO
C                ENDDO
C              END IF
...
C                  ENDDO
C                END IF
C                CALL POPINTEGER4(ad_to5)
C                DO i=ad_to5,n,1
C                  CALL POPINTEGER4(ix)
C                  DO nd=1,nbdirs
C                    ab(nd, i, j) = ab(nd, i, j) + CONJG(-x(ix))*tempb(nd
C     +                )
C                    xb(nd, ix) = xb(nd, ix) + CONJG(-a(i, j))*tempb(nd)
C                  ENDDO
C                ENDDO
...
C      END
      ...
      SUBROUTINE CTRSV_BV(UPLO, TRANS, DIAG, N, A, AB, LDA, X, XB,
     +                    INCX, NBDIRS)
!
! Vector reverse-mode (adjoint) derivative of CTRSV, black-box/Giles-style.
! Self-contained -- does not call CTRSV_B. Wirtinger/CR-calculus convention,
! same three-way TRANS branching as CTRSV_B (TRANS='T' needs the
! conjugate-both-sides trick since conj(A) isn't a direct CTRSV op flag).
!
      IMPLICIT NONE
      ...
      COMPLEX X(*), XB(NBDIRSMAX,*)

      COMPLEX BLOC(N), S(N), YB(N), AB_SLICE(LDA,LDA)
...
C     Gather original B once, recompute S once (shared across directions)
      ...
      DO I = 1, N
         S(I) = BLOC(I)
      END DO
      CALL CTRSV(UPLO, TRANS, DIAG, N, A, LDA, S, 1)
...
C        Solve M^H*Yb = seed, where M = op(A). Three distinct cases.
         IF (ISN) THEN
            CALL CTRSV(UPLO, 'C', DIAG, N, A, LDA, YB, 1)
         ELSE IF (ISC) THEN
            CALL CTRSV(UPLO, 'N', DIAG, N, A, LDA, YB, 1)
         ELSE
            DO I = 1, N
               YB(I) = CONJG(YB(I))
            END DO
            ...
            DO I = 1, N
               IF ((UPPER.AND.I.LE.J).OR.(.NOT.UPPER.AND.I.GE.J)) THEN
                  IF (.NOT.(UNIT.AND.I.EQ.J)) THEN
                     IF (ISN) THEN
                        AB_SLICE(I,J) = -YB(I)*CONJG(S(J))
                     ELSE IF (IST) THEN
                        AB_SLICE(I,J) = -YB(J)*CONJG(S(I))
                     ELSE
                        AB_SLICE(I,J) = -CONJG(YB(J))*S(I)
                     END IF
                  END IF
               END IF
            END DO
