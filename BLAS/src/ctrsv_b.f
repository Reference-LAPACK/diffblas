...
C
C  Differentiation of ctrsv in reverse (adjoint) mode:
...
C>
C  =====================================================================
C      SUBROUTINE CTRSV_B(uplo, trans, diag, n, a, ab, lda, x, xb, incx)
C      IMPLICIT NONE
...
C                    tempb = tempb + CONJG(-a(i, j))*xb(i)
C                    ab(i, j) = ab(i, j) + CONJG(-temp)*xb(i)
C                  ENDDO
...
C                    tempb0 = CONJG(1.0/a(j, j))*xb(j)
C                    xb(j) = tempb0
C                    ab(j, j) = ab(j, j) + CONJG(-(x(j)/a(j, j)))*tempb0
C                  END IF
C                ENDDO
C              ELSE
C                jx = kx + (n-1)*incx
...
C                    tempb = tempb + CONJG(-a(i, j))*xb(ix)
C                    ab(i, j) = ab(i, j) + CONJG(-temp)*xb(ix)
...
C                    tempb0 = CONJG(1.0/a(j, j))*xb(jx)
C                    xb(jx) = tempb0
C                    ab(j, j) = ab(j, j) + CONJG(-(x(jx)/a(j, j)))*tempb0
C                  END IF
C                ENDDO
C              END IF
...
C                  IF (branch .EQ. 0) THEN
C                    CALL POPCOMPLEX8(temp)
C                    tempb0 = CONJG(1.0/a(j, j))*tempb
C                    tempb = tempb0
C                    ab(j, j) = ab(j, j) + CONJG(-(temp/a(j, j)))*tempb0
C                  END IF
C                  CALL POPINTEGER4(ad_to)
C                  DO i=ad_to,1,-1
C                    ab(i, j) = ab(i, j) + CONJG(-x(i))*tempb
C                    xb(i) = xb(i) + CONJG(-a(i, j))*tempb
C                  ENDDO
C                ELSE
C                  IF (branch .EQ. 2) THEN
C                    CALL POPCOMPLEX8(temp)
C                    temp0 = CONJG(a(j, j))
C                    ab(j, j) = ab(j, j) + CONJG(CONJG(-(temp/temp0**2))*
C     +                tempb)
C                    tempb = CONJG(1.0/temp0)*tempb
C                  END IF
C                  CALL POPINTEGER4(ad_to0)
C                  DO i=ad_to0,1,-1
C                    ab(i, j) = ab(i, j) + CONJG(CONJG(-x(i))*tempb)
C                    xb(i) = xb(i) + CONJG(-CONJG(a(i, j)))*tempb
C                  ENDDO
C                END IF
C                xb(j) = xb(j) + tempb
C              ENDDO
...
C                  DO i=ad_to1,1,-1
C                    CALL POPINTEGER4(ix)
C                    ab(i, j) = ab(i, j) + CONJG(-x(ix))*tempb
C                    xb(ix) = xb(ix) + CONJG(-a(i, j))*tempb
C                  ENDDO
C                ELSE
C                  IF (branch .EQ. 2) THEN
C                    CALL POPCOMPLEX8(temp)
...
C                  DO i=ad_to2,1,-1
C                    CALL POPINTEGER4(ix)
C                    ab(i, j) = ab(i, j) + CONJG(CONJG(-x(ix))*tempb)
C                    xb(ix) = xb(ix) + CONJG(-CONJG(a(i, j)))*tempb
C                  ENDDO
C                END IF
C                xb(jx) = xb(jx) + tempb
C              ENDDO
...
C      END
      ...
      SUBROUTINE CTRSV_B(UPLO, TRANS, DIAG, N, A, AB, LDA, X, XB, INCX)
!
! Reverse-mode (adjoint) derivative of CTRSV, black-box/Giles-style,
! using the Wirtinger/CR-calculus convention (Ab = dL/dRe(A) + i*dL/dIm(A)).
! X is read-only. Handles all three TRANS cases distinctly, since
! TRANS='T' needs a conjugate-both-sides trick (conj(A) isn't directly
! expressible as a CTRSV op flag).
!
      IMPLICIT NONE
...
C     Solve M^H*Yb = seed, where M = op(A). Three distinct cases.
      ...
      ELSE
C        TRANS='T': conj(A)*Yb=seed  <=>  A*conj(Yb)=conj(seed)
         DO I = 1, N
            YB(I) = CONJG(YB(I))
         END DO
         CALL CTRSV(UPLO, 'N', DIAG, N, A, LDA, YB, 1)
         ...
         DO I = 1, N
            IF ((UPPER.AND.I.LE.J) .OR. (.NOT.UPPER.AND.I.GE.J)) THEN
               IF (.NOT.(UNIT.AND.I.EQ.J)) THEN
                  IF (ISN) THEN
                     AB(I,J) = -YB(I)*CONJG(S(J))
                  ELSE IF (IST) THEN
                     AB(I,J) = -YB(J)*CONJG(S(I))
                  ELSE
                     AB(I,J) = -CONJG(YB(J))*S(I)
                  END IF
               END IF
            END IF
         END DO
