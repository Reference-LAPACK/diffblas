...
C
C  Differentiation of strsm in reverse (adjoint) mode (with options multiDirectional):
...
C>
C  =====================================================================
C      SUBROUTINE STRSM_BV(side, uplo, transa, diag, m, n, alpha, alphab
...
CC     .. Scalar Arguments ..
C      REAL alpha
C      REAL alphab(nbdirsmax)
C      INTEGER lda, ldb, m, n
C      CHARACTER diag, side, transa, uplo
CC     ..
CC     .. Array Arguments ..
C      REAL a(lda, *), b(ldb, *)
C      REAL ab(nbdirsmax, lda, *), bb(nbdirsmax, ldb, *)
C      EXTERNAL LSAME
CC     ..
CC
CC  =====================================================================
...
C      REAL tmp
C      REAL tmpb(nbdirsmax)
C      REAL tmp0
C      REAL tmpb0(nbdirsmax)
C      REAL tmp1
C      REAL tmpb1(nbdirsmax)
C      REAL tmp2
C      REAL tmpb2(nbdirsmax)
C      REAL tmp3
C      REAL tmpb3(nbdirsmax)
C      REAL tmp4
C      REAL tmpb4(nbdirsmax)
C      INTEGER ad_to
C      INTEGER*4 branch
C      INTEGER ad_from
C      INTEGER ad_to0
...
C
      ...
      REAL B(LDB,*), BB(NBDIRSMAX,LDB,*)

      REAL X(M,N)
      REAL BB_SLICE(LDB,N), AB_SLICE(LDA,LDA)
      CHARACTER TRANSA_T
...
C        Step 1: Yb solves op(A)^T*Yb = Xb (overwrites BB_SLICE in place)
         CALL STRSM(SIDE, UPLO, TRANSA_T, DIAG, M, N, 1.0E0,
     +              A, LDA, BB_SLICE, LDB)
