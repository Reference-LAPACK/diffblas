...
C
C  Differentiation of cgeru in forward (tangent) mode (with options multiDirectional):
...
C  Differentiation of cgerc in forward (tangent) mode (with options multiDirectional):
C   variations   of useful results: a
C   with respect to varying inputs: alpha x y a
C> \brief \b CGERC
C
...
C>
C  =====================================================================
      SUBROUTINE CGERC_DV(m, n, alpha, alphad, x, xd, incx, y, yd, incy
     +                    , a, ad, lda, nbdirs)
      IMPLICIT NONE
      INCLUDE 'DIFFSIZESF.inc'
C  Hint: nbdirsmax should be the maximum number of differentiation directions
        ...
        IF (incx .EQ. 1) THEN
          DO j=1,n
            IF (y(jy) .NE. zero) THEN
              temp0 = CONJG(y(jy))
              DO nd=1,nbdirs
                tempd(nd) = temp0*alphad(nd) + alpha*CONJG(yd(nd, jy))
              ENDDO
              temp = alpha*temp0
              DO i=1,m
                DO nd=1,nbdirs
