...
C>
C  =====================================================================
      SUBROUTINE CTRSM_DV(side, uplo, transa, diag, m, n, alpha, alphad
...
C
        DO j=1,n
          DO i=1,m
            DO nd=1,nbdirs
C     FIXED: Removed zeroing of bd - should accumulate from input seed
            ENDDO
            b(i, j) = zero
          ENDDO
        ENDDO
...
C
     ...
     +                  ) - temp0*bd(nd, k, j)
                  ...
                  IF (nounit) THEN
                    temp0 = CONJG(a(i, i))
                    DO nd=1,nbdirs
                      tempd(nd) = (tempd(nd)-temp*CONJG(ad(nd, i, i))/
     +                  temp0)/temp0
                    ENDDO
                    temp = temp/temp0
                  END IF
          ...
          DO k=n,1,-1
            IF (nounit) THEN
              IF (noconj) THEN
                ...
                temp = temp0
              ELSE
                temp0 = one/CONJG(a(k, k))
                DO nd=1,nbdirs
                  tempd(nd) = -(temp0*CONJG(ad(nd, k, k))/CONJG(a(k, k))
     +              )
                ENDDO
                temp = temp0
              END IF
