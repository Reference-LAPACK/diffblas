...
C
C  Differentiation of ztbmv in reverse (adjoint) mode:
...
C>
C  =====================================================================
      SUBROUTINE ZTBMV_B(uplo, trans, diag, n, k, a, ab, lda, x, xb, 
     +                   incx)
      IMPLICIT NONE
              ...
              DO j=1,n,1
                ...
                ELSE
                  ...
                  DO i=ad_to4,ad_from4,1
                    ab(l+i, j) = ab(l+i, j) + DCONJG(CONJG(x(i))*tempb)
                    xb(i) = xb(i) + CONJG(DCONJG(a(l+i, j)))*tempb
                  ENDDO
                  CALL POPCONTROL1B(branch)
                  IF (branch .NE. 0) THEN
                    CALL POPCOMPLEX16(temp)
                    ab(kplus1, j) = ab(kplus1, j) + DCONJG(CONJG(temp)*
     +                tempb)
                    tempb = CONJG(DCONJG(a(kplus1, j)))*tempb
                  END IF
                END IF
                xb(j) = xb(j) + tempb
              ENDDO
              ...
              DO j=1,n,1
                ...
                ELSE
                  ...
                  DO i=ad_to6,ad_from6,1
                    CALL POPINTEGER4(ix)
                    ab(l+i, j) = ab(l+i, j) + DCONJG(CONJG(x(ix))*tempb)
                    xb(ix) = xb(ix) + CONJG(DCONJG(a(l+i, j)))*tempb
                  ENDDO
                  CALL POPCONTROL1B(branch)
                  IF (branch .NE. 0) THEN
                    CALL POPCOMPLEX16(temp)
          ...
          ELSE IF (incx .EQ. 1) THEN
            ...
            DO j=n,1,-1
              ...
              ELSE
                ...
                IF (branch .NE. 0) THEN
                  CALL POPCOMPLEX16(temp)
                  ab(1, j) = ab(1, j) + DCONJG(CONJG(temp)*tempb)
                  tempb = CONJG(DCONJG(a(1, j)))*tempb
                END IF
              END IF
              xb(j) = xb(j) + tempb
            ENDDO
