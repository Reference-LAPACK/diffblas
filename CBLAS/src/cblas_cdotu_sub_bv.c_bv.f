...
C
C  Differentiation of cdotu in reverse (adjoint) mode (with options multiDirectional):
...
C>
C  =====================================================================
      SUBROUTINE CDOTU_BV(n, cx, cxb, incx, cy, cyb, incy, cdotub, 
     +                    nbdirs)
      IMPLICIT NONE
...
C
        ...
        IF (branch .EQ. 0) THEN
          ...
          DO i=n,1,-1
            DO nd=1,nbdirs
              cxb(nd, i) = cxb(nd, i) + CONJG(cy(i))*ctempb(nd)
              cyb(nd, i) = cyb(nd, i) + CONJG(cx(i))*ctempb(nd)
            ENDDO
          ENDDO
        ELSE
          DO ii1=1,ISIZE1OFcx
          ...
          DO i=n,1,-1
            ...
            CALL POPINTEGER4(ix)
            DO nd=1,nbdirs
              cxb(nd, ix) = cxb(nd, ix) + CONJG(cy(iy))*ctempb(nd)
              cyb(nd, iy) = cyb(nd, iy) + CONJG(cx(ix))*ctempb(nd)
            ENDDO
          ENDDO
        END IF
      END IF
...
C  Differentiation of cdotusub in reverse (adjoint) mode (with options multiDirectional):
...
C
      SUBROUTINE CDOTUSUB_BV(n, x, xb, incx, y, yb, incy, dotu, dotub, 
     +                       nbdirs)
      IMPLICIT NONE
      INCLUDE 'DIFFSIZESF.inc'
C  Hint: nbdirsmax should be the maximum number of differentiation directions
