...
C
C  Differentiation of zsyr2k in reverse (adjoint) mode:
...
C>
C  =====================================================================
      SUBROUTINE ZSYR2K_B(uplo, trans, n, k, alpha, alphab, a, ab, lda, 
     +                    b, bb, ldb, beta, betab, c, cb, ldc)
      IMPLICIT NONE
...
C     .. Array Arguments ..
      ...
      COMPLEX*16 ab(lda, *), bb(ldb, *), cb(ldc, *)
      EXTERNAL LSAME
C     ..
C
C  =====================================================================
            ...
            DO j=n,1,-1
              DO l=k,1,-1
                ...
                IF (branch .EQ. 0) THEN
                  ...
                  DO i=ad_to3,1,-1
                    ab(i, l) = ab(i, l) + CONJG(temp1)*cb(i, j)
                    temp1b = temp1b + CONJG(a(i, l))*cb(i, j)
                    bb(i, l) = bb(i, l) + CONJG(temp2)*cb(i, j)
                    temp2b = temp2b + CONJG(b(i, l))*cb(i, j)
                  ENDDO
                  CALL POPCOMPLEX16(temp2)
                  alphab = alphab + CONJG(a(j, l))*temp2b + CONJG(b(j, l
     +              ))*temp1b
                  ab(j, l) = ab(j, l) + CONJG(alpha)*temp2b
                  CALL POPCOMPLEX16(temp1)
                  bb(j, l) = bb(j, l) + CONJG(alpha)*temp1b
                END IF
              ENDDO
...
C
          ...
          DO j=n,1,-1
            ...
            DO i=ad_to4,1,-1
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                alphab = alphab + (CONJG(temp1)+CONJG(temp2))*cb(i, j)
                temp1b = CONJG(alpha)*cb(i, j)
              ...
              END IF
              DO l=k,1,-1
                bb(l, i) = bb(l, i) + CONJG(a(l, j))*temp2b
                ab(l, j) = ab(l, j) + CONJG(b(l, i))*temp2b
                ab(l, i) = ab(l, i) + CONJG(b(l, j))*temp1b
                bb(l, j) = bb(l, j) + CONJG(a(l, i))*temp1b
              ENDDO
              CALL POPCOMPLEX16(temp2)
              CALL POPCOMPLEX16(temp1)
            ENDDO
