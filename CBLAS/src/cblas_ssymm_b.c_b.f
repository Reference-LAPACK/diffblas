...
C
C  Differentiation of ssymm in reverse (adjoint) mode:
...
C>
C  =====================================================================
      SUBROUTINE SSYMM_B(side, uplo, m, n, alpha, alphab, a, ab, lda, b
            ...
            DO j=n,1,-1
              DO i=m,1,-1
                ...
                IF (branch .EQ. 0) THEN
                  ...
                  CALL POPREAL4(c(i, j))
                  temp1b = a(i, i)*cb(i, j)
                  ab(i, i) = ab(i, i) + temp1*cb(i, j)
                  alphab = alphab + temp2*cb(i, j)
                  temp2b = alpha*cb(i, j)
                  cb(i, j) = 0.0
                ELSE
                  temp1 = alpha*b(i, j)
                ...
                DO k=ad_to,1,-1
                  bb(k, j) = bb(k, j) + a(k, i)*temp2b
                  ab(k, i) = ab(k, i) + b(k, j)*temp2b + temp1*cb(k, j)
                  CALL POPREAL4(c(k, j))
                  temp1b = temp1b + a(k, i)*cb(k, j)
                ENDDO
                CALL POPREAL4(temp2)
                alphab = alphab + b(i, j)*temp1b
                bb(i, j) = bb(i, j) + alpha*temp1b
              ENDDO
            ENDDO
          ELSE
            DO j=1,n
...
C
          ...
          DO j=n,1,-1
            ...
            DO k=n,ad_from0,-1
              temp1b = 0.0
              DO i=m,1,-1
                temp1b = temp1b + b(i, k)*cb(i, j)
                bb(i, k) = bb(i, k) + temp1*cb(i, j)
              ENDDO
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                CALL POPREAL4(temp1)
                alphab = alphab + a(k, j)*temp1b
                ab(k, j) = ab(k, j) + alpha*temp1b
              ELSE
                CALL POPREAL4(temp1)
                alphab = alphab + a(j, k)*temp1b
                ab(j, k) = ab(j, k) + alpha*temp1b
              END IF
            ENDDO
            ...
            IF (branch .EQ. 0) THEN
              ...
              DO i=m,1,-1
                betab = betab + c(i, j)*cb(i, j)
                temp1b = temp1b + b(i, j)*cb(i, j)
                bb(i, j) = bb(i, j) + temp1*cb(i, j)
                cb(i, j) = beta*cb(i, j)
              ENDDO
            ELSE
              temp1b = 0.0
            ...
            END IF
            CALL POPREAL4(temp1)
            alphab = alphab + a(j, j)*temp1b
            ab(j, j) = ab(j, j) + alpha*temp1b
          ENDDO
        END IF
      END IF
