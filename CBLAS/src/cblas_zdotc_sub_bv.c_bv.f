...
C
C  Differentiation of zdotc in reverse (adjoint) mode (with options multiDirectional):
...
C>
C  =====================================================================
      SUBROUTINE ZDOTC_BV(n, zx, zxb, incx, zy, zyb, incy, zdotcb, 
     +                    nbdirs)
      IMPLICIT NONE
...
C
        ...
        IF (branch .EQ. 0) THEN
          ...
          DO i=n,1,-1
            DO nd=1,nbdirs
              zxb(nd, i) = zxb(nd, i) + DCONJG(CONJG(zy(i))*ztempb(nd))
              zyb(nd, i) = zyb(nd, i) + CONJG(DCONJG(zx(i)))*ztempb(nd)
            ENDDO
          ENDDO
        ELSE
          DO ii1=1,ISIZE1OFzx
          ...
          DO i=n,1,-1
            ...
            CALL POPINTEGER4(ix)
            DO nd=1,nbdirs
              zxb(nd, ix) = zxb(nd, ix) + DCONJG(CONJG(zy(iy))*ztempb(nd
     +          ))
              zyb(nd, iy) = zyb(nd, iy) + CONJG(DCONJG(zx(ix)))*ztempb(
     +          nd)
            ENDDO
          ENDDO
        END IF
...
C  Differentiation of zdotcsub in reverse (adjoint) mode (with options multiDirectional):
C   gradient     of useful results: dotc
...
C
      SUBROUTINE ZDOTCSUB_BV(n, x, xb, incx, y, yb, incy, dotc, dotcb, 
     +                       nbdirs)
      IMPLICIT NONE
      INCLUDE 'DIFFSIZESF.inc'
C  Hint: nbdirsmax should be the maximum number of differentiation directions
