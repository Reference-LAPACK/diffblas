...
C>
C  =====================================================================
      SUBROUTINE CTRSM_D(side, uplo, transa, diag, m, n, alpha, alphad, 
     +                   a, ad, lda, b, bd, ldb)
      IMPLICIT NONE
C
            ...
            IF (upper) THEN
              DO j=1,n
                IF (alpha .NE. one) THEN
                  DO i=1,m
                    bd(i, j) = b(i, j)*alphad + alpha*bd(i, j)
                    b(i, j) = alpha*b(i, j)
                  ENDDO
                END IF
                DO k=m,1,-1
                  IF (b(k, j) .NE. zero) THEN
                    IF (nounit) THEN
                      temp0 = b(k, j)/a(k, k)
                      bd(k, j) = (bd(k, j)-temp0*ad(k, k))/a(k, k)
                      b(k, j) = temp0
                    END IF
                    DO i=1,k-1
                      bd(i, j) = bd(i, j) - a(i, k)*bd(k, j) - b(k, j)*
     +                  ad(i, k)
...
C
            DO j=1,n
              DO i=1,m
                tempd = b(i, j)*alphad + alpha*bd(i, j)
                temp = alpha*b(i, j)
                IF (noconj) THEN
                  DO k=1,i-1
                  ...
                  IF (nounit) THEN
                    temp0 = temp/a(i, i)
                    tempd = (tempd-temp0*ad(i, i))/a(i, i)
                    temp = temp0
                  END IF
                ELSE
                  DO k=1,i-1
     ...
     +                , j)
                  ...
                  IF (nounit) THEN
                    temp0 = CONJG(a(i, i))
                    tempd = (tempd-temp*CONJG(ad(i, i))/temp0)/temp0
                    temp = temp/temp0
                  END IF
                END IF
                bd(i, j) = tempd
            ...
            DO j=1,n
              ...
              DO k=1,j-1
                IF (a(k, j) .NE. zero) THEN
                  DO i=1,m
                    bd(i, j) = bd(i, j) - b(i, k)*ad(k, j) - a(k, j)*bd(
     +                i, k)
...
C
          DO k=n,1,-1
            IF (nounit) THEN
              IF (noconj) THEN
                temp0 = one/a(k, k)
                tempd = -(temp0*ad(k, k)/a(k, k))
                temp = temp0
              ELSE
                temp0 = one/CONJG(a(k, k))
                tempd = -(temp0*CONJG(ad(k, k))/CONJG(a(k, k)))
                temp = temp0
              END IF
              DO i=1,m
            ...
            IF (alpha .NE. one) THEN
              DO i=1,m
                bd(i, k) = b(i, k)*alphad + alpha*bd(i, k)
                b(i, k) = alpha*b(i, k)
              ENDDO
            END IF
          ENDDO
