...
C
C  Differentiation of dsymm in reverse (adjoint) mode (with options multiDirectional):
...
C  ===========
C
C       SUBROUTINE DSYMM(SIDE,UPLO,M,N,ALPHA,A,LDA,B,LDB,BETA,C,LDC)
C
C       .. Scalar Arguments ..
...
C>
C  =====================================================================
      SUBROUTINE DSYMM_BV(side, uplo, m, n, alpha, alphab, a, ab, lda, b
          ...
          IF (upper) THEN
            DO j=1,n
              DO i=1,m
                ...
                temp2 = zero
                DO k=1,i-1
                  CALL PUSHREAL8(c(k, j))
                  c(k, j) = c(k, j) + temp1*a(k, i)
                  temp2 = temp2 + b(k, j)*a(k, i)
                ENDDO
                CALL PUSHINTEGER4(k - 1)
                IF (beta .EQ. zero) THEN
                  CALL PUSHREAL8(c(i, j))
