...
C
C  Differentiation of csymm in forward (tangent) mode (with options multiDirectional):
...
C>
C  =====================================================================
      SUBROUTINE CSYMM_DV(side, uplo, m, n, alpha, alphad, a, ad, lda, b
...
C
     ...
     +                temp1*ad(nd, k, i)
                    temp2d(nd) = temp2d(nd) + a(k, i)*bd(nd, k, j) + b(k
     +                , j)*ad(nd, k, i)
                  ENDDO
                ...
                IF (beta .EQ. zero) THEN
                  DO nd=1,nbdirs
                    cd(nd, i, j) = a(i, i)*temp1d(nd) + temp1*ad(nd, i, 
     +                i) + temp2*alphad(nd) + alpha*temp2d(nd)
                  ENDDO
                  c(i, j) = temp1*a(i, i) + alpha*temp2
                ELSE
                  DO nd=1,nbdirs
                    cd(nd, i, j) = c(i, j)*betad(nd) + beta*cd(nd, i, j)
     +                + a(i, i)*temp1d(nd) + temp1*ad(nd, i, i) + temp2*
     +                alphad(nd) + alpha*temp2d(nd)
                  ENDDO
...
C
          DO j=1,n
            DO nd=1,nbdirs
              temp1d(nd) = a(j, j)*alphad(nd) + alpha*ad(nd, j, j)
            ENDDO
            temp1 = alpha*a(j, j)
            IF (beta .EQ. zero) THEN
              DO i=1,m
     ...
     +              b(i, j)*temp1d(nd) + temp1*bd(nd, i, j)
            ...
            DO k=1,j-1
              ...
              ELSE
                DO nd=1,nbdirs
                  temp1d(nd) = a(j, k)*alphad(nd) + alpha*ad(nd, j, k)
                ENDDO
                temp1 = alpha*a(j, k)
              END IF
              DO i=1,m
