...
C
C  Differentiation of chbmv in forward (tangent) mode (with options multiDirectional):
...
C>
C  =====================================================================
      SUBROUTINE CHBMV_DV(uplo, n, k, alpha, alphad, a, ad, lda, x, xd, 
...
C     .. Local Scalars ..
      COMPLEX temp1, temp2
      COMPLEX temp1d(nbdirsmax), temp2d(nbdirsmax)
        ...
        IF (beta .NE. one) THEN
          IF (incy .EQ. 1) THEN
            IF (beta .EQ. zero) THEN
              DO i=1,n
                DO nd=1,nbdirs
C     FIXED: Removed zeroing of yd - should accumulate from input seed
                ENDDO
                y(i) = zero
              ENDDO
            ELSE
...
C
     ...
     +                *ad(nd, l+i, j)
                    temp2d(nd) = temp2d(nd) + x(i)*CONJG(ad(nd, l+i, j))
     +                + temp*xd(nd, i)
                  ENDDO
                  y(i) = y(i) + temp1*a(l+i, j)
                  temp2 = temp2 + temp*x(i)
                ENDDO
                temp0 = REAL(a(kplus1, j))
                DO nd=1,nbdirs
                  yd(nd, j) = yd(nd, j) + temp0*temp1d(nd) + temp1*REAL(
     ...
     +                temp1*ad(nd, l+i, j)
                    temp2d(nd) = temp2d(nd) + x(ix)*CONJG(ad(nd, l+i, j)
     +                ) + temp*xd(nd, ix)
                  ENDDO
     ...
     +              REAL(ad(nd, kplus1, j)) + temp2*alphad(nd) + alpha*
     +              temp2d(nd)
                ENDDO
...
C
            DO j=1,n
              ...
              DO nd=1,nbdirs
                temp1d(nd) = x(j)*alphad(nd) + alpha*xd(nd, j)
                yd(nd, j) = yd(nd, j) + temp0*temp1d(nd) + temp1*REAL(ad
     +            (nd, 1, j))
              ENDDO
              temp2 = zero
              y(j) = y(j) + temp1*temp0
     ...
     +            nd)
          ...
          ELSE
            ...
            DO j=1,n
              ...
              DO nd=1,nbdirs
                temp1d(nd) = x(jx)*alphad(nd) + alpha*xd(nd, jx)
                yd(nd, jy) = yd(nd, jy) + temp0*temp1d(nd) + temp1*REAL(
     +            ad(nd, 1, j))
              ENDDO
              temp2 = zero
