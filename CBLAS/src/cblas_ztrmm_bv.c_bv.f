...
C
C  Differentiation of ztrmm in reverse (adjoint) mode (with options multiDirectional):
...
C>
C  =====================================================================
      SUBROUTINE ZTRMM_BV(side, uplo, transa, diag, m, n, alpha, alphab
...
C
     ...
     +                  tempb(nd))
                      tempb(nd) = CONJG(DCONJG(a(i, i)))*tempb(nd)
                    ENDDO
                  END IF
                END IF
                CALL POPCOMPLEX16(temp)
...
C
          ...
          DO k=n,1,-1
            ...
            ELSE IF (branch .EQ. 1) THEN
              CALL POPCOMPLEX16(temp)
              DO nd=1,nbdirs
                ab(nd, k, k) = ab(nd, k, k) + DCONJG(CONJG(temp)*tempb(
     +            nd))
                tempb(nd) = CONJG(DCONJG(a(k, k)))*tempb(nd)
              ENDDO
            END IF
            CALL POPCOMPLEX16(temp)
            DO nd=1,nbdirs
