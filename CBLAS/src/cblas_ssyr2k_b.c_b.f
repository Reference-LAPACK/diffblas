...
C
C  Differentiation of ssyr2k in reverse (adjoint) mode:
...
C>
C  =====================================================================
      SUBROUTINE SSYR2K_B(uplo, trans, n, k, alpha, alphab, a, ab, lda, 
            ...
            DO j=n,1,-1
              DO l=k,1,-1
                ...
                IF (branch .EQ. 0) THEN
                  ...
                  DO i=ad_to3,1,-1
                    ab(i, l) = ab(i, l) + temp1*cb(i, j)
                    temp1b = temp1b + a(i, l)*cb(i, j)
                    bb(i, l) = bb(i, l) + temp2*cb(i, j)
                    temp2b = temp2b + b(i, l)*cb(i, j)
                  ENDDO
                  CALL POPREAL4(temp2)
                  alphab = alphab + a(j, l)*temp2b + b(j, l)*temp1b
                  ab(j, l) = ab(j, l) + alpha*temp2b
                  CALL POPREAL4(temp1)
                  bb(j, l) = bb(j, l) + alpha*temp1b
                END IF
              ENDDO
...
C
          ...
          DO j=n,1,-1
            ...
            DO i=ad_to4,1,-1
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                alphab = alphab + (temp1+temp2)*cb(i, j)
                temp1b = alpha*cb(i, j)
                temp2b = alpha*cb(i, j)
                cb(i, j) = 0.0
              ELSE
              ...
              END IF
              DO l=k,1,-1
                bb(l, i) = bb(l, i) + a(l, j)*temp2b
                ab(l, j) = ab(l, j) + b(l, i)*temp2b
                ab(l, i) = ab(l, i) + b(l, j)*temp1b
                bb(l, j) = bb(l, j) + a(l, i)*temp1b
              ENDDO
              CALL POPREAL4(temp2)
              CALL POPREAL4(temp1)
            ENDDO
