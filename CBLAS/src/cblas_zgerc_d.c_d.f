...
C
C  Differentiation of zgeru in forward (tangent) mode:
...
C>
C  =====================================================================
      SUBROUTINE ZGERU_D(m, n, alpha, alphad, x, xd, incx, y, yd, incy, 
     +                   a, ad, lda)
      IMPLICIT NONE
C
...
C  Differentiation of zgerc in forward (tangent) mode:
C   variations   of useful results: a
C   with respect to varying inputs: alpha x y a
C> \brief \b ZGERC
C
...
C>
C  =====================================================================
      SUBROUTINE ZGERC_D(m, n, alpha, alphad, x, xd, incx, y, yd, incy, 
     +                   a, ad, lda)
      IMPLICIT NONE
C
C  -- Reference BLAS level2 routine --
...
C
        ...
        IF (incx .EQ. 1) THEN
          DO j=1,n
            IF (y(jy) .NE. zero) THEN
              temp0 = DCONJG(y(jy))
              tempd = temp0*alphad + alpha*DCONJG(yd(jy))
              temp = alpha*temp0
              DO i=1,m
                ad(i, j) = ad(i, j) + temp*xd(i) + x(i)*tempd
                a(i, j) = a(i, j) + x(i)*temp
