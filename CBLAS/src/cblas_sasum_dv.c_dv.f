...
C  Differentiation of sasumsub in forward (tangent) mode (with options multiDirectional):
...
C
      SUBROUTINE SASUMSUB_DV(n, x, xd, incx, asum, asumd, nbdirs)
      IMPLICIT NONE
      INCLUDE 'DIFFSIZESF.inc'
C  Hint: nbdirsmax should be the maximum number of differentiation directions
C
      ...
      EXTERNAL SASUM_DV
      REAL SASUM, asum
      REAL asumd(nbdirsmax)
      INTEGER n, incx
      REAL x(*)
      REAL xd(nbdirsmax, *)
      INTEGER nbdirs
C
      CALL SASUM_DV(n, x, xd, incx, asum, asumd, nbdirs)
      RETURN
      END
