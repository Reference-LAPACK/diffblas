...
C
C  Differentiation of ctbmv in reverse (adjoint) mode (with options multiDirectional):
...
C>
C  =====================================================================
      SUBROUTINE CTBMV_BV(uplo, trans, diag, n, k, a, ab, lda, x, xb, 
     +                    incx, nbdirs)
      IMPLICIT NONE
...
C
     ...
     +                  tempb(nd)
                  ...
                  IF (branch .NE. 0) THEN
                    CALL POPCOMPLEX8(temp)
                    DO nd=1,nbdirs
                      ab(nd, kplus1, j) = ab(nd, kplus1, j) + CONJG(
     +                  CONJG(temp)*tempb(nd))
                      tempb(nd) = CONJG(CONJG(a(kplus1, j)))*tempb(nd)
                    ENDDO
                  END IF
                END IF
                DO nd=1,nbdirs
     ...
     +                tempb(nd)
                ...
                IF (branch .NE. 0) THEN
                  CALL POPCOMPLEX8(temp)
                  DO nd=1,nbdirs
                    ab(nd, 1, j) = ab(nd, 1, j) + CONJG(CONJG(temp)*
     +                tempb(nd))
                    tempb(nd) = CONJG(CONJG(a(1, j)))*tempb(nd)
                  ENDDO
                END IF
              END IF
              DO nd=1,nbdirs
