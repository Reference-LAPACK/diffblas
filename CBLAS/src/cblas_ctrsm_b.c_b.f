...
C>
C  =====================================================================
      SUBROUTINE CTRSM_B(side, uplo, transa, diag, m, n, alpha, alphab, 
     +                   a, ab, lda, b, bb, ldb)
      IMPLICIT NONE
              ...
              DO i=m,1,-1
                ...
                ELSE
                  IF (branch .EQ. 2) THEN
                    CALL POPCOMPLEX8(temp)
                    temp0 = CONJG(a(i, i))
                    ab(i, i) = ab(i, i) + CONJG(CONJG(-(temp/temp0**2))*
     +                tempb)
                    tempb = CONJG(1.0/temp0)*tempb
                  END IF
                  CALL POPINTEGER4(ad_to1)
                  DO k=ad_to1,1,-1
                    ab(k, i) = ab(k, i) + CONJG(CONJG(-b(k, j))*tempb)
                    bb(k, j) = bb(k, j) + CONJG(-CONJG(a(k, i)))*tempb
                  ENDDO
                END IF
          ...
          DO k=1,n,1
            ...
            IF (branch .NE. 0) THEN
              ...
              IF (branch .EQ. 0) THEN
                CALL POPCOMPLEX8(temp)
                temp0 = CONJG(a(k, k))
                ab(k, k) = ab(k, k) + CONJG(CONJG(-(one/temp0**2))*tempb
     +            )
              ELSE
                CALL POPCOMPLEX8(temp)
