...
C>
C  =====================================================================
      SUBROUTINE ZTRSM_BV(side, uplo, transa, diag, m, n, alpha, alphab
...
C     .. Array Arguments ..
      ...
      COMPLEX*16 ab(nbdirsmax, lda, *), bb(nbdirsmax, ldb, *)
      EXTERNAL LSAME
C     ..
C
C  =====================================================================
...
C     .. Parameters ..
      ...
      INTEGER max2
      INTEGER nd
      COMPLEX*16 tempb0(nbdirsmax)
      COMPLEX*16 tmp
      COMPLEX*16 tmpb(nbdirsmax)
      COMPLEX*16 tmp0
      COMPLEX*16 tmpb0(nbdirsmax)
      DOUBLE COMPLEX temp0
      COMPLEX*16 tmp1
      COMPLEX*16 tmpb1(nbdirsmax)
      COMPLEX*16 tmp2
      COMPLEX*16 tmpb2(nbdirsmax)
      COMPLEX*16 tmp3
      COMPLEX*16 tmpb3(nbdirsmax)
      COMPLEX*16 tmp4
      COMPLEX*16 tmpb4(nbdirsmax)
      INTEGER ad_to
      INTEGER*4 branch
      INTEGER ad_from
      INTEGER ad_to0
                  ...
                  IF (branch .EQ. 0) THEN
                    ...
                    DO i=ad_to,1,-1
                      ...
                      DO nd=1,nbdirs
                        tmpb(nd) = bb(nd, i, j)
                        bb(nd, i, j) = tmpb(nd)
                        bb(nd, k, j) = bb(nd, k, j) + CONJG(-a(i, k))*
     +                    tmpb(nd)
                        ab(nd, i, k) = ab(nd, i, k) + CONJG(-b(k, j))*
     +                    tmpb(nd)
                      ENDDO
                    ENDDO
                    CALL POPCONTROL1B(branch)
                    IF (branch .NE. 0) THEN
                      ...
                      DO nd=1,nbdirs
                        tempb0(nd) = CONJG(1.0/a(k, k))*bb(nd, k, j)
                        bb(nd, k, j) = tempb0(nd)
                        ab(nd, k, k) = ab(nd, k, k) + CONJG(-(b(k, j)/a(
     +                    k, k)))*tempb0(nd)
                      ENDDO
                    END IF
                  END IF
                ...
                IF (branch .NE. 0) THEN
                  DO i=m,1,-1
                    CALL POPCOMPLEX16(b(i, j))
                    DO nd=1,nbdirs
                      alphab(nd) = alphab(nd) + CONJG(b(i, j))*bb(nd, i
     +                  , j)
                      bb(nd, i, j) = CONJG(alpha)*bb(nd, i, j)
                    ENDDO
                  ENDDO
                END IF
              ENDDO
              ...
              DO i=m,1,-1
                ...
                IF (branch .LT. 2) THEN
                  IF (branch .EQ. 0) THEN
                    ...
                    DO nd=1,nbdirs
                      tempb(nd) = tempb0(nd)
                      ab(nd, i, i) = ab(nd, i, i) + CONJG(-(temp/a(i, i)
     +                  ))*tempb0(nd)
                    ENDDO
                  END IF
                  ...
                  DO k=ad_to0,1,-1
                    DO nd=1,nbdirs
                      ab(nd, k, i) = ab(nd, k, i) + CONJG(-b(k, j))*
     +                  tempb(nd)
                      bb(nd, k, j) = bb(nd, k, j) + CONJG(-a(k, i))*
     +                  tempb(nd)
                    ENDDO
                  ENDDO
                ELSE
                  IF (branch .EQ. 2) THEN
                    ...
                    temp0 = DCONJG(a(i, i))
                    DO nd=1,nbdirs
                      ab(nd, i, i) = ab(nd, i, i) + DCONJG(CONJG(-(temp/
     +                  temp0**2))*tempb(nd))
                      tempb(nd) = CONJG(1.0/temp0)*tempb(nd)
                    ENDDO
                  END IF
                  CALL POPINTEGER4(ad_to1)
                  DO k=ad_to1,1,-1
                    DO nd=1,nbdirs
                      ab(nd, k, i) = ab(nd, k, i) + DCONJG(CONJG(-b(k, j
     +                  ))*tempb(nd))
                      bb(nd, k, j) = bb(nd, k, j) + CONJG(-DCONJG(a(k, i
     +                  )))*tempb(nd)
                    ENDDO
                  ENDDO
                END IF
                DO nd=1,nbdirs
                  alphab(nd) = alphab(nd) + CONJG(b(i, j))*tempb(nd)
                  bb(nd, i, j) = bb(nd, i, j) + CONJG(alpha)*tempb(nd)
                ENDDO
              ENDDO
            ENDDO
          ELSE
          ...
          IF (upper) THEN
            ...
            DO j=n,1,-1
              ...
              IF (branch .EQ. 0) THEN
                ...
                CALL POPCOMPLEX16(temp)
                DO nd=1,nbdirs
                  ab(nd, j, j) = ab(nd, j, j) + CONJG(-(one/a(j, j)**2))
     +              *tempb(nd)
                ENDDO
              END IF
                ...
                IF (branch .EQ. 0) THEN
                  DO i=m,1,-1
                    ...
                    DO nd=1,nbdirs
                      tmpb1(nd) = bb(nd, i, j)
                      bb(nd, i, j) = tmpb1(nd)
                      ab(nd, k, j) = ab(nd, k, j) + CONJG(-b(i, k))*
     +                  tmpb1(nd)
                      bb(nd, i, k) = bb(nd, i, k) + CONJG(-a(k, j))*
     +                  tmpb1(nd)
                    ENDDO
                  ENDDO
                END IF
              ...
              IF (branch .NE. 0) THEN
                DO i=m,1,-1
                  CALL POPCOMPLEX16(b(i, j))
                  DO nd=1,nbdirs
                    alphab(nd) = alphab(nd) + CONJG(b(i, j))*bb(nd, i, j
     +                )
                    bb(nd, i, j) = CONJG(alpha)*bb(nd, i, j)
                  ENDDO
                ENDDO
          ...
          DO k=1,n,1
            ...
            IF (branch .EQ. 0) THEN
              DO i=m,1,-1
                ...
                DO nd=1,nbdirs
                  alphab(nd) = alphab(nd) + CONJG(b(i, k))*bb(nd, i, k)
                  bb(nd, i, k) = CONJG(alpha)*bb(nd, i, k)
                ENDDO
              ENDDO
            END IF
              ...
              IF (branch .EQ. 0) THEN
                ...
                DO i=m,1,-1
                  ...
                  DO nd=1,nbdirs
                    tmpb3(nd) = bb(nd, i, j)
                    bb(nd, i, j) = tmpb3(nd)
                    tempb(nd) = tempb(nd) + CONJG(-b(i, k))*tmpb3(nd)
                    bb(nd, i, k) = bb(nd, i, k) + CONJG(-temp)*tmpb3(nd)
                  ENDDO
                ENDDO
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  CALL POPCOMPLEX16(temp)
                  DO nd=1,nbdirs
                    ab(nd, j, k) = ab(nd, j, k) + DCONJG(tempb(nd))
                  ENDDO
                ELSE
                  CALL POPCOMPLEX16(temp)
                  DO nd=1,nbdirs
            ...
            IF (branch .NE. 0) THEN
              ...
              IF (branch .EQ. 0) THEN
                ...
                temp0 = DCONJG(a(k, k))
                DO nd=1,nbdirs
                  ab(nd, k, k) = ab(nd, k, k) + DCONJG(CONJG(-(one/temp0
     +              **2))*tempb(nd))
                ENDDO
              ELSE
                CALL POPCOMPLEX16(temp)
                DO nd=1,nbdirs
                  ab(nd, k, k) = ab(nd, k, k) + CONJG(-(one/a(k, k)**2))
     +              *tempb(nd)
                ENDDO
              END IF
            END IF
              ...
              IF (branch .EQ. 0) THEN
                ...
                DO i=m,1,-1
                  ...
                  DO nd=1,nbdirs
                    tmpb4(nd) = bb(nd, i, j)
                    bb(nd, i, j) = tmpb4(nd)
                    tempb(nd) = tempb(nd) + CONJG(-b(i, k))*tmpb4(nd)
                    bb(nd, i, k) = bb(nd, i, k) + CONJG(-temp)*tmpb4(nd)
                  ENDDO
                ENDDO
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
