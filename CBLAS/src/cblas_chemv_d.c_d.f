...
C
C  Differentiation of chemv in forward (tangent) mode:
...
C>
C  =====================================================================
      SUBROUTINE CHEMV_D(uplo, n, alpha, alphad, a, ad, lda, x, xd, incx
            ...
            IF (incx .EQ. 1 .AND. incy .EQ. 1) THEN
              DO j=1,n
                ...
                DO i=1,j-1
                  ...
                  y(i) = y(i) + temp1*a(i, j)
                  temp = CONJG(a(i, j))
                  temp2d = temp2d + x(i)*CONJG(ad(i, j)) + temp*xd(i)
                  temp2 = temp2 + temp*x(i)
                ENDDO
                temp0 = REAL(a(j, j))
                yd(j) = yd(j) + temp0*temp1d + temp1*REAL(ad(j, j)) + 
            ...
            ELSE
              ...
              DO j=1,n
                ...
                DO i=1,j-1
                  ...
                  temp = CONJG(a(i, j))
                  temp2d = temp2d + x(ix)*CONJG(ad(i, j)) + temp*xd(ix)
                  temp2 = temp2 + temp*x(ix)
                  ix = ix + incx
                  iy = iy + incy
                ENDDO
                temp0 = REAL(a(j, j))
                yd(jy) = yd(jy) + temp0*temp1d + temp1*REAL(ad(j, j)) + 
     +            temp2*alphad + alpha*temp2d
