...
C
C  Differentiation of dtrmv in reverse (adjoint) mode (with options multiDirectional):
...
C  ===========
C
C       SUBROUTINE DTRMV(UPLO,TRANS,DIAG,N,A,LDA,X,INCX)
C
C       .. Scalar Arguments ..
...
C>
C  =====================================================================
      SUBROUTINE DTRMV_BV(uplo, trans, diag, n, a, ab, lda, x, xb, incx
     +                    , nbdirs)
      IMPLICIT NONE
              ...
              IF (incx .EQ. 1) THEN
                ...
                DO 100 j=n,1,-1
                  ...
                  IF (branch .EQ. 0) THEN
                    CALL POPREAL8(x(j))
                    DO nd=1,nbdirs
                      ab(nd, j, j) = ab(nd, j, j) + x(j)*xb(nd, j)
                      xb(nd, j) = a(j, j)*xb(nd, j)
                    ENDDO
              ...
              ELSE
                ...
                DO 110 j=n,1,-1
                  ...
                  IF (branch .EQ. 0) THEN
                    CALL POPREAL8(x(jx))
                    DO nd=1,nbdirs
                      ab(nd, j, j) = ab(nd, j, j) + x(jx)*xb(nd, jx)
                      xb(nd, jx) = a(j, j)*xb(nd, jx)
                    ENDDO
                  ELSE IF (branch .NE. 1) THEN
                    GOTO 110
                  END IF
