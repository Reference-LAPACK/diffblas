...
C
C  Differentiation of dtrmm in reverse (adjoint) mode (with options multiDirectional):
...
C  ===========
C
C       SUBROUTINE DTRMM(SIDE,UPLO,TRANSA,DIAG,M,N,ALPHA,A,LDA,B,LDB)
C
C       .. Scalar Arguments ..
...
C> \param[in] A
C> \verbatim
C>           A is DOUBLE PRECISION array, dimension ( LDA, k ), where k is m
...
C>
C  =====================================================================
      SUBROUTINE DTRMM_BV(side, uplo, transa, diag, m, n, alpha, alphab
                ...
                DO k=m,1,-1
                  ...
                  IF (branch .EQ. 0) THEN
                    ...
                    IF (branch .EQ. 0) THEN
                      CALL POPREAL8(temp)
                      DO nd=1,nbdirs
                        ab(nd, k, k) = ab(nd, k, k) + temp*tempb(nd)
                        tempb(nd) = a(k, k)*tempb(nd)
                      ENDDO
                    END IF
                    ...
                    DO i=ad_to,1,-1
                      CALL POPREAL8(b(i, j))
                      DO nd=1,nbdirs
                        tempb(nd) = tempb(nd) + a(i, k)*bb(nd, i, j)
                        ab(nd, i, k) = ab(nd, i, k) + temp*bb(nd, i, j)
                      ENDDO
                    ENDDO
                    CALL POPREAL8(temp)
                    DO nd=1,nbdirs
                      alphab(nd) = alphab(nd) + b(k, j)*tempb(nd)
                      bb(nd, k, j) = bb(nd, k, j) + alpha*tempb(nd)
                    ENDDO
                  END IF
                ENDDO
              ENDDO
                ...
                DO k=1,m,1
                  ...
                  IF (branch .EQ. 0) THEN
                    ...
                    IF (branch .NE. 0) THEN
                      CALL POPREAL8(b(k, j))
                      DO nd=1,nbdirs
                        ab(nd, k, k) = ab(nd, k, k) + b(k, j)*bb(nd, k, 
     +                    j)
                        bb(nd, k, j) = a(k, k)*bb(nd, k, j)
                      ENDDO
                    END IF
                    CALL POPREAL8(b(k, j))
                    CALL POPREAL8(temp)
...
C
            ...
            DO j=n,1,-1
              DO i=1,m,1
                CALL POPREAL8(b(i, j))
                DO nd=1,nbdirs
                  alphab(nd) = alphab(nd) + temp*bb(nd, i, j)
                  tempb(nd) = alpha*bb(nd, i, j)
                  bb(nd, i, j) = 0.D0
                ENDDO
                ...
                IF (branch .NE. 0) THEN
                  CALL POPREAL8(temp)
                  DO nd=1,nbdirs
                    ab(nd, i, i) = ab(nd, i, i) + temp*tempb(nd)
                    tempb(nd) = a(i, i)*tempb(nd)
                  ENDDO
                END IF
                CALL POPREAL8(temp)
                DO nd=1,nbdirs
          ...
          IF (upper) THEN
            ...
            DO j=1,n,1
                ...
                IF (branch .EQ. 0) THEN
                  ...
                  DO i=m,1,-1
                    ...
                    DO nd=1,nbdirs
                      tmpb(nd) = bb(nd, i, j)
                      bb(nd, i, j) = tmpb(nd)
                      tempb(nd) = tempb(nd) + b(i, k)*tmpb(nd)
                      bb(nd, i, k) = bb(nd, i, k) + temp*tmpb(nd)
                    ENDDO
                  ENDDO
                  CALL POPREAL8(temp)
                  DO nd=1,nbdirs
                    alphab(nd) = alphab(nd) + a(k, j)*tempb(nd)
                    ab(nd, k, j) = ab(nd, k, j) + alpha*tempb(nd)
                  ENDDO
                END IF
              ENDDO
              DO nd=1,nbdirsmax
              ...
              IF (branch .NE. 0) THEN
                CALL POPREAL8(temp)
                DO nd=1,nbdirs
                  ab(nd, j, j) = ab(nd, j, j) + temp*tempb(nd)
                  tempb(nd) = a(j, j)*tempb(nd)
                ENDDO
              END IF
              CALL POPREAL8(temp)
              DO nd=1,nbdirs
                ...
                IF (branch .EQ. 0) THEN
                  ...
                  DO i=m,1,-1
                    ...
                    DO nd=1,nbdirs
                      tmpb0(nd) = bb(nd, i, j)
                      bb(nd, i, j) = tmpb0(nd)
                      tempb(nd) = tempb(nd) + b(i, k)*tmpb0(nd)
                      bb(nd, i, k) = bb(nd, i, k) + temp*tmpb0(nd)
                    ENDDO
                  ENDDO
                  CALL POPREAL8(temp)
                  DO nd=1,nbdirs
              ...
              IF (branch .EQ. 0) THEN
                ...
                DO i=m,1,-1
                  ...
                  DO nd=1,nbdirs
                    tmpb1(nd) = bb(nd, i, j)
                    bb(nd, i, j) = tmpb1(nd)
                    tempb(nd) = tempb(nd) + b(i, k)*tmpb1(nd)
                    bb(nd, i, k) = bb(nd, i, k) + temp*tmpb1(nd)
                  ENDDO
                ENDDO
                CALL POPREAL8(temp)
                DO nd=1,nbdirs
                  alphab(nd) = alphab(nd) + a(j, k)*tempb(nd)
                  ab(nd, j, k) = ab(nd, j, k) + alpha*tempb(nd)
                ENDDO
              END IF
            ENDDO
          ENDDO
              ...
              IF (branch .EQ. 0) THEN
                ...
                DO i=m,1,-1
                  ...
                  DO nd=1,nbdirs
                    tmpb2(nd) = bb(nd, i, j)
                    bb(nd, i, j) = tmpb2(nd)
                    tempb(nd) = tempb(nd) + b(i, k)*tmpb2(nd)
                    bb(nd, i, k) = bb(nd, i, k) + temp*tmpb2(nd)
                  ENDDO
                ENDDO
                CALL POPREAL8(temp)
                DO nd=1,nbdirs
