...
C
C  Differentiation of dspr2 in forward (tangent) mode (with options multiDirectional):
C   variations   of useful results: ap
C   with respect to varying inputs: alpha x y
C> \brief \b DSPR2
C
C  =========== DOCUMENTATION ===========
C
...
C>
C  =====================================================================
      SUBROUTINE DSPR2_DV(uplo, n, alpha, alphad, x, xd, incx, y, yd, 
     +                    incy, ap, apd, nbdirs)
      IMPLICIT NONE
...
C
        DO ii1=1,ISIZE1OFap
          DO nd=1,nbdirsmax
C     FIXED: Removed zeroing of apd - should accumulate from input seed
          ENDDO
        ENDDO
        RETURN
      ELSE
...
C     FIXED: Removed zeroing of apd - should accumulate from input seed
            ...
            DO j=1,n
              IF (x(j) .NE. zero .OR. y(j) .NE. zero) THEN
                DO nd=1,nbdirs
                  temp1d(nd) = y(j)*alphad(nd) + alpha*yd(nd, j)
                  temp2d(nd) = x(j)*alphad(nd) + alpha*xd(nd, j)
                ENDDO
                temp1 = alpha*y(j)
                temp2 = alpha*x(j)
                k = kk
                DO i=1,j
                  DO nd=1,nbdirs
                    apd(nd, k) = apd(nd, k) + temp1*xd(nd, i) + x(i)*
     +                temp1d(nd) + temp2*yd(nd, i) + y(i)*temp2d(nd)
                  ENDDO
...
C     FIXED: Removed zeroing of apd - should accumulate from input seed
            ...
            DO j=1,n
              IF (x(jx) .NE. zero .OR. y(jy) .NE. zero) THEN
                DO nd=1,nbdirs
                  temp1d(nd) = y(jy)*alphad(nd) + alpha*yd(nd, jy)
                  temp2d(nd) = x(jx)*alphad(nd) + alpha*xd(nd, jx)
                ENDDO
                temp1 = alpha*y(jy)
                temp2 = alpha*x(jx)
                ix = kx
                ...
                DO k=kk,kk+j-1
                  DO nd=1,nbdirs
                    apd(nd, k) = apd(nd, k) + temp1*xd(nd, ix) + x(ix)*
     +                temp1d(nd) + temp2*yd(nd, iy) + y(iy)*temp2d(nd)
                  ENDDO
