...
C
C  Differentiation of dspr in forward (tangent) mode (with options multiDirectional):
C   variations   of useful results: ap
C   with respect to varying inputs: alpha x
C> \brief \b DSPR
C
C  =========== DOCUMENTATION ===========
C
...
C>
C  =====================================================================
      SUBROUTINE DSPR_DV(uplo, n, alpha, alphad, x, xd, incx, ap, apd, 
     +                   nbdirs)
      IMPLICIT NONE
...
C     FIXED: Removed zeroing of apd - should accumulate from input seed
            ...
            DO j=1,n
              IF (x(j) .NE. zero) THEN
                ...
                DO i=1,j
                  DO nd=1,nbdirs
                    apd(nd, k) = apd(nd, k) + temp*xd(nd, i) + x(i)*
     +                tempd(nd)
                  ENDDO
                  ap(k) = ap(k) + x(i)*temp
                  k = k + 1
...
C     FIXED: Removed zeroing of apd - should accumulate from input seed
            ...
            DO j=1,n
              IF (x(jx) .NE. zero) THEN
                ...
                DO k=kk,kk+j-1
                  DO nd=1,nbdirs
                    apd(nd, k) = apd(nd, k) + temp*xd(nd, ix) + x(ix)*
     +                tempd(nd)
                  ENDDO
                  ap(k) = ap(k) + x(ix)*temp
                  ix = ix + incx
...
C     FIXED: Removed zeroing of apd - should accumulate from input seed
          ...
          DO j=1,n
            IF (x(j) .NE. zero) THEN
              ...
              DO i=j,n
                DO nd=1,nbdirs
                  apd(nd, k) = apd(nd, k) + temp*xd(nd, i) + x(i)*tempd(
     +              nd)
                ENDDO
                ap(k) = ap(k) + x(i)*temp
                k = k + 1
