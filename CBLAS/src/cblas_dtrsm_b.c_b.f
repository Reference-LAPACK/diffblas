...
C>
C  =====================================================================
      SUBROUTINE DTRSM_B(side, uplo, transa, diag, m, n, alpha, alphab, 
     +                   a, ab, lda, b, bb, ldb)
      IMPLICIT NONE
...
C     .. Array Arguments ..
      ...
      DOUBLE PRECISION ab(lda, *), bb(ldb, *)
      EXTERNAL LSAME
C     ..
C
C  =====================================================================
                ...
                DO k=1,m,1
                  ...
                  IF (branch .EQ. 0) THEN
                    ...
                    DO i=ad_to,1,-1
                      ...
                      tmpb = bb(i, j)
                      bb(i, j) = tmpb
                      bb(k, j) = bb(k, j) - a(i, k)*tmpb
                      ab(i, k) = ab(i, k) - b(k, j)*tmpb
                    ENDDO
                    CALL POPCONTROL1B(branch)
                    IF (branch .NE. 0) THEN
                      CALL POPREAL8(b(k, j))
                      tempb0 = bb(k, j)/a(k, k)
                      bb(k, j) = tempb0
                      ab(k, k) = ab(k, k) - b(k, j)*tempb0/a(k, k)
                    END IF
                  END IF
                ENDDO
                CALL POPCONTROL1B(branch)
                ...
                DO k=m,1,-1
                  ...
                  IF (branch .EQ. 0) THEN
                    ...
                    DO i=m,ad_from,-1
                      ...
                      tmpb0 = bb(i, j)
                      bb(i, j) = tmpb0
                      bb(k, j) = bb(k, j) - a(i, k)*tmpb0
                      ab(i, k) = ab(i, k) - b(k, j)*tmpb0
                    ENDDO
                    CALL POPCONTROL1B(branch)
                    IF (branch .NE. 0) THEN
                      CALL POPREAL8(b(k, j))
            ...
            DO j=n,1,-1
              DO i=m,1,-1
                ...
                IF (branch .EQ. 0) THEN
                  ...
                  tempb0 = tempb/a(i, i)
                  tempb = tempb0
                  ab(i, i) = ab(i, i) - temp*tempb0/a(i, i)
                END IF
                ...
                ENDDO
                alphab = alphab + b(i, j)*tempb
                bb(i, j) = bb(i, j) + alpha*tempb
              ENDDO
            ENDDO
          ELSE
            DO j=1,n
          ...
          IF (upper) THEN
            ...
            DO j=n,1,-1
              ...
              IF (branch .EQ. 0) THEN
                ...
                ENDDO
                CALL POPREAL8(temp)
                ab(j, j) = ab(j, j) - one*tempb/a(j, j)**2
              END IF
              ...
              DO k=ad_to1,1,-1
                ...
                IF (branch .EQ. 0) THEN
                  DO i=m,1,-1
                    ...
                    tmpb1 = bb(i, j)
                    bb(i, j) = tmpb1
                    ab(k, j) = ab(k, j) - b(i, k)*tmpb1
                    bb(i, k) = bb(i, k) - a(k, j)*tmpb1
                  ENDDO
                END IF
              ENDDO
              CALL POPCONTROL1B(branch)
              ...
              DO k=n,ad_from1,-1
                ...
                IF (branch .EQ. 0) THEN
                  DO i=m,1,-1
                    ...
                    tmpb2 = bb(i, j)
                    bb(i, j) = tmpb2
                    ab(k, j) = ab(k, j) - b(i, k)*tmpb2
                    bb(i, k) = bb(i, k) - a(k, j)*tmpb2
                  ENDDO
                END IF
              ENDDO
              CALL POPCONTROL1B(branch)
...
C
          ...
          DO k=1,n,1
            ...
            DO j=ad_to2,1,-1
              ...
              IF (branch .EQ. 0) THEN
                ...
                DO i=m,1,-1
                  ...
                  tmpb3 = bb(i, j)
                  bb(i, j) = tmpb3
                  tempb = tempb - b(i, k)*tmpb3
                  bb(i, k) = bb(i, k) - temp*tmpb3
                ENDDO
                CALL POPREAL8(temp)
                ab(j, k) = ab(j, k) + tempb
              END IF
            ...
            IF (branch .NE. 0) THEN
              ...
              ENDDO
              CALL POPREAL8(temp)
              ab(k, k) = ab(k, k) - one*tempb/a(k, k)**2
            END IF
          ENDDO
        ELSE
          DO k=1,n
            ...
            DO j=n,ad_from2,-1
              ...
              IF (branch .EQ. 0) THEN
                ...
                DO i=m,1,-1
                  ...
                  tmpb4 = bb(i, j)
                  bb(i, j) = tmpb4
                  tempb = tempb - b(i, k)*tmpb4
                  bb(i, k) = bb(i, k) - temp*tmpb4
                ENDDO
                CALL POPREAL8(temp)
                ab(j, k) = ab(j, k) + tempb
              END IF
