/* Test program for cblas_dspr2 vector reverse mode (VJP verification, generic, loop over directions) */
...
int main(void) {
    ...
    double A[PACKED_SIZE], A_orig[PACKED_SIZE], A_dir[PACKED_SIZE];
    double A_b[PACKED_SIZE][NBDirsMax], A_b_orig[PACKED_SIZE][NBDirsMax];
    double A_plus[PACKED_SIZE], A_minus[PACKED_SIZE];
    ...
    for (i = 0; i < PACKED_SIZE; i++) { A[i] = (rand()/(double)RAND_MAX)*2.0 - 1.0; }
    for (i = 0; i < PACKED_SIZE; i++) { A_dir[i] = (rand()/(double)RAND_MAX)*2.0 - 1.0; }
    ...
    for (i = 0; i < PACKED_SIZE; i++) for (j = 0; j < NBDirsMax; j++) { A_b[i][j] = (rand()/(double)RAND_MAX)*2.0 - 1.0; A_b_orig[i][j] = A_b[i][j]; }
    ...
    cblas_dspr2_bv(layout, uplo, n, alpha, &alpha_b, X, X_b, incX, Y, Y_b, incY, A, A_b, nbdirs);
    ...
    for (idir = 0; idir < nbdirs; idir++) {
        ...
        memcpy(A, A_orig, sizeof(A[0])*(PACKED_SIZE));
        ...
        vjp_fd = 0.0;
        {
            ...
            for (i = 0; i < PACKED_SIZE; i++) temp_products[i] = A_b_orig[i][idir] * ((A_plus[i] - A_minus[i]) / (2.0*h));
        ...
        {
            double temp_products[PACKED_SIZE];
            for (i = 0; i < PACKED_SIZE; i++) temp_products[i] = A_dir[i] * A_b[i][idir];
            qsort(temp_products, (size_t)PACKED_SIZE, sizeof(double), compare_abs_d);
