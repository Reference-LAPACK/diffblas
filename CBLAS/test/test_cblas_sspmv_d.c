/* Test program for cblas_sspmv differentiation */
...
extern void cblas_sspmv_d(const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo, const CBLAS_INT N, const float alpha, float alpha_d, const float *AP, float *AP_d, const float *X, float *X_d, const CBLAS_INT incX, const float beta, float beta_d, float *Y, float *Y_d, const CBLAS_INT incY);
