/* Test program for cblas_cdotc_sub reverse mode (VJP verification, generic) */
...
extern void cblas_cdotc_sub_b(const CBLAS_INT N, const void *X, void *X_b, const CBLAS_INT incX, const void *Y, void *Y_b, const CBLAS_INT incY, void *dotc, void *dotc_b);
...
int main(void) {
    ...
    float complex dotc[1], dotc_b[1], dotc_orig[1], dotc_dir[1];
    float complex dotc_plus[1], dotc_minus[1], dotc_central_diff[1], dotc_b_orig[1];
    ...
    cblas_cdotc_sub_b(n, X, X_b, incX, Y, Y_b, incY, dotc, dotc_b);
