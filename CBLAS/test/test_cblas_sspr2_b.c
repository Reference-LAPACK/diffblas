/* Test program for cblas_sspr2 reverse mode (VJP verification, generic) */
...
extern void cblas_sspr2_b(const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo, const CBLAS_INT N, const float alpha, float *alpha_b, const float *X, float *X_b, const CBLAS_INT incX, const float *Y, float *Y_b, const CBLAS_INT incY, float *A, float *A_b);
...
int main(void) {
    ...
    float Y[MAX_SIZE], Y_b[MAX_SIZE], Y_orig[MAX_SIZE], Y_dir[MAX_SIZE];
    float A[PACKED_SIZE], A_b[PACKED_SIZE], A_orig[PACKED_SIZE], A_dir[PACKED_SIZE];
    float A_plus[PACKED_SIZE], A_minus[PACKED_SIZE], A_central_diff[PACKED_SIZE], A_b_orig[PACKED_SIZE];
    ...
    cblas_sspr2_b(layout, uplo, n, alpha, &alpha_b, X, X_b, incX, Y, Y_b, incY, A, A_b);
