/* Test program for cblas_strsv forward vector (dv) differentiation */
...
extern void cblas_strsv_dv(CBLAS_LAYOUT layout, CBLAS_UPLO Uplo, CBLAS_TRANSPOSE TransA, CBLAS_DIAG Diag, CBLAS_INT N, const float *A, float (*Ad)[NBDirsMax], CBLAS_INT lda, float *X, float (*Xd)[NBDirsMax], CBLAS_INT incX, int nbdirs);
