/* Test program for cblas_chemm differentiation */
...
extern void cblas_chemm_d(const CBLAS_LAYOUT layout, const CBLAS_SIDE Side, const CBLAS_UPLO Uplo, const CBLAS_INT M, const CBLAS_INT N, const void *alpha, void *alpha_d, const void *A, void *A_d, const CBLAS_INT lda, const void *B, void *B_d, const CBLAS_INT ldb, const void *beta, void *beta_d, void *C, void *C_d, const CBLAS_INT ldc);
...
int main(void) {
    ...
    /* A: Hermitian (match BLAS/test) */
    for (i = 0; i < MAX_SIZE; i++) {
        A_d[i * MAX_SIZE + i] = ((float)rand() / RAND_MAX) * 2.0f - 1.0f;  /* real diagonal */
        for (j = i + 1; j < MAX_SIZE; j++) {
