/* Test program for cblas_ssyrk reverse mode (VJP verification, generic) */
...
extern void cblas_ssyrk_b(const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo, const CBLAS_TRANSPOSE Trans, const CBLAS_INT N, const CBLAS_INT K, const float alpha, float *alpha_b, const float *A, float *A_b, const CBLAS_INT lda, const float beta, float *beta_b, float *C, float *C_b, const CBLAS_INT ldc);
...
int main(void) {
    ...
    cblas_ssyrk_b(layout, uplo, trans, n, k, alpha, &alpha_b, A, A_b, lda, beta, &beta_b, C, C_b, ldc);
