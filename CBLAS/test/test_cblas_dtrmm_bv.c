/* Test program for cblas_dtrmm vector reverse mode (VJP verification, generic, loop over directions) */
...
int main(void) {
    ...
    double B_plus[MAX_SIZE*MAX_SIZE], B_minus[MAX_SIZE*MAX_SIZE];
    ...
    for (i = 0; i < MAX_SIZE*MAX_SIZE; i++) for (j = 0; j < NBDirsMax; j++) { B_b[i][j] = (rand()/(double)RAND_MAX)*2.0 - 1.0; B_b_orig[i][j] = B_b[i][j]; }
    for (j = 0; j < NBDirsMax; j++) alpha_b[j] = 0.0;
    for (i = 0; i < MAX_SIZE*MAX_SIZE; i++) for (j = 0; j < NBDirsMax; j++) A_b[i][j] = 0.0;

    cblas_dtrmm_bv(layout, side, uplo, transa, diag, m, n, alpha, &alpha_b, A, &A_b[0][0], lda, B, &B_b[0][0], ldb, nbdirs);
    ...
    for (idir = 0; idir < nbdirs; idir++) {
        ...
        vjp_fd = 0.0;
        {
            ...
            for (i = 0; i < MAX_SIZE*MAX_SIZE; i++) temp_products[i] = B_b_orig[i][idir] * ((B_plus[i] - B_minus[i]) / (2.0*h));
