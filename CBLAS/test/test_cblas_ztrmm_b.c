/* Test program for cblas_ztrmm reverse mode (VJP verification, generic) */
...
extern void cblas_ztrmm(const CBLAS_LAYOUT layout, const CBLAS_SIDE Side, const CBLAS_UPLO Uplo, const CBLAS_TRANSPOSE TransA, const CBLAS_DIAG Diag, const CBLAS_INT M, const CBLAS_INT N, const void *alpha, const void *A, const CBLAS_INT lda, void *B, const CBLAS_INT ldb);
extern void cblas_ztrmm_b(const CBLAS_LAYOUT layout, const CBLAS_SIDE Side, const CBLAS_UPLO Uplo, const CBLAS_TRANSPOSE TransA, const CBLAS_DIAG Diag, const CBLAS_INT M, const CBLAS_INT N, const void *alpha, void *alpha_b, const void *A, void *A_b, const CBLAS_INT lda, void *B, void *B_b, const CBLAS_INT ldb);
...
int main(void) {
    ...
    double complex B_plus[MAX_SIZE*MAX_SIZE], B_minus[MAX_SIZE*MAX_SIZE], B_central_diff[MAX_SIZE*MAX_SIZE], B_b_orig[MAX_SIZE*MAX_SIZE];
    ...
    for (i = 0; i < MAX_SIZE*MAX_SIZE; i++) { B_b[i] = (rand()/(double)RAND_MAX)*2.0 - 1.0 + I*((rand()/(double)RAND_MAX)*2.0 - 1.0); B_b_orig[i] = B_b[i]; }
    for (i = 0; i < MAX_SIZE; i++) alpha_b[i] = 0.0;
    for (i = 0; i < MAX_SIZE*MAX_SIZE; i++) A_b[i] = 0.0;

    cblas_ztrmm_b(layout, side, uplo, transa, diag, m, n, &alpha, alpha_b, A, A_b, lda, B, B_b, ldb);
    ...
    vjp_fd = 0.0;
    {
        ...
        for (i = 0; i < MAX_SIZE*MAX_SIZE; i++) temp_products[i] = creal(conj(B_b_orig[i]) * ((B_plus[i] - B_minus[i]) / (2.0*h)));
