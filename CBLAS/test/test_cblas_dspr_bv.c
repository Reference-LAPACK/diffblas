/* Test program for cblas_dspr vector reverse mode (VJP verification, generic, loop over directions) */
...
int main(void) {
    ...
    double Ap[PACKED_SIZE], Ap_orig[PACKED_SIZE], Ap_dir[PACKED_SIZE];
    double Ap_b[PACKED_SIZE][NBDirsMax], Ap_b_orig[PACKED_SIZE][NBDirsMax];
    double Ap_plus[PACKED_SIZE], Ap_minus[PACKED_SIZE];
    ...
    cblas_dspr_bv(layout, uplo, n, alpha, &alpha_b, X, X_b, incX, Ap, Ap_b, nbdirs);
