/* Test program for cblas_cgbmv vector reverse mode (VJP verification, generic, loop over directions) */
...
extern void cblas_cgbmv(const CBLAS_LAYOUT layout, const CBLAS_TRANSPOSE TransA, const CBLAS_INT M, const CBLAS_INT N, const CBLAS_INT KL, const CBLAS_INT KU, const void *alpha, const void *A, const CBLAS_INT lda, const void *X, const CBLAS_INT incX, const void *beta, void *Y, const CBLAS_INT incY);
...
int main(void) {
    ...
    float complex Y_plus[MAX_SIZE], Y_minus[MAX_SIZE];
    ...
    for (j = 0; j < MAX_SIZE; j++) {
        ...
        for (i = 0; i < band_rows; i++) {
            A[i + j * MAX_SIZE] = ((float)rand() / RAND_MAX) * 2.0f - 1.0f + I * (((float)rand() / RAND_MAX) * 2.0f - 1.0f);
        }
    }
    /* A: general band storage (KL+KU+1) x N (match BLAS/test) */
    ...
    for (j = 0; j < MAX_SIZE; j++) {
        int band_rows = KL + KU + 1;
        for (i = 0; i < band_rows; i++) {
            A_dir[i + j * MAX_SIZE] = ((float)rand() / RAND_MAX) * 2.0f - 1.0f + I * (((float)rand() / RAND_MAX) * 2.0f - 1.0f);
        }
    }
    ...
    cblas_cgbmv_bv(layout, transa, m, n, KL, KU, &alpha, (void*)&alpha_b, A, (void*)A_b, lda, X, (void*)X_b, incX, &beta, (void*)&beta_b, Y, (void*)Y_b, incY, nbdirs);
    ...
    for (idir = 0; idir < nbdirs; idir++) {
        ...
        cblas_cgbmv(layout, transa, m, n, KL, KU, &alpha, A, lda, X, incX, &beta, Y, incY);
