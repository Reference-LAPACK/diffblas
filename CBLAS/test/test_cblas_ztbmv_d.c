/* Test program for cblas_ztbmv differentiation */
...
extern void cblas_ztbmv_d(const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo, const CBLAS_TRANSPOSE TransA, const CBLAS_DIAG Diag, const CBLAS_INT N, const CBLAS_INT K, const void *A, void *A_d, const CBLAS_INT lda, void *X, void *X_d, const CBLAS_INT incX);
...
int main(void) {
    ...
    for (j = 0; j < MAX_SIZE; j++) {
        ...
        for (i = first_row; i <= j && i < MAX_SIZE; i++) {
            int band_row = K + i - j;  /* BLAS: a(i,j) -> A(band_row, j) */
            A_d[band_row + j * MAX_SIZE] = ((double)rand() / RAND_MAX) * 2.0 - 1.0 + I * (((double)rand() / RAND_MAX) * 2.0 - 1.0);
        }
    }
    for (i = 0; i < MAX_SIZE; i++) {
