/* Test program for cblas_ctpmv differentiation */
...
extern void cblas_ctpmv_d(const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo, const CBLAS_TRANSPOSE TransA, const CBLAS_DIAG Diag, const CBLAS_INT N, const void *Ap, void *Ap_d, void *X, void *X_d, const CBLAS_INT incX);
...
int main(void) {
    ...
    float complex Ap[PACKED_SIZE];
    float complex Ap_d[PACKED_SIZE];  /* Derivative seeds */
    float complex Ap_d_orig[PACKED_SIZE];
    float complex Ap_orig[PACKED_SIZE];
    ...
    for (j = 0; j < MAX_SIZE; j++) {
        for (i = 0; i <= j; i++) {
            Ap[j * (j + 1) / 2 + i] = ((float)rand() / RAND_MAX) * 2.0f - 1.0f + I * (((float)rand() / RAND_MAX) * 2.0f - 1.0f);
        }
    }
    ...
    for (i = 0; i < PACKED_SIZE; i++) {
        Ap_d[i] = ((float)rand() / RAND_MAX) * 2.0f - 1.0f + I * (((float)rand() / RAND_MAX) * 2.0f - 1.0f);
    }
    for (i = 0; i < MAX_SIZE; i++) {
