/* Test program for cblas_strsm forward vector (dv) differentiation */
...
extern void cblas_strsm_dv(CBLAS_LAYOUT layout, CBLAS_SIDE Side, CBLAS_UPLO Uplo, CBLAS_TRANSPOSE TransA, CBLAS_DIAG Diag, CBLAS_INT M, CBLAS_INT N, float alpha, float alphad[NBDirsMax], const float *A, float (*Ad)[NBDirsMax], CBLAS_INT lda, float *B, float (*Bd)[NBDirsMax], CBLAS_INT ldb, int nbdirs);
...
int main(void) {
    ...
    float B_output[MAX_SIZE * MAX_SIZE];
    float B_ad_output[MAX_SIZE * MAX_SIZE];
    float B_forward[MAX_SIZE * MAX_SIZE], B_backward[MAX_SIZE * MAX_SIZE];
    ...
    /* Verify AD primal output matches original (same as _d) */
    {
        ...
        for (i = 0; i < MAX_SIZE * MAX_SIZE; i++) {
            float diff = fabs(B_ad_output[i] - B_output[i]);
