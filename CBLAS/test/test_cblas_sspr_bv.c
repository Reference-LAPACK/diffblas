/* Test program for cblas_sspr vector reverse mode (VJP verification, generic, loop over directions) */
...
int main(void) {
    ...
    float Ap_plus[PACKED_SIZE], Ap_minus[PACKED_SIZE];
    ...
    for (i = 0; i < PACKED_SIZE; i++) for (j = 0; j < NBDirsMax; j++) { Ap_b[i][j] = (rand()/(double)RAND_MAX)*2.0 - 1.0; Ap_b_orig[i][j] = Ap_b[i][j]; }
    for (j = 0; j < NBDirsMax; j++) alpha_b[j] = 0.0f;
    for (i = 0; i < MAX_SIZE; i++) for (j = 0; j < NBDirsMax; j++) X_b[i][j] = 0.0f;

    cblas_sspr_bv(layout, uplo, n, alpha, &alpha_b, X, X_b, incX, Ap, Ap_b, nbdirs);
    ...
    for (idir = 0; idir < nbdirs; idir++) {
        ...
        vjp_fd = 0.0f;
        {
            ...
            for (i = 0; i < PACKED_SIZE; i++) temp_products[i] = Ap_b_orig[i][idir] * ((Ap_plus[i] - Ap_minus[i]) / (2.0*h));
