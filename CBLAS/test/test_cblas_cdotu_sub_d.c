/* Test program for cblas_cdotu_sub differentiation */
...
extern void cblas_cdotu_sub_d(const CBLAS_INT N, const void *X, void *X_d, const CBLAS_INT incX, const void *Y, void *Y_d, const CBLAS_INT incY, void *dotu, void *dotu_d);
...
int main(void) {
    ...
    CBLAS_INT incY;
    float complex dotu[MAX_SIZE];
    float complex dotu_d[MAX_SIZE];  /* Derivative seeds */
    float complex dotu_d_orig[MAX_SIZE];  /* Save derivative seeds for finite differences */
    float complex dotu_orig[MAX_SIZE];
    ...
    for (i = 0; i < MAX_SIZE; i++) {
        dotu[i] = ((float)rand() / RAND_MAX) * 2.0f - 1.0f + I * (((float)rand() / RAND_MAX) * 2.0f - 1.0f);
    }
    ...
    for (i = 0; i < MAX_SIZE; i++) {
        dotu_d[i] = ((float)rand() / RAND_MAX) * 2.0f - 1.0f + I * (((float)rand() / RAND_MAX) * 2.0f - 1.0f);
    }
    ...
    memcpy(dotu_d_orig, dotu_d, sizeof(dotu_d));
    ...
    memcpy(dotu_orig, dotu, sizeof(dotu));
