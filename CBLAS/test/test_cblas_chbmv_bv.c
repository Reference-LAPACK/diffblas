/* Test program for cblas_chbmv vector reverse mode (VJP verification, generic, loop over directions) */
...
extern void cblas_chbmv(const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo, const CBLAS_INT N, const CBLAS_INT K, const void *alpha, const void *A, const CBLAS_INT lda, const void *X, const CBLAS_INT incX, const void *beta, void *Y, const CBLAS_INT incY);
...
int main(void) {
    ...
    for (j = 0; j < MAX_SIZE; j++) {
        ...
        for (i = first_row; i <= j && i < MAX_SIZE; i++) {
            int band_row = k + i - j;  /* BLAS: a(i,j) -> A(band_row, j) */
            if (i == j) {  /* diagonal: real for Hermitian */
                A_dir[band_row + j * MAX_SIZE] = ((float)rand() / RAND_MAX) * 2.0f - 1.0f;
            } else {
                A_dir[band_row + j * MAX_SIZE] = ((float)rand() / RAND_MAX) * 2.0f - 1.0f + I * (((float)rand() / RAND_MAX) * 2.0f - 1.0f);
            }
        }
    }
    ...
    cblas_chbmv_bv(layout, uplo, n, k, &alpha, (void*)&alpha_b, A, (void*)A_b, lda, X, (void*)X_b, incX, &beta, (void*)&beta_b, Y, (void*)Y_b, incY, nbdirs);
