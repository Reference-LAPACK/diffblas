/* Test program for cblas_snrm2 vector reverse (bv) differentiation (scalar result) */
...
extern float cblas_snrm2(CBLAS_INT N, const float *X, CBLAS_INT incX);
extern void cblas_snrm2_bv(CBLAS_INT N, const float *X, float (*X_b)[NBDirsMax], CBLAS_INT incX, float result_b[NBDirsMax], int nbdirs);
...
int main(void) {
    ...
    cblas_snrm2_bv(N, X, X_b, incX, result_b, nbdirs);
    ...
    for (idir = 0; idir < nbdirs; idir++) {
        ...
        float result_forward = cblas_snrm2(
        N,
        X,
        incX
        );
        ...
        /* Backward */
        for (i = 0; i < MAX_SIZE; i++) X[i] = X_orig[i] - h * X_dir[i];
        float result_backward = cblas_snrm2(
        N,
        X,
        incX
        );
