/* Test program for cblas_sgbmv reverse mode (VJP verification, generic) */
...
extern void cblas_sgbmv_b(const CBLAS_LAYOUT layout, const CBLAS_TRANSPOSE TransA, const CBLAS_INT M, const CBLAS_INT N, const CBLAS_INT KL, const CBLAS_INT KU, const float alpha, float *alpha_b, const float *A, float *A_b, const CBLAS_INT lda, const float *X, float *X_b, const CBLAS_INT incX, const float beta, float *beta_b, float *Y, float *Y_b, const CBLAS_INT incY);
...
int main(void) {
    ...
    cblas_sgbmv_b(layout, transa, m, n, KL, KU, alpha, &alpha_b, A, A_b, lda, X, X_b, incX, beta, &beta_b, Y, Y_b, incY);
