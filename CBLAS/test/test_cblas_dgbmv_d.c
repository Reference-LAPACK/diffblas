/* Test program for cblas_dgbmv differentiation */
...
extern void cblas_dgbmv_d(const CBLAS_LAYOUT layout, const CBLAS_TRANSPOSE TransA, const CBLAS_INT M, const CBLAS_INT N, const CBLAS_INT KL, const CBLAS_INT KU, const double alpha, double alpha_d, const double *A, double *A_d, const CBLAS_INT lda, const double *X, double *X_d, const CBLAS_INT incX, const double beta, double beta_d, double *Y, double *Y_d, const CBLAS_INT incY);
...
int main(void) {
    ...
    for (j = 0; j < MAX_SIZE; j++) {
        int band_rows = KL + KU + 1;
        for (i = 0; i < band_rows; i++) {
            A_d[i + j * MAX_SIZE] = ((double)rand() / RAND_MAX) * 2.0 - 1.0;
        }
    }
    for (i = 0; i < MAX_SIZE; i++) {
