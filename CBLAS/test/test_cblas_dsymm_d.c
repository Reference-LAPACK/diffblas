/* Test program for cblas_dsymm differentiation */
...
/* Original function */
extern void cblas_dsymm(const CBLAS_LAYOUT layout, const CBLAS_SIDE Side, const CBLAS_UPLO Uplo, const CBLAS_INT M, const CBLAS_INT N, const double alpha, const double *A, const CBLAS_INT lda, const double *B, const CBLAS_INT ldb, const double beta, double *C, const CBLAS_INT ldc);
/* Differentiated function */
extern void cblas_dsymm_d(const CBLAS_LAYOUT layout, const CBLAS_SIDE Side, const CBLAS_UPLO Uplo, const CBLAS_INT M, const CBLAS_INT N, const double alpha, double alpha_d, const double *A, double *A_d, const CBLAS_INT lda, const double *B, double *B_d, const CBLAS_INT ldb, const double beta, double beta_d, double *C, double *C_d, const CBLAS_INT ldc);
...
int main(void) {
    ...
    for (i = 0; i < MAX_SIZE; i++) {
        for (j = i; j < MAX_SIZE; j++) {
            A_d[i * MAX_SIZE + j] = ((double)rand() / RAND_MAX) * 2.0 - 1.0;
        }
    }
    for (i = 1; i < MAX_SIZE; i++) {
        for (j = 0; j < i; j++) {
    ...
    for (i = 0; i < 4; i++) {
        printf("  C_d[%d] = %.6e, A_d[%d] = %.6e, B_d[%d] = %.6e\n", i, C_d[i], i, A_d_orig[i], i, B_d_orig[i]);
    }
