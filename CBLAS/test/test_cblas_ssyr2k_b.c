/* Test program for cblas_ssyr2k reverse mode (VJP verification, generic) */
...
extern void cblas_ssyr2k_b(const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo, const CBLAS_TRANSPOSE Trans, const CBLAS_INT N, const CBLAS_INT K, const float alpha, float *alpha_b, const float *A, float *A_b, const CBLAS_INT lda, const float *B, float *B_b, const CBLAS_INT ldb, const float beta, float *beta_b, float *C, float *C_b, const CBLAS_INT ldc);
...
int main(void) {
    ...
    cblas_ssyr2k_b(layout, uplo, trans, n, k, alpha, &alpha_b, A, A_b, lda, B, B_b, ldb, beta, &beta_b, C, C_b, ldc);
