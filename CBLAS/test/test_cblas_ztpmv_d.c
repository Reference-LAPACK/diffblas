/* Test program for cblas_ztpmv differentiation */
...
extern void cblas_ztpmv_d(const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo, const CBLAS_TRANSPOSE TransA, const CBLAS_DIAG Diag, const CBLAS_INT N, const void *Ap, void *Ap_d, void *X, void *X_d, const CBLAS_INT incX);
...
int main(void) {
    ...
    double complex Ap[PACKED_SIZE];
    double complex Ap_d[PACKED_SIZE];  /* Derivative seeds */
    double complex Ap_d_orig[PACKED_SIZE];
    double complex Ap_orig[PACKED_SIZE];
    ...
    for (j = 0; j < MAX_SIZE; j++) {
        for (i = 0; i <= j; i++) {
            Ap[j * (j + 1) / 2 + i] = ((double)rand() / RAND_MAX) * 2.0 - 1.0 + I * (((double)rand() / RAND_MAX) * 2.0 - 1.0);
        }
    }
    ...
    for (i = 0; i < PACKED_SIZE; i++) {
        Ap_d[i] = ((double)rand() / RAND_MAX) * 2.0 - 1.0 + I * (((double)rand() / RAND_MAX) * 2.0 - 1.0);
    }
    for (i = 0; i < MAX_SIZE; i++) {
