/* Test program for cblas_zhemm reverse mode (VJP verification, generic) */
...
extern void cblas_zhemm_b(const CBLAS_LAYOUT layout, const CBLAS_SIDE Side, const CBLAS_UPLO Uplo, const CBLAS_INT M, const CBLAS_INT N, const void *alpha, void *alpha_b, const void *A, void *A_b, const CBLAS_INT lda, const void *B, void *B_b, const CBLAS_INT ldb, const void *beta, void *beta_b, void *C, void *C_b, const CBLAS_INT ldc);
...
int main(void) {
    ...
    double complex B[MAX_SIZE*MAX_SIZE], B_b[MAX_SIZE*MAX_SIZE], B_orig[MAX_SIZE*MAX_SIZE], B_dir[MAX_SIZE*MAX_SIZE];
    double complex beta[MAX_SIZE], beta_b[MAX_SIZE], beta_orig[MAX_SIZE], beta_dir[MAX_SIZE];
    double complex C[MAX_SIZE*MAX_SIZE], C_b[MAX_SIZE*MAX_SIZE], C_orig[MAX_SIZE*MAX_SIZE], C_dir[MAX_SIZE*MAX_SIZE];
    double complex C_plus[MAX_SIZE*MAX_SIZE], C_minus[MAX_SIZE*MAX_SIZE], C_central_diff[MAX_SIZE*MAX_SIZE], C_b_orig[MAX_SIZE*MAX_SIZE];
    ...
    /* Enforce Hermitian A and A_dir: real diagonal, lower = conj(upper) */
    for (j = 0; j < n; j++) {
        for (i = j + 1; i < n; i++) A[i + j*lda] = conj(A[j + i*lda]);
        A[j + j*lda] = creal(A[j + j*lda]);
    }
    for (j = 0; j < n; j++) {
        for (i = j + 1; i < n; i++) A_dir[i + j*lda] = conj(A_dir[j + i*lda]);
        A_dir[j + j*lda] = creal(A_dir[j + j*lda]);
    }
    memcpy(A_orig, A, sizeof(A[0])*(MAX_SIZE*MAX_SIZE));
    ...
    cblas_zhemm_b(layout, side, uplo, m, n, &alpha, alpha_b, A, A_b, lda, B, B_b, ldb, &beta, beta_b, C, C_b, ldc);
