/* Test program for cblas_ztrmv reverse mode (VJP verification, generic) */
...
extern void cblas_ztrmv_b(const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo, const CBLAS_TRANSPOSE TransA, const CBLAS_DIAG Diag, const CBLAS_INT N, const void *A, void *A_b, const CBLAS_INT lda, void *X, void *X_b, const CBLAS_INT incX);
...
int main(void) {
    ...
    cblas_ztrmv_b(layout, uplo, transa, diag, n, A, A_b, lda, X, X_b, incX);

    for (i = 0; i < MAX_SIZE*MAX_SIZE; i++) A[i] = A_orig[i] + h * A_dir[i];
    for (i = 0; i < MAX_SIZE; i++) X[i] = X_orig[i] + h * X_dir[i];
    cblas_ztrmv(layout, uplo, transa, diag, n, A, lda, X, incX);
