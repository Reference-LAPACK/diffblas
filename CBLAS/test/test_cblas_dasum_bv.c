/* Test program for cblas_dasum vector reverse (bv) differentiation (scalar result) */
...
extern double cblas_dasum(CBLAS_INT N, const double *X, CBLAS_INT incX);
extern void cblas_dasum_bv(CBLAS_INT N, const double *X, double (*X_b)[NBDirsMax], CBLAS_INT incX, double result_b[NBDirsMax], int nbdirs);
...
int main(void) {
    ...
    cblas_dasum_bv(N, X, X_b, incX, result_b, nbdirs);
    ...
    for (idir = 0; idir < nbdirs; idir++) {
        ...
        double result_forward = cblas_dasum(
        N,
        X,
        incX
        );
        ...
        /* Backward */
        for (i = 0; i < MAX_SIZE; i++) X[i] = X_orig[i] - h * X_dir[i];
        double result_backward = cblas_dasum(
        N,
        X,
        incX
        );
