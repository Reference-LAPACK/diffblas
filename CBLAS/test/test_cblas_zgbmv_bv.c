/* Test program for cblas_zgbmv vector reverse mode (VJP verification, generic, loop over directions) */
...
extern void cblas_zgbmv(const CBLAS_LAYOUT layout, const CBLAS_TRANSPOSE TransA, const CBLAS_INT M, const CBLAS_INT N, const CBLAS_INT KL, const CBLAS_INT KU, const void *alpha, const void *A, const CBLAS_INT lda, const void *X, const CBLAS_INT incX, const void *beta, void *Y, const CBLAS_INT incY);
...
int main(void) {
    ...
    double complex Y[MAX_SIZE], Y_orig[MAX_SIZE], Y_dir[MAX_SIZE];
    double complex Y_b[MAX_SIZE][NBDirsMax], Y_b_orig[MAX_SIZE][NBDirsMax];
    double complex Y_plus[MAX_SIZE], Y_minus[MAX_SIZE];
    ...
    for (j = 0; j < MAX_SIZE; j++) {
        int band_rows = KL + KU + 1;
        for (i = 0; i < band_rows; i++) {
            A_dir[i + j * MAX_SIZE] = ((double)rand() / RAND_MAX) * 2.0 - 1.0 + I * (((double)rand() / RAND_MAX) * 2.0 - 1.0);
        }
    }
    ...
    for (i = 0; i < MAX_SIZE; i++) for (j = 0; j < NBDirsMax; j++) { Y_b[i][j] = (rand()/(double)RAND_MAX)*2.0 - 1.0 + I*((rand()/(double)RAND_MAX)*2.0 - 1.0); Y_b_orig[i][j] = Y_b[i][j]; }
    ...
    cblas_zgbmv_bv(layout, transa, m, n, KL, KU, &alpha, (void*)&alpha_b, A, (void*)A_b, lda, X, (void*)X_b, incX, &beta, (void*)&beta_b, Y, (void*)Y_b, incY, nbdirs);
    ...
    for (idir = 0; idir < nbdirs; idir++) {
        ...
        cblas_zgbmv(layout, transa, m, n, KL, KU, &alpha, A, lda, X, incX, &beta, Y, incY);
        ...
        vjp_fd = 0.0;
        {
            ...
            for (i = 0; i < MAX_SIZE; i++) temp_products[i] = creal(conj(Y_b_orig[i][idir]) * ((Y_plus[i] - Y_minus[i]) / (2.0*h)));
        ...
        vjp_ad += creal(conj(alpha_dir) * alpha_b[idir]);
        {
            ...
            for (j = 0; j < n; j++) for (i = 0; i < band_rows; i++) {
                temp_products[n_band++] = creal(conj(A_dir[i+j*lda]) * A_b[i+j*lda][idir]);
            }
