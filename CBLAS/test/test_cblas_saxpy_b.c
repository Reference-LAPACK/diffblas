/* Test program for cblas_saxpy reverse mode (VJP verification, generic) */
...
extern void cblas_saxpy(const CBLAS_INT N, const float alpha, const float *X, const CBLAS_INT incX, float *Y, const CBLAS_INT incY);
extern void cblas_saxpy_b(const CBLAS_INT N, const float alpha, float *alpha_b, const float *X, float *X_b, const CBLAS_INT incX, float *Y, float *Y_b, const CBLAS_INT incY);
...
int main(void) {
    ...
    cblas_saxpy_b(n, alpha, &alpha_b, X, X_b, incX, Y, Y_b, incY);
