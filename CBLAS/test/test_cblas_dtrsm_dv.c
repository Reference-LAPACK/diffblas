/* Test program for cblas_dtrsm forward vector (dv) differentiation */
...
extern void cblas_dtrsm_dv(CBLAS_LAYOUT layout, CBLAS_SIDE Side, CBLAS_UPLO Uplo, CBLAS_TRANSPOSE TransA, CBLAS_DIAG Diag, CBLAS_INT M, CBLAS_INT N, double alpha, double alphad[NBDirsMax], const double *A, double (*Ad)[NBDirsMax], CBLAS_INT lda, double *B, double (*Bd)[NBDirsMax], CBLAS_INT ldb, int nbdirs);
...
int main(void) {
    ...
    double B_output[MAX_SIZE * MAX_SIZE];
    double B_ad_output[MAX_SIZE * MAX_SIZE];
    double B_forward[MAX_SIZE * MAX_SIZE], B_backward[MAX_SIZE * MAX_SIZE];
    ...
    /* Verify AD primal output matches original (same as _d) */
    {
        ...
        for (i = 0; i < MAX_SIZE * MAX_SIZE; i++) {
            double diff = fabs(B_ad_output[i] - B_output[i]);
