/* Test program for cblas_dsyrk forward vector (dv) differentiation */
...
extern void cblas_dsyrk_dv(CBLAS_LAYOUT layout, CBLAS_UPLO Uplo, CBLAS_TRANSPOSE Trans, CBLAS_INT N, CBLAS_INT K, double alpha, double alphad[NBDirsMax], const double *A, double (*Ad)[NBDirsMax], CBLAS_INT lda, double beta, double betad[NBDirsMax], double *C, double (*Cd)[NBDirsMax], CBLAS_INT ldc, int nbdirs);
