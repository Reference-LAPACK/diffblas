/* Test program for cblas_dsymm vector reverse mode (VJP verification, generic, loop over directions) */
...
int main(void) {
    ...
    for (i = 0; i < MAX_SIZE; i++) {
        for (j = i; j < MAX_SIZE; j++) {
            A_dir[i * MAX_SIZE + j] = ((double)rand() / RAND_MAX) * 2.0 - 1.0;
        }
    }
    for (i = 1; i < MAX_SIZE; i++) {
        for (j = 0; j < i; j++) {
    ...
    cblas_dsymm_bv(layout, side, uplo, m, n, alpha, &alpha_b, A, &A_b[0][0], lda, B, &B_b[0][0], ldb, beta, &beta_b, C, &C_b[0][0], ldc, nbdirs);
