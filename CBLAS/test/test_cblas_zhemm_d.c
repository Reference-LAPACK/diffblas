/* Test program for cblas_zhemm differentiation */
...
extern void cblas_zhemm_d(const CBLAS_LAYOUT layout, const CBLAS_SIDE Side, const CBLAS_UPLO Uplo, const CBLAS_INT M, const CBLAS_INT N, const void *alpha, void *alpha_d, const void *A, void *A_d, const CBLAS_INT lda, const void *B, void *B_d, const CBLAS_INT ldb, const void *beta, void *beta_d, void *C, void *C_d, const CBLAS_INT ldc);
...
int main(void) {
    ...
    double complex B_d[MAX_SIZE * MAX_SIZE];  /* Derivative seeds */
    double complex B_d_orig[MAX_SIZE * MAX_SIZE];  /* Save derivative seeds for finite differences */
    double complex B_orig[MAX_SIZE * MAX_SIZE];
    ...
    for (i = 0; i < MAX_SIZE * MAX_SIZE; i++) {
        B[i] = ((double)rand() / RAND_MAX) * 2.0 - 1.0 + I * (((double)rand() / RAND_MAX) * 2.0 - 1.0);
    }
    ...
    /* A: Hermitian (match BLAS/test) */
    for (i = 0; i < MAX_SIZE; i++) {
        A_d[i * MAX_SIZE + i] = ((double)rand() / RAND_MAX) * 2.0 - 1.0;  /* real diagonal */
        for (j = i + 1; j < MAX_SIZE; j++) {
            A_d[i * MAX_SIZE + j] = ((double)rand() / RAND_MAX) * 2.0 - 1.0 + I * (((double)rand() / RAND_MAX) * 2.0 - 1.0);
        }
    }
    for (i = 1; i < MAX_SIZE; i++) {
        for (j = 0; j < i; j++) {
            A_d[i * MAX_SIZE + j] = conj(A_d[j * MAX_SIZE + i]);  /* Hermitian */
        }
    }
    for (i = 0; i < MAX_SIZE * MAX_SIZE; i++) {
        B_d[i] = ((double)rand() / RAND_MAX) * 2.0 - 1.0 + I * (((double)rand() / RAND_MAX) * 2.0 - 1.0);
    }
