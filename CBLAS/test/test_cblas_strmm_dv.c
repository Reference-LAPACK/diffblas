/* Test program for cblas_strmm forward vector (dv) differentiation */
...
extern void cblas_strmm_dv(CBLAS_LAYOUT layout, CBLAS_SIDE Side, CBLAS_UPLO Uplo, CBLAS_TRANSPOSE TransA, CBLAS_DIAG Diag, CBLAS_INT M, CBLAS_INT N, float alpha, float alphad[NBDirsMax], const float *A, float (*Ad)[NBDirsMax], CBLAS_INT lda, float *B, float (*Bd)[NBDirsMax], CBLAS_INT ldb, int nbdirs);
