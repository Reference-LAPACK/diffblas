/* Test program for cblas_sger forward vector (dv) differentiation */
...
extern void cblas_sger_dv(CBLAS_LAYOUT layout, CBLAS_INT M, CBLAS_INT N, float alpha, float alphad[NBDirsMax], const float *X, float (*Xd)[NBDirsMax], CBLAS_INT incX, const float *Y, float (*Yd)[NBDirsMax], CBLAS_INT incY, float *A, float (*Ad)[NBDirsMax], CBLAS_INT lda, int nbdirs);
