/* Test program for cblas_ztrmm forward vector (dv) differentiation */
...
extern void cblas_ztrmm_dv(CBLAS_LAYOUT layout, CBLAS_SIDE Side, CBLAS_UPLO Uplo, CBLAS_TRANSPOSE TransA, CBLAS_DIAG Diag, CBLAS_INT M, CBLAS_INT N, const void *alpha, const void *alphad, const void *A, void *Ad, CBLAS_INT lda, void *B, void *Bd, CBLAS_INT ldb, int nbdirs);
...
int main(void) {
    ...
    double complex B_output[MAX_SIZE * MAX_SIZE];
    double complex B_ad_output[MAX_SIZE * MAX_SIZE];
    double complex B_forward[MAX_SIZE * MAX_SIZE], B_backward[MAX_SIZE * MAX_SIZE];
    ...
    /* Verify AD primal output matches original (same as _d) */
    {
        ...
        for (i = 0; i < MAX_SIZE * MAX_SIZE; i++) {
            double diff = cabs(B_ad_output[i] - B_output[i]);
    ...
    for (idir = 0; idir < nbdirs; idir++) {
        ...
        for (i = 0; i < MAX_SIZE * MAX_SIZE; i++) {
            double fd = (B_forward[i] - B_backward[i]) / (2.0 * h);
            double ad = Bd[i][idir];
