/* Test program for cblas_cdotu_sub vector reverse mode (VJP verification, generic, loop over directions) */
...
extern void cblas_cdotu_sub(const CBLAS_INT N, const void *X, const CBLAS_INT incX, const void *Y, const CBLAS_INT incY, void *dotu);
...
int main(void) {
    ...
    float complex dotu[1], dotu_orig[1], dotu_dir[1];
    float complex dotu_b[1][NBDirsMax], dotu_b_orig[1][NBDirsMax];
    float complex dotu_plus[1], dotu_minus[1];
    ...
    cblas_cdotu_sub_bv(n, X, (void*)X_b, incX, Y, (void*)Y_b, incY, dotu, (void*)dotu_b, nbdirs);
