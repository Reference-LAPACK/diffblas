/* Test program for cblas_zhemm vector reverse mode (VJP verification, generic, loop over directions) */
...
extern void cblas_zhemm(const CBLAS_LAYOUT layout, const CBLAS_SIDE Side, const CBLAS_UPLO Uplo, const CBLAS_INT M, const CBLAS_INT N, const void *alpha, const void *A, const CBLAS_INT lda, const void *B, const CBLAS_INT ldb, const void *beta, void *C, const CBLAS_INT ldc);
...
int main(void) {
    ...
    double complex B[MAX_SIZE*MAX_SIZE], B_orig[MAX_SIZE*MAX_SIZE], B_dir[MAX_SIZE*MAX_SIZE];
    double complex B_b[MAX_SIZE*MAX_SIZE][NBDirsMax], B_b_orig[MAX_SIZE*MAX_SIZE][NBDirsMax];
    double complex beta, beta_b[NBDirsMax], beta_orig, beta_dir, beta_b_orig[NBDirsMax];
    double complex C[MAX_SIZE*MAX_SIZE], C_orig[MAX_SIZE*MAX_SIZE], C_dir[MAX_SIZE*MAX_SIZE];
    double complex C_b[MAX_SIZE*MAX_SIZE][NBDirsMax], C_b_orig[MAX_SIZE*MAX_SIZE][NBDirsMax];
    double complex C_plus[MAX_SIZE*MAX_SIZE], C_minus[MAX_SIZE*MAX_SIZE];
    ...
    /* A: Hermitian (match BLAS/test) */
    for (i = 0; i < MAX_SIZE; i++) {
        A[i * MAX_SIZE + i] = ((double)rand() / RAND_MAX) * 2.0 - 1.0;  /* real diagonal */
        for (j = i + 1; j < MAX_SIZE; j++) {
            A[i * MAX_SIZE + j] = ((double)rand() / RAND_MAX) * 2.0 - 1.0 + I * (((double)rand() / RAND_MAX) * 2.0 - 1.0);
        }
    }
    for (i = 1; i < MAX_SIZE; i++) {
        for (j = 0; j < i; j++) {
            A[i * MAX_SIZE + j] = conj(A[j * MAX_SIZE + i]);  /* Hermitian */
        }
    }
    /* A: Hermitian (match BLAS/test) */
    for (i = 0; i < MAX_SIZE; i++) {
        A_dir[i * MAX_SIZE + i] = ((double)rand() / RAND_MAX) * 2.0 - 1.0;  /* real diagonal */
        for (j = i + 1; j < MAX_SIZE; j++) {
            A_dir[i * MAX_SIZE + j] = ((double)rand() / RAND_MAX) * 2.0 - 1.0 + I * (((double)rand() / RAND_MAX) * 2.0 - 1.0);
        }
    }
    for (i = 1; i < MAX_SIZE; i++) {
        for (j = 0; j < i; j++) {
            A_dir[i * MAX_SIZE + j] = conj(A_dir[j * MAX_SIZE + i]);  /* Hermitian */
        }
    }
    ...
    for (j = 0; j < n; j++) {
        for (i = j + 1; i < n; i++) A[i + j*lda] = conj(A[j + i*lda]); A[j + j*lda] = creal(A[j + j*lda]); }
    for (j = 0; j < n; j++) {
        for (i = j + 1; i < n; i++) A_dir[i + j*lda] = conj(A_dir[j + i*lda]); A_dir[j + j*lda] = creal(A_dir[j + j*lda]); }
    memcpy(A_orig, A, sizeof(A[0])*(MAX_SIZE*MAX_SIZE));
    ...
    cblas_zhemm_bv(layout, side, uplo, m, n, &alpha, (void*)&alpha_b, A, (void*)A_b, lda, B, (void*)B_b, ldb, &beta, (void*)&beta_b, C, (void*)C_b, ldc, nbdirs);
    ...
    for (idir = 0; idir < nbdirs; idir++) {
        ...
        for (j = 0; j < n; j++) { for (i = j + 1; i < n; i++) A_dir[i + j*lda] = conj(A_dir[j + i*lda]); A_dir[j + j*lda] = creal(A_dir[j + j*lda]); }
        for (i = 0; i < MAX_SIZE*MAX_SIZE; i++) B_dir[i] = (rand()/(double)RAND_MAX)*2.0 - 1.0 + I*((rand()/(double)RAND_MAX)*2.0 - 1.0);
