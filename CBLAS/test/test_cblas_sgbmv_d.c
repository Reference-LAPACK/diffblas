/* Test program for cblas_sgbmv differentiation */
...
extern void cblas_sgbmv_d(const CBLAS_LAYOUT layout, const CBLAS_TRANSPOSE TransA, const CBLAS_INT M, const CBLAS_INT N, const CBLAS_INT KL, const CBLAS_INT KU, const float alpha, float alpha_d, const float *A, float *A_d, const CBLAS_INT lda, const float *X, float *X_d, const CBLAS_INT incX, const float beta, float beta_d, float *Y, float *Y_d, const CBLAS_INT incY);
...
int main(void) {
    ...
    for (j = 0; j < MAX_SIZE; j++) {
        int band_rows = KL + KU + 1;
        for (i = 0; i < band_rows; i++) {
            A_d[i + j * MAX_SIZE] = ((float)rand() / RAND_MAX) * 2.0f - 1.0f;
        }
    }
    for (i = 0; i < MAX_SIZE; i++) {
