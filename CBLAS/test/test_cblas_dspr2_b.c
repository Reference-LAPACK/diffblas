/* Test program for cblas_dspr2 reverse mode (VJP verification, generic) */
...
extern void cblas_dspr2(const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo, const CBLAS_INT N, const double alpha, const double *X, const CBLAS_INT incX, const double *Y, const CBLAS_INT incY, double *A);
extern void cblas_dspr2_b(const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo, const CBLAS_INT N, const double alpha, double *alpha_b, const double *X, double *X_b, const CBLAS_INT incX, const double *Y, double *Y_b, const CBLAS_INT incY, double *A, double *A_b);
...
int main(void) {
    ...
    double A[PACKED_SIZE], A_b[PACKED_SIZE], A_orig[PACKED_SIZE], A_dir[PACKED_SIZE];
    double A_plus[PACKED_SIZE], A_minus[PACKED_SIZE], A_central_diff[PACKED_SIZE], A_b_orig[PACKED_SIZE];
    ...
    for (i = 0; i < PACKED_SIZE; i++) { A[i] = (rand()/(double)RAND_MAX)*2.0 - 1.0; A_dir[i] = (rand()/(double)RAND_MAX)*2.0 - 1.0; }

    alpha_orig = alpha;
    memcpy(X_orig, X, sizeof(X[0])*(MAX_SIZE));
    memcpy(Y_orig, Y, sizeof(Y[0])*(MAX_SIZE));
    memcpy(A_orig, A, sizeof(A[0])*(PACKED_SIZE));

    for (i = 0; i < PACKED_SIZE; i++) { A_b[i] = (rand()/(double)RAND_MAX)*2.0 - 1.0; A_b_orig[i] = A_b[i]; }
    alpha_b = 0.0;
    for (i = 0; i < MAX_SIZE; i++) X_b[i] = 0.0;
    for (i = 0; i < MAX_SIZE; i++) Y_b[i] = 0.0;

    cblas_dspr2_b(layout, uplo, n, alpha, &alpha_b, X, X_b, incX, Y, Y_b, incY, A, A_b);

    alpha = alpha_orig + h * alpha_dir;
    for (i = 0; i < MAX_SIZE; i++) X[i] = X_orig[i] + h * X_dir[i];
    for (i = 0; i < MAX_SIZE; i++) Y[i] = Y_orig[i] + h * Y_dir[i];
    for (i = 0; i < PACKED_SIZE; i++) A[i] = A_orig[i] + h * A_dir[i];
    cblas_dspr2(layout, uplo, n, alpha, X, incX, Y, incY, A);
    memcpy(A_plus, A, sizeof(A[0])*(PACKED_SIZE));
    ...
    for (i = 0; i < PACKED_SIZE; i++) A[i] = A_orig[i] - h * A_dir[i];
    cblas_dspr2(layout, uplo, n, alpha, X, incX, Y, incY, A);
    memcpy(A_minus, A, sizeof(A[0])*(PACKED_SIZE));

    vjp_fd = 0.0;
    {
        double temp_products[PACKED_SIZE];
        for (i = 0; i < PACKED_SIZE; i++) temp_products[i] = A_b_orig[i] * ((A_plus[i] - A_minus[i]) / (2.0*h));
