/* Test program for cblas_cdotu_sub reverse mode (VJP verification, generic) */
...
extern void cblas_cdotu_sub_b(const CBLAS_INT N, const void *X, void *X_b, const CBLAS_INT incX, const void *Y, void *Y_b, const CBLAS_INT incY, void *dotu, void *dotu_b);
...
int main(void) {
    ...
    float complex dotu[1], dotu_b[1], dotu_orig[1], dotu_dir[1];
    float complex dotu_plus[1], dotu_minus[1], dotu_central_diff[1], dotu_b_orig[1];
    ...
    cblas_cdotu_sub_b(n, X, X_b, incX, Y, Y_b, incY, dotu, dotu_b);
