/* Test program for cblas_zhbmv vector reverse mode (VJP verification, generic, loop over directions) */
...
int main(void) {
    ...
    for (j = 0; j < MAX_SIZE; j++) {
        ...
        for (i = first_row; i <= j && i < MAX_SIZE; i++) {
            ...
            if (i == j) {  /* diagonal: real for Hermitian */
                ...
                A[band_row + j * MAX_SIZE] = ((double)rand() / RAND_MAX) * 2.0 - 1.0 + I * (((double)rand() / RAND_MAX) * 2.0 - 1.0);
            }
        }
    }
    /* A: upper band storage (k+1) x N; full a(i,j) at A[k+i-j + j*lda], i = max(0,j-k)..j */
    ...
    for (j = 0; j < MAX_SIZE; j++) {
        ...
        for (i = first_row; i <= j && i < MAX_SIZE; i++) {
            int band_row = k + i - j;  /* BLAS: a(i,j) -> A(band_row, j) */
            if (i == j) {  /* diagonal: real for Hermitian */
                A_dir[band_row + j * MAX_SIZE] = ((double)rand() / RAND_MAX) * 2.0 - 1.0;
            } else {
                A_dir[band_row + j * MAX_SIZE] = ((double)rand() / RAND_MAX) * 2.0 - 1.0 + I * (((double)rand() / RAND_MAX) * 2.0 - 1.0);
            }
        }
    }
    ...
    cblas_zhbmv_bv(layout, uplo, n, k, &alpha, (void*)&alpha_b, A, (void*)A_b, lda, X, (void*)X_b, incX, &beta, (void*)&beta_b, Y, (void*)Y_b, incY, nbdirs);
    ...
    for (idir = 0; idir < nbdirs; idir++) {
        ...
        for (j = 0; j < n; j++) { A_dir[k + j * lda] = creal(A_dir[k + j * lda]); }
