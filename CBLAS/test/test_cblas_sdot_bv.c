/* Test program for cblas_sdot vector reverse (bv) differentiation (scalar result) */
...
extern float cblas_sdot(CBLAS_INT N, const float *X, CBLAS_INT incX, const float *Y, CBLAS_INT incY);
extern void cblas_sdot_bv(CBLAS_INT N, const float *X, float (*X_b)[NBDirsMax], CBLAS_INT incX, const float *Y, float (*Y_b)[NBDirsMax], CBLAS_INT incY, float result_b[NBDirsMax], int nbdirs);
...
int main(void) {
    ...
    for (i = 0; i < MAX_SIZE; i++) Y_dir[i] = ((float)rand() / RAND_MAX) * 2.0 - 1.0;
    ...
    cblas_sdot_bv(N, X, X_b, incX, Y, Y_b, incY, result_b, nbdirs);
