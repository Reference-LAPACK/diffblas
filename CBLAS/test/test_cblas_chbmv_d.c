/* Test program for cblas_chbmv differentiation */
...
extern void cblas_chbmv_d(const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo, const CBLAS_INT N, const CBLAS_INT K, const void *alpha, void *alpha_d, const void *A, void *A_d, const CBLAS_INT lda, const void *X, void *X_d, const CBLAS_INT incX, const void *beta, void *beta_d, void *Y, void *Y_d, const CBLAS_INT incY);
...
int main(void) {
    ...
    for (j = 0; j < MAX_SIZE; j++) {
        ...
        for (i = first_row; i <= j && i < MAX_SIZE; i++) {
            int band_row = K + i - j;  /* BLAS: a(i,j) -> A(band_row, j) */
            if (i == j) {  /* diagonal: real for Hermitian */
                A_d[band_row + j * MAX_SIZE] = ((float)rand() / RAND_MAX) * 2.0f - 1.0f;
            } else {
                A_d[band_row + j * MAX_SIZE] = ((float)rand() / RAND_MAX) * 2.0f - 1.0f + I * (((float)rand() / RAND_MAX) * 2.0f - 1.0f);
            }
        }
    }
    for (i = 0; i < MAX_SIZE; i++) {
