/* Test program for cblas_sscal reverse mode (VJP verification, generic) */
...
extern void cblas_sscal(const CBLAS_INT N, const float alpha, float *X, const CBLAS_INT incX);
extern void cblas_sscal_b(const CBLAS_INT N, const float alpha, float *alpha_b, float *X, float *X_b, const CBLAS_INT incX);
...
int main(void) {
    ...
    cblas_sscal_b(n, alpha, &alpha_b, X, X_b, incX);
