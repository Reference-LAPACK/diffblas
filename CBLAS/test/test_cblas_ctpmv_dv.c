/* Test program for cblas_ctpmv forward vector (dv) differentiation */
...
extern void cblas_ctpmv_dv(CBLAS_LAYOUT layout, CBLAS_UPLO Uplo, CBLAS_TRANSPOSE TransA, CBLAS_DIAG Diag, CBLAS_INT N, const void *Ap, void *Apd, void *X, void *Xd, CBLAS_INT incX, int nbdirs);
...
int main(void) {
    ...
    float complex Apd[PACKED_SIZE][NBDirsMax];
    float complex Ap_orig[PACKED_SIZE];
    float complex Apd_orig[PACKED_SIZE][NBDirsMax];
