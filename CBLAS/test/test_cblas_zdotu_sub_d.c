/* Test program for cblas_zdotu_sub differentiation */
...
extern void cblas_zdotu_sub_d(const CBLAS_INT N, const void *X, void *X_d, const CBLAS_INT incX, const void *Y, void *Y_d, const CBLAS_INT incY, void *dotu, void *dotu_d);
...
int main(void) {
    ...
    CBLAS_INT incY;
    double complex dotu[MAX_SIZE];
    double complex dotu_d[MAX_SIZE];  /* Derivative seeds */
    double complex dotu_d_orig[MAX_SIZE];  /* Save derivative seeds for finite differences */
    double complex dotu_orig[MAX_SIZE];
    ...
    for (i = 0; i < MAX_SIZE; i++) {
        dotu[i] = ((double)rand() / RAND_MAX) * 2.0 - 1.0 + I * (((double)rand() / RAND_MAX) * 2.0 - 1.0);
    }
    ...
    for (i = 0; i < MAX_SIZE; i++) {
        dotu_d[i] = ((double)rand() / RAND_MAX) * 2.0 - 1.0 + I * (((double)rand() / RAND_MAX) * 2.0 - 1.0);
    }
