/* Test program for cblas_ctpmv vector reverse mode (VJP verification, generic, loop over directions) */
...
extern void cblas_ctpmv(const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo, const CBLAS_TRANSPOSE TransA, const CBLAS_DIAG Diag, const CBLAS_INT N, const void *Ap, void *X, const CBLAS_INT incX);
...
int main(void) {
    ...
    float complex Ap[PACKED_SIZE], Ap_orig[PACKED_SIZE], Ap_dir[PACKED_SIZE];
    float complex Ap_b[PACKED_SIZE][NBDirsMax], Ap_b_orig[PACKED_SIZE][NBDirsMax];
    ...
    cblas_ctpmv_bv(layout, uplo, transa, diag, n, Ap, (void*)Ap_b, X, (void*)X_b, incX, nbdirs);
    ...
    for (idir = 0; idir < nbdirs; idir++) {
        ...
        for (i = 0; i < PACKED_SIZE; i++) Ap_dir[i] = (rand()/(double)RAND_MAX)*2.0 - 1.0 + I*((rand()/(double)RAND_MAX)*2.0 - 1.0);
        ...
        cblas_ctpmv(layout, uplo, transa, diag, n, Ap, X, incX);
        ...
        vjp_ad = 0.0f;
        {
            ...
            for (i = 0; i < PACKED_SIZE; i++) temp_products[i] = creal(conj(Ap_dir[i]) * Ap_b[i][idir]);
