/* Test program for cblas_zgbmv differentiation */
...
extern void cblas_zgbmv_d(const CBLAS_LAYOUT layout, const CBLAS_TRANSPOSE TransA, const CBLAS_INT M, const CBLAS_INT N, const CBLAS_INT KL, const CBLAS_INT KU, const void *alpha, void *alpha_d, const void *A, void *A_d, const CBLAS_INT lda, const void *X, void *X_d, const CBLAS_INT incX, const void *beta, void *beta_d, void *Y, void *Y_d, const CBLAS_INT incY);
...
int main(void) {
    ...
    for (j = 0; j < MAX_SIZE; j++) {
        int band_rows = KL + KU + 1;
        for (i = 0; i < band_rows; i++) {
            A_d[i + j * MAX_SIZE] = ((double)rand() / RAND_MAX) * 2.0 - 1.0 + I * (((double)rand() / RAND_MAX) * 2.0 - 1.0);
        }
    }
    for (i = 0; i < MAX_SIZE; i++) {
