/* Test program for cblas_ssyr2 forward vector (dv) differentiation */
...
extern void cblas_ssyr2_dv(CBLAS_LAYOUT layout, CBLAS_UPLO Uplo, CBLAS_INT N, float alpha, float alphad[NBDirsMax], const float *X, float (*Xd)[NBDirsMax], CBLAS_INT incX, const float *Y, float (*Yd)[NBDirsMax], CBLAS_INT incY, float *A, float (*Ad)[NBDirsMax], CBLAS_INT lda, int nbdirs);
...
int main(void) {
    ...
    float A_output[MAX_SIZE * MAX_SIZE];
    float A_ad_output[MAX_SIZE * MAX_SIZE];
    float A_forward[MAX_SIZE * MAX_SIZE], A_backward[MAX_SIZE * MAX_SIZE];
