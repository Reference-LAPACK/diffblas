/* Test program for cblas_zdotu_sub vector reverse mode (VJP verification, generic, loop over directions) */
...
int main(void) {
    ...
    double complex dotu[1], dotu_orig[1], dotu_dir[1];
    double complex dotu_b[1][NBDirsMax], dotu_b_orig[1][NBDirsMax];
    double complex dotu_plus[1], dotu_minus[1];
    ...
    for (i = 0; i < 1; i++) for (j = 0; j < NBDirsMax; j++) { dotu_b[i][j] = (rand()/(double)RAND_MAX)*2.0 - 1.0 + I*((rand()/(double)RAND_MAX)*2.0 - 1.0); dotu_b_orig[i][j] = dotu_b[i][j]; }
    for (i = 0; i < MAX_SIZE; i++) for (j = 0; j < NBDirsMax; j++) X_b[i][j] = 0.0;
    for (i = 0; i < MAX_SIZE; i++) for (j = 0; j < NBDirsMax; j++) Y_b[i][j] = 0.0;

    cblas_zdotu_sub_bv(n, X, (void*)X_b, incX, Y, (void*)Y_b, incY, dotu, (void*)dotu_b, nbdirs);
    ...
    for (idir = 0; idir < nbdirs; idir++) {
        ...
        memcpy(dotu, dotu_orig, sizeof(dotu[0])*(1));
        ...
        for (i = 0; i < 1; i++) dotu_dir[i] = (rand()/(double)RAND_MAX)*2.0 - 1.0 + I*((rand()/(double)RAND_MAX)*2.0 - 1.0);
        /* Forward */
        ...
        vjp_fd = 0.0;
        {
            ...
            for (i = 0; i < 1; i++) temp_products[i] = creal(conj(dotu_b_orig[i][idir]) * ((dotu_plus[i] - dotu_minus[i]) / (2.0*h)));
