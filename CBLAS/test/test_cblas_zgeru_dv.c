/* Test program for cblas_zgeru forward vector (dv) differentiation */
...
extern void cblas_zgeru_dv(CBLAS_LAYOUT layout, CBLAS_INT M, CBLAS_INT N, const void *alpha, const void *alphad, const void *X, void *Xd, CBLAS_INT incX, const void *Y, void *Yd, CBLAS_INT incY, void *A, void *Ad, CBLAS_INT lda, int nbdirs);
...
int main(void) {
    ...
    double complex A_output[MAX_SIZE * MAX_SIZE];
    double complex A_ad_output[MAX_SIZE * MAX_SIZE];
    double complex A_forward[MAX_SIZE * MAX_SIZE], A_backward[MAX_SIZE * MAX_SIZE];
    ...
    /* Verify AD primal output matches original (same as _d) */
    {
        ...
        for (i = 0; i < MAX_SIZE * MAX_SIZE; i++) {
            double diff = cabs(A_ad_output[i] - A_output[i]);
