/* Test program for cblas_stbmv forward vector (dv) differentiation */
...
extern void cblas_stbmv_dv(CBLAS_LAYOUT layout, CBLAS_UPLO Uplo, CBLAS_TRANSPOSE TransA, CBLAS_DIAG Diag, CBLAS_INT N, CBLAS_INT K, const float *A, float (*Ad)[NBDirsMax], CBLAS_INT lda, float *X, float (*Xd)[NBDirsMax], CBLAS_INT incX, int nbdirs);
