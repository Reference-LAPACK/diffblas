/* Test program for cblas_dsyrk differentiation */
...
/* Original function */
extern void cblas_dsyrk(const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo, const CBLAS_TRANSPOSE Trans, const CBLAS_INT N, const CBLAS_INT K, const double alpha, const double *A, const CBLAS_INT lda, const double beta, double *C, const CBLAS_INT ldc);
/* Differentiated function */
extern void cblas_dsyrk_d(const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo, const CBLAS_TRANSPOSE Trans, const CBLAS_INT N, const CBLAS_INT K, const double alpha, double alpha_d, const double *A, double *A_d, const CBLAS_INT lda, const double beta, double beta_d, double *C, double *C_d, const CBLAS_INT ldc);
...
int main(void) {
    ...
    double C_d[MAX_SIZE * MAX_SIZE];  /* Derivative seeds */
    double C_d_orig[MAX_SIZE * MAX_SIZE];  /* Save derivative seeds for finite differences */
    ...
    for (i = 0; i < MAX_SIZE * MAX_SIZE; i++) {
        C[i] = ((double)rand() / RAND_MAX) * 2.0 - 1.0;
    }
    ...
    for (i = 0; i < MAX_SIZE * MAX_SIZE; i++) {
        C_d[i] = ((double)rand() / RAND_MAX) * 2.0 - 1.0;
    }
    ...
    for (i = 0; i < MAX_SIZE * MAX_SIZE; i++) {
        double fd_derivative = (C_forward[i] - C_backward[i]) / (2.0 * h);
        double ad_derivative = C_d[i];

        double abs_error = fabs(fd_derivative - ad_derivative);
        double ad_ref = (fabs(ad_derivative) > 1.0e-10) ? fabs(ad_derivative) : 1.0e-10;
