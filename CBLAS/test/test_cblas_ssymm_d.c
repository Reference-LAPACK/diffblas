/* Test program for cblas_ssymm differentiation */
...
extern void cblas_ssymm_d(const CBLAS_LAYOUT layout, const CBLAS_SIDE Side, const CBLAS_UPLO Uplo, const CBLAS_INT M, const CBLAS_INT N, const float alpha, float alpha_d, const float *A, float *A_d, const CBLAS_INT lda, const float *B, float *B_d, const CBLAS_INT ldb, const float beta, float beta_d, float *C, float *C_d, const CBLAS_INT ldc);
...
int main(void) {
    ...
    for (i = 0; i < MAX_SIZE; i++) {
        for (j = i; j < MAX_SIZE; j++) {
            A_d[i * MAX_SIZE + j] = ((float)rand() / RAND_MAX) * 2.0f - 1.0f;
        }
    }
    for (i = 1; i < MAX_SIZE; i++) {
        for (j = 0; j < i; j++) {
