/* Test program for cblas_daxpy reverse mode (VJP verification, generic) */
...
extern void cblas_daxpy_b(const CBLAS_INT N, const double alpha, double *alpha_b, const double *X, double *X_b, const CBLAS_INT incX, double *Y, double *Y_b, const CBLAS_INT incY);
...
int main(void) {
    ...
    cblas_daxpy_b(n, alpha, &alpha_b, X, X_b, incX, Y, Y_b, incY);
