/* Test program for cblas_dgemm reverse mode (VJP verification) */
...
extern void cblas_dgemm(const CBLAS_LAYOUT layout, const CBLAS_TRANSPOSE TransA, const CBLAS_TRANSPOSE TransB, const CBLAS_INT M, const CBLAS_INT N, const CBLAS_INT K, const double alpha, const double *A, const CBLAS_INT lda, const double *B, const CBLAS_INT ldb, const double beta, double *C, const CBLAS_INT ldc);
extern void cblas_dgemm_b(const CBLAS_LAYOUT layout, const CBLAS_TRANSPOSE TransA, const CBLAS_TRANSPOSE TransB, const CBLAS_INT M, const CBLAS_INT N, const CBLAS_INT K, const double alpha, double *alpha_b, const double *A, double *A_b, const CBLAS_INT lda, const double *B, double *B_b, const CBLAS_INT ldb, const double beta, double *beta_b, double *C, double *C_b, const CBLAS_INT ldc);
...
int main(void) {
    ...
    double alpha, alpha_b, alpha_dir;
    double beta, beta_b, beta_dir;
    double A[MAX_SIZE*MAX_SIZE], B[MAX_SIZE*MAX_SIZE], C[MAX_SIZE*MAX_SIZE];
    double A_b[MAX_SIZE*MAX_SIZE], B_b[MAX_SIZE*MAX_SIZE], C_b[MAX_SIZE*MAX_SIZE];
    double A_dir[MAX_SIZE*MAX_SIZE], B_dir[MAX_SIZE*MAX_SIZE], C_dir[MAX_SIZE*MAX_SIZE];
    double C_forward[MAX_SIZE*MAX_SIZE], C_backward[MAX_SIZE*MAX_SIZE];
    double C_b_orig[MAX_SIZE*MAX_SIZE];  /* save cotangent before _b overwrites */
    double alpha_orig, beta_orig, A_orig[MAX_SIZE*MAX_SIZE], B_orig[MAX_SIZE*MAX_SIZE], C_orig[MAX_SIZE*MAX_SIZE];  /* for restore like BLAS test */
    ...
    for (i = 0; i < MAX_SIZE*MAX_SIZE; i++) {
        C_b[i] = (rand()/(double)RAND_MAX)*2.0 - 1.0;
    ...
    alpha_b = 0.0; beta_b = 0.0;
    for (i = 0; i < MAX_SIZE*MAX_SIZE; i++) { A_b[i] = 0.0; B_b[i] = 0.0; }
    /* Call reverse mode: interleaved (primal, adjoint) per Tapenade signature */
    cblas_dgemm_b(CblasColMajor, CblasNoTrans, CblasNoTrans, m, n, k,
        alpha, &alpha_b, A, A_b, lda, B, B_b, ldb, beta, &beta_b, C, C_b, ldc);
    ...
    /* VJP left side: cotangent^T @ central_diff (FD), sorted summation - match BLAS test_dgemm_reverse.f90 */
    {
        ...
        for (i = 0; i < n_products; i++) temp_products[i] = C_b_orig[i] * ((C_forward[i] - C_backward[i]) / (2.0*h));
    ...
    vjp_ad = 0.0;
    vjp_ad += alpha_dir * alpha_b + beta_dir * beta_b;
    {
        double temp_products[MAX_SIZE*MAX_SIZE];
        int n_products = MAX_SIZE*MAX_SIZE, idx;
        for (i = 0; i < n_products; i++) temp_products[i] = A_dir[i] * A_b[i];
        qsort(temp_products, (size_t)n_products, sizeof(double), compare_abs_d);
        for (idx = 0; idx < n_products; idx++) vjp_ad += temp_products[idx];
        for (i = 0; i < n_products; i++) temp_products[i] = B_dir[i] * B_b[i];
        qsort(temp_products, (size_t)n_products, sizeof(double), compare_abs_d);
        for (idx = 0; idx < n_products; idx++) vjp_ad += temp_products[idx];
        for (i = 0; i < n_products; i++) temp_products[i] = C_dir[i] * C_b[i];
        qsort(temp_products, (size_t)n_products, sizeof(double), compare_abs_d);
        for (idx = 0; idx < n_products; idx++) vjp_ad += temp_products[idx];
    }
