/* Test program for cblas_cdotc_sub differentiation */
...
extern void cblas_cdotc_sub_d(const CBLAS_INT N, const void *X, void *X_d, const CBLAS_INT incX, const void *Y, void *Y_d, const CBLAS_INT incY, void *dotc, void *dotc_d);
...
int main(void) {
    ...
    CBLAS_INT incY;
    float complex dotc[MAX_SIZE];
    float complex dotc_d[MAX_SIZE];  /* Derivative seeds */
    float complex dotc_d_orig[MAX_SIZE];  /* Save derivative seeds for finite differences */
    float complex dotc_orig[MAX_SIZE];
    ...
    for (i = 0; i < MAX_SIZE; i++) {
        dotc[i] = ((float)rand() / RAND_MAX) * 2.0f - 1.0f + I * (((float)rand() / RAND_MAX) * 2.0f - 1.0f);
    }
    ...
    for (i = 0; i < MAX_SIZE; i++) {
        dotc_d[i] = ((float)rand() / RAND_MAX) * 2.0f - 1.0f + I * (((float)rand() / RAND_MAX) * 2.0f - 1.0f);
    }
