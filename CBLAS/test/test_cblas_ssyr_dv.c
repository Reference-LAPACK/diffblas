/* Test program for cblas_ssyr forward vector (dv) differentiation */
...
extern void cblas_ssyr_dv(CBLAS_LAYOUT layout, CBLAS_UPLO Uplo, CBLAS_INT N, float alpha, float alphad[NBDirsMax], const float *X, float (*Xd)[NBDirsMax], CBLAS_INT incX, float *A, float (*Ad)[NBDirsMax], CBLAS_INT lda, int nbdirs);
