/* Test program for cblas_zgeru vector reverse mode (VJP verification, generic, loop over directions) */
...
extern void cblas_zgeru(const CBLAS_LAYOUT layout, const CBLAS_INT M, const CBLAS_INT N, const void *alpha, const void *X, const CBLAS_INT incX, const void *Y, const CBLAS_INT incY, void *A, const CBLAS_INT lda);
...
int main(void) {
    ...
    double complex A_plus[MAX_SIZE*MAX_SIZE], A_minus[MAX_SIZE*MAX_SIZE];
    ...
    for (i = 0; i < MAX_SIZE*MAX_SIZE; i++) for (j = 0; j < NBDirsMax; j++) { A_b[i][j] = (rand()/(double)RAND_MAX)*2.0 - 1.0 + I*((rand()/(double)RAND_MAX)*2.0 - 1.0); A_b_orig[i][j] = A_b[i][j]; }
    ...
    cblas_zgeru_bv(layout, m, n, &alpha, (void*)&alpha_b, X, (void*)X_b, incX, Y, (void*)Y_b, incY, A, (void*)A_b, lda, nbdirs);
    ...
    for (idir = 0; idir < nbdirs; idir++) {
        ...
        vjp_fd = 0.0;
        {
            ...
            for (i = 0; i < MAX_SIZE*MAX_SIZE; i++) temp_products[i] = creal(conj(A_b_orig[i][idir]) * ((A_plus[i] - A_minus[i]) / (2.0*h)));
