/* Test program for cblas_ztpmv vector reverse mode (VJP verification, generic, loop over directions) */
...
int main(void) {
    ...
    double complex Ap[PACKED_SIZE], Ap_orig[PACKED_SIZE], Ap_dir[PACKED_SIZE];
    double complex Ap_b[PACKED_SIZE][NBDirsMax], Ap_b_orig[PACKED_SIZE][NBDirsMax];
    ...
    for (i = 0; i < PACKED_SIZE; i++) for (j = 0; j < NBDirsMax; j++) Ap_b[i][j] = 0.0;

    cblas_ztpmv_bv(layout, uplo, transa, diag, n, Ap, (void*)Ap_b, X, (void*)X_b, incX, nbdirs);
