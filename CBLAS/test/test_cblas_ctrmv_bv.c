/* Test program for cblas_ctrmv vector reverse mode (VJP verification, generic, loop over directions) */
...
extern void cblas_ctrmv(const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo, const CBLAS_TRANSPOSE TransA, const CBLAS_DIAG Diag, const CBLAS_INT N, const void *A, const CBLAS_INT lda, void *X, const CBLAS_INT incX);
...
int main(void) {
    ...
    float complex X_plus[MAX_SIZE], X_minus[MAX_SIZE];
    ...
    for (j = 0; j < n; j++) {
        for (i = 0; i < n; i++) {
            if (uplo == CblasUpper && i > j) { A[i + j*lda] = 0.0f; A_dir[i + j*lda] = 0.0f; }
            if (uplo == CblasLower && i < j) { A[i + j*lda] = 0.0f; A_dir[i + j*lda] = 0.0f; }
        }
        if (diag == CblasUnit) { A[j + j*lda] = 1.0f; A_dir[j + j*lda] = 0.0f; }
    }
    memcpy(A_orig, A, sizeof(A[0])*(MAX_SIZE*MAX_SIZE));
    ...
    cblas_ctrmv_bv(layout, uplo, transa, diag, n, A, (void*)A_b, lda, X, (void*)X_b, incX, nbdirs);
    ...
    for (idir = 0; idir < nbdirs; idir++) {
        ...
        for (i = 0; i < MAX_SIZE*MAX_SIZE; i++) A_dir[i] = (rand()/(double)RAND_MAX)*2.0 - 1.0 + I*((rand()/(double)RAND_MAX)*2.0 - 1.0);
        for (j = 0; j < n; j++) { for (i = 0; i < n; i++) { if (uplo == CblasUpper && i > j) A_dir[i + j*lda] = 0.0f; if (uplo == CblasLower && i < j) A_dir[i + j*lda] = 0.0f; } if (diag == CblasUnit) A_dir[j + j*lda] = 0.0f; }
        ...
        cblas_ctrmv(layout, uplo, transa, diag, n, A, lda, X, incX);
