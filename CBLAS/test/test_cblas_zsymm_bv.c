/* Test program for cblas_zsymm vector reverse mode (VJP verification, generic, loop over directions) */
...
extern void cblas_zsymm(const CBLAS_LAYOUT layout, const CBLAS_SIDE Side, const CBLAS_UPLO Uplo, const CBLAS_INT M, const CBLAS_INT N, const void *alpha, const void *A, const CBLAS_INT lda, const void *B, const CBLAS_INT ldb, const void *beta, void *C, const CBLAS_INT ldc);
...
int main(void) {
    ...
    for (i = 1; i < MAX_SIZE; i++) {
        for (j = 0; j < i; j++) {
            A_dir[i * MAX_SIZE + j] = A_dir[j * MAX_SIZE + i];  /* symmetric */
        }
    }
    ...
    cblas_zsymm_bv(layout, side, uplo, m, n, &alpha, (void*)&alpha_b, A, (void*)A_b, lda, B, (void*)B_b, ldb, &beta, (void*)&beta_b, C, (void*)C_b, ldc, nbdirs);
