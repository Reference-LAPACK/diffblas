/* Test program for cblas_zdotc_sub reverse mode (VJP verification, generic) */
...
extern void cblas_zdotc_sub_b(const CBLAS_INT N, const void *X, void *X_b, const CBLAS_INT incX, const void *Y, void *Y_b, const CBLAS_INT incY, void *dotc, void *dotc_b);
...
int main(void) {
    ...
    double complex dotc[1], dotc_b[1], dotc_orig[1], dotc_dir[1];
    double complex dotc_plus[1], dotc_minus[1], dotc_central_diff[1], dotc_b_orig[1];
    ...
    for (i = 0; i < 1; i++) { dotc_b[i] = (rand()/(double)RAND_MAX)*2.0 - 1.0 + I*((rand()/(double)RAND_MAX)*2.0 - 1.0); dotc_b_orig[i] = dotc_b[i]; }
    for (i = 0; i < MAX_SIZE; i++) X_b[i] = 0.0;
    for (i = 0; i < MAX_SIZE; i++) Y_b[i] = 0.0;

    cblas_zdotc_sub_b(n, X, X_b, incX, Y, Y_b, incY, dotc, dotc_b);
    ...
    vjp_fd = 0.0;
    {
        ...
        for (i = 0; i < 1; i++) temp_products[i] = creal(conj(dotc_b_orig[i]) * ((dotc_plus[i] - dotc_minus[i]) / (2.0*h)));
