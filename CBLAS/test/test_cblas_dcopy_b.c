/* Test program for cblas_dcopy reverse mode (VJP verification, generic) */
...
extern void set_isize1ofdx_(int *val);

static int compare_abs_d(const void *a, const void *b) { double x = fabs(*(const double*)a), y = fabs(*(const double*)b); return (x > y) - (x < y); }

extern void cblas_dcopy(const CBLAS_INT N, const double *X, const CBLAS_INT incX, double *Y, const CBLAS_INT incY);
extern void cblas_dcopy_b(const CBLAS_INT N, const double *X, double *X_b, const CBLAS_INT incX, double *Y, double *Y_b, const CBLAS_INT incY);

int main(void) {
    int i, j, idx, n_products;
    {
        int diffblas_isize = MAX_SIZE;
        set_isize1ofdx_(&diffblas_isize);
    }
    double h = 1.0e-7;
    ...
    cblas_dcopy_b(n, X, X_b, incX, Y, Y_b, incY);
