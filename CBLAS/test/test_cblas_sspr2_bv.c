/* Test program for cblas_sspr2 vector reverse mode (VJP verification, generic, loop over directions) */
...
int main(void) {
    ...
    float A[PACKED_SIZE], A_orig[PACKED_SIZE], A_dir[PACKED_SIZE];
    float A_b[PACKED_SIZE][NBDirsMax], A_b_orig[PACKED_SIZE][NBDirsMax];
    float A_plus[PACKED_SIZE], A_minus[PACKED_SIZE];
    ...
    cblas_sspr2_bv(layout, uplo, n, alpha, &alpha_b, X, X_b, incX, Y, Y_b, incY, A, &A_b[0][0], nbdirs);
