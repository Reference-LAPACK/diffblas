/* Test program for cblas_sgemv forward vector (dv) differentiation */
...
extern void cblas_sgemv_dv(CBLAS_LAYOUT layout, CBLAS_TRANSPOSE TransA, CBLAS_INT M, CBLAS_INT N, float alpha, float alphad[NBDirsMax], const float *A, float (*Ad)[NBDirsMax], CBLAS_INT lda, const float *X, float (*Xd)[NBDirsMax], CBLAS_INT incX, float beta, float betad[NBDirsMax], float *Y, float (*Yd)[NBDirsMax], CBLAS_INT incY, int nbdirs);
