/* Test program for cblas_zdotc_sub differentiation */
...
/* Original function */
extern void cblas_zdotc_sub(const CBLAS_INT N, const void *X, const CBLAS_INT incX, const void *Y, const CBLAS_INT incY, void *dotc);
/* Differentiated function */
extern void cblas_zdotc_sub_d(const CBLAS_INT N, const void *X, void *X_d, const CBLAS_INT incX, const void *Y, void *Y_d, const CBLAS_INT incY, void *dotc, void *dotc_d);
...
int main(void) {
    ...
    CBLAS_INT incY;
    double complex dotc[MAX_SIZE];
    double complex dotc_d[MAX_SIZE];  /* Derivative seeds */
    double complex dotc_d_orig[MAX_SIZE];  /* Save derivative seeds for finite differences */
    double complex dotc_orig[MAX_SIZE];
    ...
    for (i = 0; i < MAX_SIZE; i++) {
        dotc[i] = ((double)rand() / RAND_MAX) * 2.0 - 1.0 + I * (((double)rand() / RAND_MAX) * 2.0 - 1.0);
    }
    ...
    for (i = 0; i < MAX_SIZE; i++) {
        dotc_d[i] = ((double)rand() / RAND_MAX) * 2.0 - 1.0 + I * (((double)rand() / RAND_MAX) * 2.0 - 1.0);
    }
    ...
    memcpy(dotc_d_orig, dotc_d, sizeof(dotc_d));
    ...
    memcpy(dotc_orig, dotc, sizeof(dotc));
